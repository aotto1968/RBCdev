 ...
 *  rbcUnixDnd.c --
 *
 *	This module implements a drag-and-drop manager for the RBC
...
#define DND_THREAD_KEY	"RBC Dnd Data"
 ...
 *   a "RbcDndTarget" property in XA_STRING format.  This property 
 ...
 *   "RbcDndTarget" property are identified.  When the token is dropped 
 ...
 *					    rbc::dnd pull to transfer the data
...
typedef struct {
    ...
    int protocol;		/* Drag-and-drop protocol used by the source:
				 * either PROTO_RBC or PROTO_XDND. */
} DropPending;
...
typedef struct WinfoStruct {
    ...
    Rbc_Chain *chainPtr;	/* List of this window's children. If NULL,
    ...
    int offset;			/* Offset of field in record. */
{
	...
	*cursorPtrPtr = NULL;
	...
	cursorArr = RbcCalloc(nElems + 1, sizeof(Tk_Cursor));
	for (i = 0; i < nElems; i++) {
    ...
    Window window;
{
	...
	for (i = 0; i < nChildren; i++) {
	     ...
	     */
	    Rbc_ChainPrepend(chainPtr, (ClientData)childArr[i]);
	}
	if (childArr != NULL) {
    ...
    Display *display;
{
    ...
    size = Rbc_MaxRequestSize(display, sizeof(char));
    size -= 32;
    return size;
}
    ...
    int x, y;
{
     ...
     */
  descend:
    for (linkPtr = Rbc_ChainFirstLink(rootPtr->chainPtr); linkPtr != NULL;
	linkPtr = Rbc_ChainNextLink(linkPtr)) {
	rootPtr = Rbc_ChainGetValue(linkPtr);
	if (!rootPtr->initialized) {
	    GetWinfo(dndPtr->display, rootPtr);
	}
	if (rootPtr->window == Rbc_GetRealWindowId(dndPtr->tokenPtr->tkwin)) {
    ...
    Tk_Window tkwin;		/* Window of drag&drop source. */
{
    ...
    cursor = None;
    Tcl_DStringInit(&dString);
    Rbc_DStringAppendElements(&dString, Tk_PathName(tkwin), "cget", "-cursor",
	      (char *)NULL);
    ...
    Dnd *dndPtr;
{
	...
	if ((Tk_Width(tokenPtr->tkwin) != Tk_ReqWidth(tokenPtr->tkwin)) ||
	    ...
	    Rbc_ResizeToplevel(tokenPtr->tkwin, 
		Tk_ReqWidth(tokenPtr->tkwin),
		Tk_ReqHeight(tokenPtr->tkwin));
	}
	Rbc_MapToplevel(tokenPtr->tkwin);
	Rbc_RaiseToplevel(tokenPtr->tkwin);
    }
}
    ...
    ClientData clientData;
{
    ...
    Rbc_Fill3DRectangle(tokenPtr->tkwin, Tk_WindowId(tokenPtr->tkwin), border, 
...
{ 
    ...
    window = Rbc_GetRealWindowId(tokenPtr->tkwin);
...
{ 
    ...
    Rbc_TranslateAnchor(x, y, Tk_Width(tokenPtr->tkwin),
    ...
    int status;
{
	...
	Tcl_DStringAppendElement(&dString, Rbc_Utoa(dndPtr->timestamp));
    ...
    Dnd *dndPtr;
{
    ...
    tokenPtr = RbcCalloc(1, sizeof(Token));
    assert(tokenPtr);
    ...
    Tcl_DString *resultPtr;
{
    ...
    x = dndPtr->dragX - Rbc_RootX(dndPtr->tkwin);
    y = dndPtr->dragY - Rbc_RootY(dndPtr->tkwin);
    Tcl_DStringAppendElement(&dString, Tk_PathName(dndPtr->tkwin));
    Tcl_DStringAppendElement(&dString, "x");
    Tcl_DStringAppendElement(&dString, Rbc_Itoa(x));
    Tcl_DStringAppendElement(&dString, "y");
    Tcl_DStringAppendElement(&dString, Rbc_Itoa(y));
    Tcl_DStringAppendElement(&dString, "timestamp");
    Tcl_DStringAppendElement(&dString, Rbc_Utoa(timestamp));
    ...
    Dnd *dndPtr;
{
    ...
    if (!Tcl_GetCommandInfo(interp, "::rbc::Dnd::ObjInit", &cmdInfo)) {
	static char cmd[] = "source [file join $::rbci::library dnd.tcl]";
	/* 
	 * If the "DndInit" routine hasn't been sourced, do it now.
	 */
	if (Tcl_GlobalEval(interp, cmd) != TCL_OK) {
	    Tcl_AddErrorInfo(interp,
		     "\n    (while loading bindings for ::rbc::dnd)");
	    return TCL_ERROR;
	}
    }
    /*  
     ...
     */
    if (Rbc_ConfigModified(configSpecs, "-target", "-onenter",  "-onmotion",
	   "-onleave", (char *)NULL)) {
    ...
    Tcl_DStringInit(&dString);
    Rbc_DStringAppendElements(&dString, "::rbc::Dnd::ObjInit", 
	Tk_PathName(dndPtr->tkwin), Rbc_Itoa(button), (char *)NULL);
    ...
    XEvent *eventPtr;
{
    ...
    Tcl_DStringAppendElement(&dString, "timestamp");
    Tcl_DStringAppendElement(&dString, Rbc_Utoa(dropPtr->timestamp));
    ...
    int timestamp;
{
	...
	if (data != NULL) {
	    dndPtr->matchingFormats = RbcStrdup(data);
	    XFree(data);
	}
    ...
    int button, keyState, timestamp;
{
    ...
    Tcl_DStringAppendElement(&dString, Tk_PathName(dndPtr->tkwin));
    x -= Rbc_RootX(dndPtr->tkwin); /* Send coordinates relative to target. */
    y -= Rbc_RootY(dndPtr->tkwin);
    Tcl_DStringAppendElement(&dString, "x");
    Tcl_DStringAppendElement(&dString, Rbc_Itoa(x));
    Tcl_DStringAppendElement(&dString, "y");
    Tcl_DStringAppendElement(&dString, Rbc_Itoa(y));
    ...
    Tcl_DStringAppendElement(&dString, Rbc_Itoa(button));
    Tcl_DStringAppendElement(&dString, "state");
    Tcl_DStringAppendElement(&dString, Rbc_Itoa(keyState));
    Tcl_DStringAppendElement(&dString, "timestamp");
    Tcl_DStringAppendElement(&dString, Rbc_Utoa(timestamp));
    Tcl_Preserve(interp);
    Tcl_DStringInit(&savedResult);
    ...
    int button, keyState, timestamp;
{
	...
	Tcl_DStringAppendElement(&dString, "::rbc::Dnd::StdDrop");
    }
    Tcl_DStringAppendElement(&dString, Tk_PathName(dndPtr->tkwin));
    dndPtr->dropX = x - Rbc_RootX(dndPtr->tkwin);
    dndPtr->dropY = y - Rbc_RootY(dndPtr->tkwin);
    Tcl_DStringAppendElement(&dString, "x");
    Tcl_DStringAppendElement(&dString, Rbc_Itoa(dndPtr->dropX));
    Tcl_DStringAppendElement(&dString, "y");
    Tcl_DStringAppendElement(&dString, Rbc_Itoa(dndPtr->dropY));
    Tcl_DStringAppendElement(&dString, "formats");
    Tcl_DStringAppendElement(&dString, formats);
    Tcl_DStringAppendElement(&dString, "button");
    Tcl_DStringAppendElement(&dString, Rbc_Itoa(button));
    ...
    Winfo *windowPtr;		/* window rep to be freed */
{
    ...
    for (linkPtr = Rbc_ChainFirstLink(windowPtr->chainPtr); linkPtr != NULL;
	linkPtr = Rbc_ChainNextLink(linkPtr)) {
	childPtr = Rbc_ChainGetValue(linkPtr);
	FreeWinfo(childPtr);	/* Recursively free children. */
    }
    if (windowPtr->matches != NULL) {
	ckfree(windowPtr->matches);
    }
    Rbc_ChainDestroy(windowPtr->chainPtr);
    ckfree(windowPtr);
}
    ...
    Winfo *windowPtr;		/* window rep to be initialized */
{
	...
	for (linkPtr = Rbc_ChainFirstLink(chainPtr); linkPtr != NULL;
	    linkPtr = Rbc_ChainNextLink(linkPtr)) {
	    childPtr = RbcCalloc(1, sizeof(Winfo));
	    assert(childPtr);
	    childPtr->initialized = FALSE;
	    childPtr->window = (Window)Rbc_ChainGetValue(linkPtr);
	    childPtr->parentPtr = windowPtr;
	    Rbc_ChainSetValue(linkPtr, childPtr);
	}
 ...
 *	window.  Queries the "RbcDndTarget" property info where
    ...
    Dnd *dndPtr;
{
    ...
    rootPtr = RbcCalloc(1, sizeof(Winfo));
    assert(rootPtr);
    ...
    char *data;
{
	...
	windowPtr->matches = RbcStrdup(Tcl_DStringValue(&dString));
    }
    Tcl_DStringFree(&dString);	
    ...
    Dnd *dndPtr;		/* drag&drop source window */
{
	...
	/* See if this window has a "RbcDndTarget" property. */
    ...
    int x, y;
{
	...
	Tcl_DStringAppendElement(&dString, Tk_PathName(dndPtr->tkwin));
	rx = dndPtr->dragX - Rbc_RootX(dndPtr->tkwin);
	ry = dndPtr->dragY - Rbc_RootY(dndPtr->tkwin);
	Tcl_DStringAppendElement(&dString, "x");
	Tcl_DStringAppendElement(&dString, Rbc_Itoa(rx));
	Tcl_DStringAppendElement(&dString, "y");
	Tcl_DStringAppendElement(&dString, Rbc_Itoa(ry));
	Tcl_DStringAppendElement(&dString, "button");
	Tcl_DStringAppendElement(&dString, Rbc_Itoa(dndPtr->button));
	Tcl_DStringAppendElement(&dString, "state");
	Tcl_DStringAppendElement(&dString, Rbc_Itoa(dndPtr->keyState));
	Tcl_DStringAppendElement(&dString, "timestamp");
	Tcl_DStringAppendElement(&dString, Rbc_Utoa(dndPtr->timestamp));
    ...
    char **argv;
{
	...
	Tcl_DStringAppendElement(&dString, "timestamp");
	Tcl_DStringAppendElement(&dString, 
		Rbc_Utoa(dndPtr->pendingPtr->timestamp));
	Tcl_DStringAppendElement(&dString, "format");
...
static Rbc_OpSpec tokenOps[] =
{
    {"cget", 2, (Rbc_Op)TokenCgetOp, 5, 5, "widget option",},
    {"configure", 2, (Rbc_Op)TokenConfigureOp, 4, 0, 
	"widget ?option value?...",},
    {"window", 5, (Rbc_Op)TokenWindowOp, 4, 0, 
	"widget ?option value?...",},
};

static int nTokenOps = sizeof(tokenOps) / sizeof(Rbc_OpSpec);
    ...
    char **argv;
{
    ...
    proc = Rbc_GetOp(interp, nTokenOps, tokenOps, RBC_OP_ARG2, argc, argv, 0);
    if (proc == NULL) {
	return TCL_ERROR;
    }
...
static Rbc_OpSpec dndOps[] =
{
    {"cancel", 2, (Rbc_Op)CancelOp, 3, 3, "widget",},
    {"cget", 2, (Rbc_Op)CgetOp, 4, 4, "widget option",},
    {"configure", 4, (Rbc_Op)ConfigureOp, 3, 0, 
	"widget ?option value?...",},
#ifdef notdef
    {"convert", 4, (Rbc_Op)ConvertOp, 5, 5, "widget data format",},
#endif
    {"delete", 2, (Rbc_Op)DeleteOp, 3, 0,"?-source|-target? widget...",},
    {"drag", 3, (Rbc_Op)DragOp, 3, 0, "widget x y ?token?",},
    {"drop", 3, (Rbc_Op)DropOp, 3, 0, "widget x y ?token?",},
    {"getdata", 1, (Rbc_Op)GetdataOp, 3, 0, "widget ?format command?",},
    {"names", 1, (Rbc_Op)NamesOp, 2, 4, "?-source|-target? ?pattern?",},
    {"pull", 1, (Rbc_Op)PullOp, 4, 4, "widget format",},
    {"register", 1, (Rbc_Op)RegisterOp, 3, 0, "widget ?option value?...",},
    {"select", 3, (Rbc_Op)SelectOp, 6, 6, "widget x y timestamp",},
    {"setdata", 3, (Rbc_Op)SetdataOp, 3, 0, "widget ?format command?",},
    {"token", 5, (Rbc_Op)TokenOp, 3, 0, "args...",},
};

static int nDndOps = sizeof(dndOps) / sizeof(Rbc_OpSpec);
    ...
    char **argv;		/* argument strings */
{
    ...
    proc = Rbc_GetOp(interp, nDndOps, dndOps, RBC_OP_ARG1, argc, argv, 0);
    if (proc == NULL) {
	return TCL_ERROR;
    }
    result = (*proc) (clientData, interp, argc, argv);
     ...
     Tcl_Interp *interp;
{
	...
	dataPtr = ckalloc(sizeof(DndInterpData));
	assert(dataPtr);
	...
	dataPtr->mesgAtom = XInternAtom(display, "RBC Dnd Message", False);
	dataPtr->targetAtom = XInternAtom(display, "RBC Dnd Target", False);
	dataPtr->formatsAtom = XInternAtom(display, "RBC Dnd Formats",False);
	dataPtr->commAtom = XInternAtom(display, "RBC Dnd CommData", False);
...
{
    if (Tcl_CreateCommand(interp, "::rbc::dnd", DndCmd, 
          (ClientData)GetDndInterpData(interp), 
 ...
 */
static void
Rbc_DndSetOutlineBitmap(tkwin, bitmap, x, y)
    Tk_Window tkwin;
    Pixmap bitmap;
    int x, y;
{
    ...
    XEvent *eventPtr;
{
    ...
    formatArr = RbcCalloc(nAtoms + 1, sizeof(char *));
    ...
    char **formatArr;
{
	...
	string = RbcStrdup(Tcl_DStringValue(&dString));
	Tcl_DStringFree(&dString);
	return string;
    }
    return NULL;
    ...
    Dnd *dndPtr;
{
     ...
     */
    tkwin = Rbc_GetToplevel(dndPtr->tkwin);
    window = Rbc_GetRealWindowId(tkwin); /* Use the wrapper window as
	...
	handlerPtr = ckalloc(sizeof(XDndHandler));
	...
	Tcl_SetHashValue(hPtr, handlerPtr);
    }
    return handlerPtr;
}
    ...
    Dnd *dndPtr;
{
    ...
    tkwin = Rbc_GetToplevel(dndPtr->tkwin);
    window = Rbc_GetRealWindowId(tkwin); /* Use the wrapper window as the real 
					  * toplevel window. */
