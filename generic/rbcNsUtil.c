...
/*
TclVarHashTable *
Rbc_GetArrayVariableTable(interp, varName, flags)
    Tcl_Interp *interp;
    CONST char *varName;
    int flags;
{
...
/*
Tcl_Namespace *
Rbc_GetVariableNamespace(interp, name)
    Tcl_Interp *interp;
    CONST char *name;
{
    Var *varPtr;
...
Tcl_CallFrame *
Rbc_EnterNamespace(interp, nsPtr)
    Tcl_Interp *interp;
    Tcl_Namespace *nsPtr;
{
    Tcl_CallFrame *framePtr;

    framePtr = ckalloc(sizeof(Tcl_CallFrame));
    assert(framePtr);
...
void
Rbc_LeaveNamespace(interp, framePtr)
    Tcl_Interp *interp;
    Tcl_CallFrame *framePtr;
{
...
Tcl_Namespace *
Rbc_GetCommandNamespace(interp, cmdToken)
...
int
Rbc_ParseQualifiedName(interp, qualName, nsPtrPtr, namePtrPtr)
    Tcl_Interp *interp;
    CONST char *qualName;
...
char *
Rbc_GetQualifiedName(nsPtr, name, resultPtr)
    Tcl_Namespace *nsPtr;
    CONST char *name;
    ...
    ClientData clientData;
{
    ...
    list = (Rbc_List)clientData;
    for (node = Rbc_ListFirstNode(list); node != NULL;
	node = Rbc_ListNextNode(node)) {
	deleteProc = (Tcl_CmdDeleteProc *)Rbc_ListGetValue(node);
	clientData = (ClientData)Rbc_ListGetKey(node);
...
/*
void
Rbc_DestroyNsDeleteNotify(interp, nsPtr, clientData)
    Tcl_Interp *interp;
    Tcl_Namespace *nsPtr;
    ClientData clientData;
{
    ...
    string = ckalloc(sizeof(nsPtr->fullName) + strlen(NS_DELETE_CMD) + 4);
    ...
    list = (Rbc_List)cmdInfo.clientData;
    node = Rbc_ListGetNode(list, clientData);
    if (node != NULL) {
	Rbc_ListDeleteNode(node);
    }
  done:
    ckfree(string);
}
...
/*
int
Rbc_CreateNsDeleteNotify(interp, nsPtr, clientData, deleteProc)
    Tcl_Interp *interp;
    Tcl_Namespace *nsPtr;
    ClientData clientData;
    Tcl_CmdDeleteProc *deleteProc;
{
	...
	Rbc_CreateCommand(interp, string, NamespaceDeleteCmd, list, 
		NamespaceDeleteNotify);
    } else {
	list = (Rbc_List)cmdInfo.clientData;
    }
    ckfree(string);
    Rbc_ListAppend(list, clientData, (ClientData)deleteProc);
    return TCL_OK;
}
*/
...
// */
//Tcl_Command
//Rbc_CreateCommand(interp, cmdName, proc, clientData, deleteProc)
...
// */
//Tcl_Command
//Rbc_CreateCommandObj(interp, cmdName, proc, clientData, deleteProc)
//    Tcl_Interp *interp;		/* Token for command interpreter returned by
//				 * a previous call to Tcl_CreateInterp. */
//    CONST char *cmdName;	/* Name of command. If it contains namespace
//				 * qualifiers, the new command is put in the
