...
/*
 *	Each "drag&drop" target widget is tagged with a "RbcDrag&DropTarget" 
 ...
 *	"RbcDrag&DropTarget" property are identified.  When the token is 
...
static char dragDropCmd[] = "::rbc::drag&drop";

static char className[] = "DragDropToken";	/* CLASS NAME of token window */
static char propName[] = "RbcDrag&DropTarget";	/* Property name */
...
static Tk_ConfigSpec configSpecs[] =
{
	...
	Tk_Offset(Source, sendTypes), TK_CONFIG_NULL_OK, &rbcListOption},
...
{
    Rbc_Chain *chainPtr = (Rbc_Chain *) clientData;

    Rbc_ChainAppend(chainPtr, (ClientData)hWnd);
    return TRUE;
}
    ...
    int active;
{
    ...
    Rbc_Fill3DRectangle(tokenPtr->tkwin, Tk_WindowId(tokenPtr->tkwin),
    ...
    int *newPtr;		/* returns non-zero => new record created */
{
    ...
    srcPtr = RbcCalloc(1, sizeof(Source));
    assert(srcPtr);
    srcPtr->tkwin = tkwin;
    ...
    int flags;			/* flags controlling interpretation */
{
    ...
    if (!Tcl_GetCommandInfo(interp, "::rbc::Drag&Drop::ObjInit", &cmdInfo)) {
	static char cmd[] = "source [file join $::rbci::library dragdrop.tcl]";

	if (Tcl_GlobalEval(interp, cmd) != TCL_OK) {
	    Tcl_AddErrorInfo(interp,
		    "\n    (while loading bindings for ::rbc::drag&drop)");
	    return TCL_ERROR;
	}
    }
    Tcl_DStringInit(&dString);
    Rbc_DStringAppendElements(&dString, "::rbc::Drag&Drop::ObjInit",
      Tk_PathName(srcPtr->tkwin), Rbc_Itoa(srcPtr->button), (char *)NULL);
    ...
    Tk_Window tkwin;		/* Widget pathname for desired record */
{
    ...
    targetPtr = RbcCalloc(1, sizeof(Target));
    assert(targetPtr);
    ...
    targetPtr->hashPtr = Tcl_CreateHashEntry(&targetTable, (char *)tkwin, 
	     &isNew);
    Tcl_SetHashValue(targetPtr->hashPtr, targetPtr);
    ...
    register Source *srcPtr;	/* drag&drop source record */
{
    ...
    Tcl_DStringInit(&dString);
    Rbc_DStringAppendElements(&dString, "send", targetInfo[INTERP_NAME],
	dragDropCmd, "location", (char *)NULL);
    Tcl_DStringAppendElement(&dString, Rbc_Itoa(srcPtr->token.lastX));
    Tcl_DStringAppendElement(&dString, Rbc_Itoa(srcPtr->token.lastY));
 ...
 *	window.  Queries the "RbcDrag&DropTarget" property info where
...
InitRoot(srcPtr)
    Source *srcPtr;
{
    srcPtr->rootPtr = RbcCalloc(1, sizeof(AnyWindow));
    ...
    int x, y;
{
    ...
    nativeTokenWindow = (WINDOW)Rbc_GetRealWindowId(srcPtr->token.tkwin);
    /*
				 ...
				 * (in virtual coords) */
{
    ...
    /* See if this window has a "RbcDrag&DropTarget" property. */
    ...
    AnyWindow *windowPtr;		/* window rep to be initialized */
{
	...
	for (linkPtr = Rbc_ChainFirstLink(chainPtr); linkPtr != NULL;
	    linkPtr = Rbc_ChainNextLink(linkPtr)) {
	    childPtr = RbcCalloc(1, sizeof(AnyWindow));
	    assert(childPtr);
	    childPtr->initialized = FALSE;
	    childPtr->nativeWindow = (WINDOW)Rbc_ChainGetValue(linkPtr);
    ...
    char **argv;
{
	...
	srcPtr->pkgCmdResult = RbcStrdup(result);
	if (status != TCL_OK) {
	    /*
    ...
    char **argv;
{
    ...
    Rbc_SetBooleanResult(interp, (nActive > 0));
    return TCL_OK;
}
    ...
    char **argv;
{
    ...
    Tcl_AppendElement(interp, Rbc_Itoa(locX));
    Tcl_AppendElement(interp, Rbc_Itoa(locY));
    return TCL_OK;
}
...
{
    if (Tcl_CreateCommand(interp, "::rbc::drag&drop", DragDropCmd, 
	...
	errorCmd = RbcStrdup(DEF_ERROR_PROC);
	nActive = 0;
	locX = locY = 0;
	initialized = TRUE;
#ifndef WIN32
