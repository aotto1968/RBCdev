    ...
    char **argv;
{
    ...
    kernel = ckalloc(sizeof(double) * nValues);
    sum = 0.0;
    for (i = 0; i < nValues; i++) {
    ...
    char **argv;
{
	...
	Rbc_ResamplePhoto(interp, srcPhoto, 0, 0, src.width, src.height, destPhoto,
...
Rbc_WinopInit(interp)
    Tcl_Interp *interp;
{
    if (Tcl_CreateCommand(interp, "::rbc::winop", WinopCmd, 
