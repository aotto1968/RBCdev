...
static Rbc_TreeApplyProc DeleteApplyProc;
static Rbc_TreeApplyProc CreateApplyProc;

extern Rbc_CustomOption rbcTreeViewDataOption;

static Rbc_OptionParseProc ObjToTree;
static Rbc_OptionPrintProc TreeToObj;
static Rbc_OptionFreeProc FreeTree;
Rbc_CustomOption rbcTreeViewTreeOption =
{
    ObjToTree, TreeToObj, FreeTree, NULL,
};

static Rbc_OptionParseProc ObjToIcons;
static Rbc_OptionPrintProc IconsToObj;
static Rbc_OptionFreeProc FreeIcons;
Rbc_CustomOption rbcTreeViewIconsOption =
{
...
static Rbc_OptionParseProc ObjToButton;
static Rbc_OptionPrintProc ButtonToObj;
static Rbc_CustomOption buttonOption = {
    ObjToButton, ButtonToObj, NULL, NULL,
};

static Rbc_OptionParseProc ObjToUid;
static Rbc_OptionPrintProc UidToObj;
static Rbc_OptionFreeProc FreeUid;
Rbc_CustomOption rbcTreeViewUidOption = {
    ObjToUid, UidToObj, FreeUid, NULL,
};

static Rbc_OptionParseProc ObjToScrollmode;
static Rbc_OptionPrintProc ScrollmodeToObj;
static Rbc_CustomOption scrollmodeOption = {
    ObjToScrollmode, ScrollmodeToObj, NULL, NULL,
};

static Rbc_OptionParseProc ObjToSelectmode;
static Rbc_OptionPrintProc SelectmodeToObj;
static Rbc_CustomOption selectmodeOption = {
    ObjToSelectmode, SelectmodeToObj, NULL, NULL,
};

static Rbc_OptionParseProc ObjToSeparator;
static Rbc_OptionPrintProc SeparatorToObj;
static Rbc_OptionFreeProc FreeSeparator;
static Rbc_CustomOption separatorOption = {
    ObjToSeparator, SeparatorToObj, FreeSeparator, NULL,
};

static Rbc_OptionParseProc ObjToLabel;
static Rbc_OptionPrintProc LabelToObj;
static Rbc_OptionFreeProc FreeLabel;
static Rbc_CustomOption labelOption =
{
...
EXTERN int Rbc_TreeCmdGetToken _ANSI_ARGS_((Tcl_Interp *interp, 
	CONST char *treeName, Rbc_Tree *treePtr));

extern Rbc_TreeApplyProc Rbc_TreeViewSortApplyProc;
static Rbc_BindPickProc PickItem;
static Rbc_BindTagProc GetTags;
 ...
 */
void
Rbc_TreeViewEventuallyRedraw(TreeView *tvPtr)
{
...
void
Rbc_TreeViewTraceColumn(TreeView *tvPtr, TreeViewColumn *columnPtr)
{
    Rbc_TreeCreateTrace(tvPtr->tree, NULL /* Node */, columnPtr->key, NULL,
...
ObjToTree(
    ...
    int offset)
{
    Rbc_Tree *treePtr = (Rbc_Tree *)(widgRec + offset);
    Rbc_Tree tree;
    char *string;
    ...
    if ((string[0] != '\0') && 
	(Rbc_TreeGetToken(interp, string, &tree) != TCL_OK)) {
	return TCL_ERROR;
    }
    *treePtr = tree;
    return TCL_OK;
...
TreeToObj(
    ...
    int offset)
{
    Rbc_Tree tree = *(Rbc_Tree *)(widgRec + offset);

    if (tree == NULL) {
	return rbcEmptyStringObjPtr;
    } else {
	return Tcl_NewStringObj(Rbc_TreeName(tree), -1);
    }
}
    ...
    int offset)
{
	 ...
	 */
	root = Rbc_TreeRootNode(*treePtr);
	Rbc_TreeApply(root, DeleteApplyProc, tvPtr);
	Rbc_TreeViewClearSelection(tvPtr);
	Rbc_TreeReleaseToken(*treePtr);
    }
}
    ...
    int offset)
{
	...
	*labelPtr = Rbc_TreeViewGetUid(tvPtr, string);
    }
    return TCL_OK;
}
    ...
    int offset)
{
	...
	string = Rbc_TreeNodeLabel(entryPtr->node);
    } else {
	string = labelUid;
    }
    ...
    int offset)
{
	...
	Rbc_TreeViewFreeUid(tvPtr, *labelPtr);
    }
}
 ...
 */
UID
Rbc_TreeViewGetUid(TreeView *tvPtr, CONST char *string)
{
    Tcl_HashEntry *hPtr;
    int isNew;
    int refCount;

    hPtr = Tcl_CreateHashEntry(&tvPtr->uidTable, string, &isNew);
    if (isNew) {
	refCount = 1;
    } else {
    ...
    return Tcl_GetHashKey(&tvPtr->uidTable, hPtr);
}
 ...
 */
void
Rbc_TreeViewFreeUid(TreeView *tvPtr, UID uid)
{
    Tcl_HashEntry *hPtr;
    int refCount;

    hPtr = Tcl_FindHashEntry(&tvPtr->uidTable, uid);
    assert(hPtr != NULL);
    refCount = (int)Tcl_GetHashValue(hPtr);
    refCount--;
    if (refCount > 0) {
    ...
    int offset)
{
    ...
    string = Tcl_GetString(objPtr);
    if (*string != '\0') {
	newId = Rbc_TreeViewGetUid(tvPtr, string);
    }
    *uidPtr = newId;
    return TCL_OK;
}
    ...
    int offset)
{
	...
	Rbc_TreeViewFreeUid(tvPtr, *uidPtr);
    }
}
...
TreeViewIcon
Rbc_TreeViewGetIcon(TreeView *tvPtr, CONST char *iconName)
{
    Tcl_HashEntry *hPtr;
    int isNew;
    struct TreeViewIconStruct *iconPtr;

    hPtr = Tcl_CreateHashEntry(&tvPtr->iconTable, iconName, &isNew);
    if (isNew) {
	Tk_Image tkImage;
	int width, height;
	...
	iconPtr = ckalloc(sizeof(struct TreeViewIconStruct));
	iconPtr->tkImage = tkImage;
	iconPtr->hashPtr = hPtr;
	iconPtr->refCount = 1;
	iconPtr->width = width;
	iconPtr->height = height;
	Tcl_SetHashValue(hPtr, iconPtr);
    } else {
	iconPtr = Tcl_GetHashValue(hPtr);
	iconPtr->refCount++;
    }
    return iconPtr;
}
...
void
Rbc_TreeViewFreeIcon(
    TreeView *tvPtr,
    struct TreeViewIconStruct *iconPtr)
{
    iconPtr->refCount--;
    if (iconPtr->refCount == 0) {
	Tcl_DeleteHashEntry(iconPtr->hashPtr);
...
DumpIconTable(TreeView *tvPtr)
{
    ...
    for (hPtr = Tcl_FirstHashEntry(&tvPtr->iconTable, &cursor);
	 hPtr != NULL; hPtr = Tcl_NextHashEntry(&cursor)) {
	iconPtr = Tcl_GetHashValue(hPtr);
	Tk_FreeImage(iconPtr->tkImage);
	ckfree(iconPtr);
    }
    Tcl_DeleteHashTable(&tvPtr->iconTable);
}
    ...
    int offset)
{
	...
	icons = ckalloc(sizeof(TreeViewIcon *) * (objc + 1));
	assert(icons);
	for (i = 0; i < objc; i++) {
	    icons[i] = Rbc_TreeViewGetIcon(tvPtr, Tcl_GetString(objv[i]));
	    if (icons[i] == NULL) {
		result = TCL_ERROR;
		break;
	    }
    ...
    int offset)
{
	...
	for (iconPtr = icons; *iconPtr != NULL; iconPtr++) {
	    objPtr = Tcl_NewStringObj(Rbc_NameOfImage((*iconPtr)->tkImage), -1);
    ...
    int offset)
{
	...
	for (iconPtr = icons; *iconPtr != NULL; iconPtr++) {
	    Rbc_TreeViewFreeIcon(tvPtr, *iconPtr);
	}
	ckfree(icons);
    }
}
...
TreeViewEntry *
Rbc_NodeToEntry(TreeView *tvPtr, Rbc_TreeNode node)
{
    Tcl_HashEntry *hPtr;
    ...
    unsigned int flags)
{
	...
	Rbc_TreeNode node, next;

	for (node = Rbc_TreeFirstChild(entryPtr->node); node != NULL; 
	     node = next) {
	    next = Rbc_TreeNextSibling(node);
	    /* 
	     * Get the next child before calling Rbc_TreeViewApply
	     * recursively.  This is because the apply callback may
	     * delete the node and its link.
	     */
	    childPtr = Rbc_NodeToEntry(tvPtr, node);
	    if (Rbc_TreeViewApply(tvPtr, childPtr, proc, flags) != TCL_OK) {
		return TCL_ERROR;
	    }
	}
    }
...
int
Rbc_TreeViewEntryIsHidden(TreeViewEntry *entryPtr)
{
    TreeView *tvPtr = entryPtr->tvPtr; 

    if ((tvPtr->flags & TV_HIDE_LEAVES) && (Rbc_TreeIsLeaf(entryPtr->node))) {
	return TRUE;
    }
...
#ifdef notdef
int
Rbc_TreeViewEntryIsMapped(TreeViewEntry *entryPtr)
{
    TreeView *tvPtr = entryPtr->tvPtr; 

    /* Don't check if the entry itself is open, only that its
     * ancestors are. */
    if (Rbc_TreeViewEntryIsHidden(entryPtr)) {
	return FALSE;
    }
...
TreeViewEntry *
Rbc_TreeViewFirstChild(TreeViewEntry *entryPtr, unsigned int mask)
{
    Rbc_TreeNode node;
    TreeView *tvPtr = entryPtr->tvPtr; 
...
TreeViewEntry *
Rbc_TreeViewLastChild(TreeViewEntry *entryPtr, unsigned int mask)
{
    Rbc_TreeNode node;
    TreeView *tvPtr = entryPtr->tvPtr; 
...
TreeViewEntry *
Rbc_TreeViewNextSibling(TreeViewEntry *entryPtr, unsigned int mask)
{
    Rbc_TreeNode node;
    TreeView *tvPtr = entryPtr->tvPtr; 
...
TreeViewEntry *
Rbc_TreeViewPrevSibling(TreeViewEntry *entryPtr, unsigned int mask)
{
    Rbc_TreeNode node;
    TreeView *tvPtr = entryPtr->tvPtr; 
 ...
 */
TreeViewEntry *
Rbc_TreeViewPrevEntry(TreeViewEntry *entryPtr, unsigned int mask)
{
    TreeView *tvPtr = entryPtr->tvPtr; 
    TreeViewEntry *prevPtr;

    if (entryPtr->node == Rbc_TreeRootNode(tvPtr->tree)) {
	return NULL;		/* The root is the first node. */
    }
    prevPtr = Rbc_TreeViewPrevSibling(entryPtr, mask);
    if (prevPtr == NULL) {
	/* There are no siblings previous to this one, so pick the parent. */
	prevPtr = Rbc_TreeViewParentEntry(entryPtr);
    } else {
	/*
	...
	while ((entryPtr->flags & mask) == 0) {
	    entryPtr = Rbc_TreeViewLastChild(entryPtr, mask);
	    if (entryPtr == NULL) {
		break;		/* Found a leaf. */
	    }
	    prevPtr = entryPtr;
 ...
 */
TreeViewEntry *
Rbc_TreeViewNextEntry(TreeViewEntry *entryPtr, unsigned int mask)
{
    TreeView *tvPtr = entryPtr->tvPtr; 
    ...
    ignoreLeaf = ((tvPtr->flags & TV_HIDE_LEAVES) && 
		  (Rbc_TreeIsLeaf(entryPtr->node)));

    if ((!ignoreLeaf) && ((entryPtr->flags & mask) == 0)) {
	nextPtr = Rbc_TreeViewFirstChild(entryPtr, mask); 
	if (nextPtr != NULL) {
    ...
    while (entryPtr != tvPtr->rootPtr) {
	nextPtr = Rbc_TreeViewNextSibling(entryPtr, mask);
	if (nextPtr != NULL) {
	    return nextPtr;
	}
	entryPtr = Rbc_TreeViewParentEntry(entryPtr);
...
void
Rbc_TreeViewConfigureButtons(TreeView *tvPtr)
{
    GC newGC;
    ...
    int flags)
{
	...
	rbcTreeViewEntrySpecs, objc, objv, (char *)entryPtr, flags) != TCL_OK) {
//printC(Tcl_GetStringResult(tvPtr->interp))
	return TCL_ERROR;
    }
    /* 
	...
	font = entryPtr->font;
	if (font == NULL) {
	    font = Rbc_TreeViewGetStyleFont(tvPtr, tvPtr->treeColumn.stylePtr);
	}
    ...
    if (Rbc_ObjConfigModified(rbcTreeViewEntrySpecs, "-font", (char *)NULL)) {
...
void
Rbc_TreeViewDestroyValue(TreeView *tvPtr, TreeViewValue *valuePtr)
{
...
DestroyEntry(DestroyData data)
{
    ...
    rbcTreeViewUidOption.clientData = tvPtr;
    labelOption.clientData = tvPtr;
    Rbc_FreeObjOptions(rbcTreeViewEntrySpecs, (char *)entryPtr, tvPtr->display,
	0);
    if (!Rbc_TreeTagTableIsShared(tvPtr->tree)) {
	/* Don't clear tags unless this client is the only one using
	 * the tag table.*/
	Rbc_TreeClearTags(tvPtr->tree, entryPtr->node);
    }
    if (entryPtr->gc != NULL) {
	Tk_FreeGC(tvPtr->display, entryPtr->gc);
    }
	...
	ckfree(entryPtr->fullName);
    }
    if (entryPtr->textPtr != NULL) {
	ckfree(entryPtr->textPtr);
    }
    Rbc_PoolFreeItem(tvPtr->entryPool, entryPtr);
}

TreeViewEntry *
Rbc_TreeViewParentEntry(TreeViewEntry *entryPtr)
{
    TreeView *tvPtr = entryPtr->tvPtr; 
    Rbc_TreeNode node;
    ...
    node = Rbc_TreeNodeParent(entryPtr->node);
    if (node == NULL) {
	return NULL;
    }
    return Rbc_NodeToEntry(tvPtr, node);
}
...
FreeEntry(TreeView *tvPtr, TreeViewEntry *entryPtr)
{
	...
	tvPtr->focusPtr = Rbc_TreeViewParentEntry(entryPtr);
    ...
    Rbc_DeleteBindings(tvPtr->bindTable, entryPtr);
//MVvar("entryTable→find<%p>",entryPtr->node)
    hPtr = Tcl_FindHashEntry(&tvPtr->entryTable, entryPtr->node);
    if (hPtr != NULL) {
//MVvar("entryTable→delete<%p>",entryPtr->node)
	Tcl_DeleteHashEntry(hPtr);
    }
    entryPtr->node = NULL;
...
int
Rbc_TreeViewEntryIsSelected(TreeView *tvPtr, TreeViewEntry *entryPtr)
{
    Tcl_HashEntry *hPtr;
...
void
Rbc_TreeViewSelectEntry(TreeView *tvPtr, TreeViewEntry *entryPtr)
{
    int isNew;
    Tcl_HashEntry *hPtr;

    hPtr = Tcl_CreateHashEntry(&tvPtr->selectTable, (char *)entryPtr, &isNew);
    if (isNew) {
	Rbc_ChainLink *linkPtr;

	linkPtr = Rbc_ChainAppend(tvPtr->selChainPtr, entryPtr);
	Tcl_SetHashValue(hPtr, linkPtr);
    }
}

void
Rbc_TreeViewDeselectEntry(TreeView *tvPtr, TreeViewEntry *entryPtr)
{
    Tcl_HashEntry *hPtr;
	...
	linkPtr = Tcl_GetHashValue(hPtr);
	Rbc_ChainDeleteLink(tvPtr->selChainPtr, linkPtr);
	Tcl_DeleteHashEntry(hPtr);
    }
}

char *
Rbc_TreeViewGetFullName(
    TreeView *tvPtr,
    TreeViewEntry *entryPtr,
    int checkEntryLabel,
    Tcl_DString *resultPtr)
{
    ...
    level = Rbc_TreeNodeDepth(tvPtr->tree, entryPtr->node);
    if (tvPtr->rootPtr->labelUid == NULL) {
	level--;
    }
    if (level > 64) {
	names = ckalloc((level + 2) * sizeof(char *));
	assert(names);
    } else {
	names = staticSpace;
    }
	...
	if (checkEntryLabel) {
	    ...
	    names[i] = Rbc_TreeNodeLabel(entryPtr->node);
	}
	node = Rbc_TreeNodeParent(entryPtr->node);
	if (node != NULL) {
	    entryPtr = Rbc_NodeToEntry(tvPtr, node);
...
int
Rbc_TreeViewCloseEntry(TreeView *tvPtr, TreeViewEntry *entryPtr)
{
    char *cmd;
	...
	Rbc_TreeViewPercentSubst(tvPtr, entryPtr, cmd, &dString);
...
int
Rbc_TreeViewOpenEntry(TreeView *tvPtr, TreeViewEntry *entryPtr)
{
    char *cmd;
...
Rbc_TreeViewCreateEntry(
    TreeView *tvPtr,
    Rbc_TreeNode node,		/* Node that has just been created. */
    int objc,
    Tcl_Obj *CONST *objv,
    int flags)
{
...
//printC("isNew")
	...
	entryPtr = Tcl_GetHashValue(hPtr);
    }
    ...
    int order)			/* Not used. */
{
     ...
     */
    return Rbc_TreeUnsetValueByKey(tvPtr->interp, tvPtr->tree, node, 
...
TreeEventProc(ClientData clientData, Rbc_TreeNotifyEvent *eventPtr)
{
	 ...
	 */
	if (node != NULL) {
	    FreeEntry(tvPtr, Rbc_NodeToEntry(tvPtr, node));
	}
	break;
...
TreeViewValue *
Rbc_TreeViewFindValue(TreeViewEntry *entryPtr, TreeViewColumn *columnPtr)
{
...
void
Rbc_TreeViewAddValue(TreeViewEntry *entryPtr, TreeViewColumn *columnPtr)
{
    if (Rbc_TreeViewFindValue(entryPtr, columnPtr) == NULL) {
	Tcl_Obj *objPtr;

	if (Rbc_TreeViewGetData(entryPtr, columnPtr->key, &objPtr) == TCL_OK) {
	    TreeViewValue *valuePtr;

	    /* Add a new value only if a data entry exists. */
	    valuePtr = Rbc_PoolAllocItem(entryPtr->tvPtr->valuePool, 
...
TreeTraceProc(
    ...
    Rbc_TreeKey key,		/* Key of value that's been updated. */
    unsigned int flags)
{
    Tcl_HashEntry *hPtr;
    TreeView *tvPtr = clientData; 
...
//MVvar("entryTable→find<%p>",node)
	...
	hPtr = Tcl_FindHashEntry(&tvPtr->columnTable, key);
	if (hPtr == NULL) {
	    return TCL_OK;	/* Data value isn't used by widget. */
	}
	columnPtr = Tcl_GetHashValue(hPtr);
    ...
    TreeViewStyle *stylePtr)
{
	...
	icon = NULL;
	newStylePtr = NULL;
	if (Rbc_TreeViewGetData(entryPtr, valuePtr->columnPtr->key, 
		&valueObjPtr) != TCL_OK) {
	    return;			/* No data ??? */
	}
	...
	if (string[0] == '@') {	/* Name of style or Tk image. */
		...
		if (Rbc_TreeViewGetStyle((Tcl_Interp *)NULL, tvPtr, name, 
					 &newStylePtr) != TCL_OK) {
		    icon = Rbc_TreeViewGetIcon(tvPtr, name);
		    if (icon == NULL) {
			goto handleString;
		    }
		    /* Create a new style by the name of the image. */
		    newStylePtr = Rbc_TreeViewCreateStyle((Tcl_Interp *)NULL, 
			tvPtr, STYLE_TEXTBOX, name);
		    assert(newStylePtr);
		    Rbc_TreeViewUpdateStyleGCs(tvPtr, newStylePtr);
		}
	    ...
	    if (icon != NULL) {
		Rbc_TreeViewSetStyleIcon(tvPtr, newStylePtr, icon);
	    }
...
/*ARGSUSED*/
TreeViewEntry *
Rbc_TreeViewNearestEntry(TreeView *tvPtr, int x, int y, int selectOne)
{
...
ClientData
Rbc_TreeViewEntryTag(TreeView *tvPtr, CONST char *string)
{
    Tcl_HashEntry *hPtr;
    int isNew;			/* Not used. */

    hPtr = Tcl_CreateHashEntry(&tvPtr->entryTagTable, string, &isNew);
    return Tcl_GetHashKey(&tvPtr->entryTagTable, hPtr);
}

ClientData
Rbc_TreeViewButtonTag(TreeView *tvPtr, CONST char *string)
{
    Tcl_HashEntry *hPtr;
    int isNew;			/* Not used. */

    hPtr = Tcl_CreateHashEntry(&tvPtr->buttonTagTable, string, &isNew);
    return Tcl_GetHashKey(&tvPtr->buttonTagTable, hPtr);
}

ClientData
Rbc_TreeViewColumnTag(TreeView *tvPtr, CONST char *string)
{
    Tcl_HashEntry *hPtr;
    int isNew;			/* Not used. */

    hPtr = Tcl_CreateHashEntry(&tvPtr->columnTagTable, string, &isNew);
    return Tcl_GetHashKey(&tvPtr->columnTagTable, hPtr);
}

ClientData
Rbc_TreeViewStyleTag(TreeView *tvPtr, CONST char *string)
{
    Tcl_HashEntry *hPtr;
    int isNew;			/* Not used. */

    hPtr = Tcl_CreateHashEntry(&tvPtr->styleTagTable, string, &isNew);
    return Tcl_GetHashKey(&tvPtr->styleTagTable, hPtr);
}
...
GetTags(
    ...
    Rbc_List ids)		/* (out) List of binding ids to be
				 * applied for this object. */
{
    ...
    tvPtr = Rbc_GetBindingData(table);
    if (context == (ClientData)ITEM_ENTRY_BUTTON) {
	TreeViewEntry *entryPtr = object;

	Rbc_ListAppend(ids, Rbc_TreeViewButtonTag(tvPtr, "Button"), 0);
	if (entryPtr->tagsUid != NULL) {
	    if (Tcl_SplitList((Tcl_Interp *)NULL, entryPtr->tagsUid, &nNames,
			      &names) == TCL_OK) {
		for (p = names; *p != NULL; p++) {
		    Rbc_ListAppend(ids, Rbc_TreeViewButtonTag(tvPtr, *p), 0);
		}
		ckfree(names);
	    }
	} else {
	    Rbc_ListAppend(ids, Rbc_TreeViewButtonTag(tvPtr, "Entry"), 0);
	    Rbc_ListAppend(ids, Rbc_TreeViewButtonTag(tvPtr, "all"), 0);
	}
    } else if (context == (ClientData)ITEM_COLUMN_TITLE) {
	TreeViewColumn *columnPtr = object;

	Rbc_ListAppend(ids, (char *)columnPtr, 0);
	if (columnPtr->tagsUid != NULL) {
	    if (Tcl_SplitList((Tcl_Interp *)NULL, columnPtr->tagsUid, &nNames,
		      &names) == TCL_OK) {
		for (p = names; *p != NULL; p++) {
		    Rbc_ListAppend(ids, Rbc_TreeViewColumnTag(tvPtr, *p), 0);
		}
		ckfree(names);
	    }
	}
    } else if (context == ITEM_COLUMN_RULE) {
	Rbc_ListAppend(ids, Rbc_TreeViewColumnTag(tvPtr, "Rule"), 0);
    } else {
	TreeViewEntry *entryPtr = object;

	Rbc_ListAppend(ids, (char *)entryPtr, 0);
	if (entryPtr->tagsUid != NULL) {
	    if (Tcl_SplitList((Tcl_Interp *)NULL, entryPtr->tagsUid, &nNames,
		      &names) == TCL_OK) {
		for (p = names; *p != NULL; p++) {
		    Rbc_ListAppend(ids, Rbc_TreeViewEntryTag(tvPtr, *p), 0);
		}
		ckfree(names);
	    }
	} else if (context == ITEM_ENTRY){
	    Rbc_ListAppend(ids, Rbc_TreeViewEntryTag(tvPtr, "Entry"), 0);
	    Rbc_ListAppend(ids, Rbc_TreeViewEntryTag(tvPtr, "all"), 0);
	} else {
		...
		Rbc_ListAppend(ids, 
	            Rbc_TreeViewEntryTag(tvPtr, stylePtr->name), 0);
		Rbc_ListAppend(ids, 
		    Rbc_TreeViewEntryTag(tvPtr, valuePtr->columnPtr->key), 0);
		Rbc_ListAppend(ids, 
		    Rbc_TreeViewEntryTag(tvPtr, stylePtr->classPtr->className),
		    0);
#ifndef notdef
		Rbc_ListAppend(ids, Rbc_TreeViewEntryTag(tvPtr, "Entry"), 0);
		Rbc_ListAppend(ids, Rbc_TreeViewEntryTag(tvPtr, "all"), 0);
				 ...
				 * ITEM_STYLE. */
{
	...
	*contextPtr = NULL;
    ...
    columnPtr = Rbc_TreeViewNearestColumn(tvPtr, x, y, contextPtr);
...
GetEntryExtents(TreeView *tvPtr, TreeViewEntry *entryPtr)
{
	...
	if (label[0] == '\0') {
		...
		Tcl_DStringFree(&dString);
		entryPtr->textPtr = Rbc_GetTextLayout(entryPtr->fullName, &ts);
	    } else {
		entryPtr->textPtr = Rbc_GetTextLayout(label, &ts);
	    }
    ...
    CONST char *className)
{
    ...
    tvPtr = RbcCalloc(1, sizeof(TreeView));
    assert(tvPtr);
    tvPtr->tkwin = tkwin;
    ...
    tvPtr->scrollMode = RBC_SCROLL_MODE_HIERBOX;
    ...
    tvPtr->colChainPtr = Rbc_ChainCreate();
    tvPtr->buttonFlags = BUTTON_AUTO;
    tvPtr->selChainPtr = Rbc_ChainCreate();
    Tcl_InitHashTable(&tvPtr->entryTable, TCL_ONE_WORD_KEYS);
    Tcl_InitHashTable(&tvPtr->columnTable, TCL_ONE_WORD_KEYS);
    Tcl_InitHashTable(&tvPtr->iconTable, TCL_STRING_KEYS);
    Tcl_InitHashTable(&tvPtr->selectTable, TCL_ONE_WORD_KEYS);
    Tcl_InitHashTable(&tvPtr->uidTable, TCL_STRING_KEYS);
    Tcl_InitHashTable(&tvPtr->styleTable, TCL_STRING_KEYS);
    tvPtr->bindTable = Rbc_CreateBindingTable(interp, tkwin, tvPtr, PickItem, 
	GetTags);
    Tcl_InitHashTable(&tvPtr->entryTagTable, TCL_STRING_KEYS);
    Tcl_InitHashTable(&tvPtr->columnTagTable, TCL_STRING_KEYS);
    Tcl_InitHashTable(&tvPtr->buttonTagTable, TCL_STRING_KEYS);
    Tcl_InitHashTable(&tvPtr->styleTagTable, TCL_STRING_KEYS);

    tvPtr->entryPool = Rbc_PoolCreate(RBC_FIXED_SIZE_ITEMS);
    tvPtr->valuePool = Rbc_PoolCreate(RBC_FIXED_SIZE_ITEMS);
    Rbc_SetWindowInstanceData(tkwin, tvPtr);
    tvPtr->cmdToken = Tcl_CreateObjCommand(interp, Tk_PathName(tvPtr->tkwin), 
	Rbc_TreeViewWidgetInstCmd, tvPtr, WidgetInstCmdDeleteProc);
     ...
     * the treeview column. 
     */  
    tvPtr->stylePtr = Rbc_TreeViewCreateStyle(interp, tvPtr, STYLE_TEXTBOX,
	"text");
    ...
    Tcl_DStringAppend(&dString, "RBC TreeView ", -1);
    Tcl_DStringAppend(&dString, Tk_PathName(tvPtr->tkwin), -1);
    result = Rbc_TreeViewCreateColumn(tvPtr, &tvPtr->treeColumn, 
				      Tcl_DStringValue(&dString), "");
    Tcl_DStringFree(&dString);
    if (result != TCL_OK) {
	return NULL;
    }
    Rbc_ChainAppend(tvPtr->colChainPtr, &tvPtr->treeColumn);
    return tvPtr;
}
...
DestroyTreeView(DestroyData dataPtr)	/* Pointer to the widget record. */
{
    ...
    Rbc_TreeDeleteEventHandler(tvPtr->tree, TREE_NOTIFY_ALL, TreeEventProc, 
	   tvPtr);
    for (hPtr = Tcl_FirstHashEntry(&tvPtr->entryTable, &cursor); hPtr != NULL;
    ...
    rbcTreeViewIconsOption.clientData = tvPtr;
    Rbc_FreeObjOptions(rbcTreeViewSpecs, (char *)tvPtr, tvPtr->display, 0);
    if (tvPtr->tkwin != NULL) {
	...
	Rbc_FreePrivateGC(tvPtr->display, tvPtr->focusGC);
    }
    if (tvPtr->visibleArr != NULL) {
	ckfree(tvPtr->visibleArr);
    }
    if (tvPtr->flatArr != NULL) {
	ckfree(tvPtr->flatArr);
    }
    if (tvPtr->levelInfo != NULL) {
	ckfree(tvPtr->levelInfo);
    }
    ...
    if (tvPtr->stylePtr != NULL) {
	Rbc_TreeViewFreeStyle(tvPtr, tvPtr->stylePtr);
    }

    Rbc_TreeViewDestroyColumns(tvPtr);
    Rbc_DestroyBindingTable(tvPtr->bindTable);
    Rbc_ChainDestroy(tvPtr->selChainPtr);
    Tcl_DeleteHashTable(&tvPtr->entryTagTable);
    Tcl_DeleteHashTable(&tvPtr->columnTagTable);
    Tcl_DeleteHashTable(&tvPtr->buttonTagTable);
    Tcl_DeleteHashTable(&tvPtr->styleTagTable);
    ...
    Tcl_DeleteHashTable(&tvPtr->styleTable);
    Tcl_DeleteHashTable(&tvPtr->selectTable);
    Tcl_DeleteHashTable(&tvPtr->uidTable);
    Tcl_DeleteHashTable(&tvPtr->entryTable);

    Rbc_PoolDestroy(tvPtr->entryPool);
    Rbc_PoolDestroy(tvPtr->valuePool);
    DumpIconTable(tvPtr);
    ckfree(tvPtr);
}
    ...
    XEvent *eventPtr)		/* Information about event. */
{
	...
	if (eventPtr->xexpose.count == 0) {
	    Rbc_TreeViewEventuallyRedraw(tvPtr);
	    Rbc_PickCurrentItem(tvPtr->bindTable);
	}
	...
	if (tvPtr->flags & TV_SELECT_PENDING) {
	    Tcl_CancelIdleCall(Rbc_TreeViewSelectCmdProc, tvPtr);
	}
				 ...
				 * NULL character. */
{
	...
	for (linkPtr = Rbc_ChainFirstLink(tvPtr->selChainPtr); 
 ...
 */
int
Rbc_TreeViewUpdateWidget(Tcl_Interp *interp, TreeView *tvPtr)	
{
    GC newGC;
    XGCValues gcValues;
    int setupTree;
	...
	Rbc_SetDashes(tvPtr->display, newGC, &tvPtr->focusDashes);
    }
    if (tvPtr->focusGC != NULL) {
	Rbc_FreePrivateGC(tvPtr->display, tvPtr->focusGC);
    }
	...
	if (Rbc_TreeCreate(interp, string, &token) != TCL_OK) {
	    return TCL_ERROR;
	}
	tvPtr->tree = token;
	setupTree = TRUE;
     ...
     */
    if (Rbc_ObjConfigModified(rbcTreeViewSpecs, "-tree", (char *)NULL)) {
	setupTree = TRUE;
    }
     ...
     */
    if (Rbc_ObjConfigModified(rbcTreeViewSpecs, "-font", 
     ...
     */
    if (Rbc_ObjConfigModified(rbcTreeViewSpecs, "-hideleaves", "-flat",
			      (char *)NULL)) {
	TreeViewEntry *entryPtr;
	...
	Rbc_TreeCreateEventHandler(tvPtr->tree, TREE_NOTIFY_ALL, TreeEventProc, 
		   tvPtr);
	TraceColumns(tvPtr);
	root = Rbc_TreeRootNode(tvPtr->tree);

	/* Automatically add view-entry values to the new tree. */
	Rbc_TreeApply(root, CreateApplyProc, tvPtr);
	tvPtr->focusPtr = tvPtr->rootPtr = Rbc_NodeToEntry(tvPtr, root);
	tvPtr->selMarkPtr = tvPtr->selAnchorPtr = NULL;
	Rbc_SetFocusItem(tvPtr->bindTable, tvPtr->rootPtr, ITEM_ENTRY);

	/* Automatically open the root node. */
	if (Rbc_TreeViewOpenEntry(tvPtr, tvPtr->rootPtr) != TCL_OK) {
	    return TCL_ERROR;
	}
	if (!(tvPtr->flags & TV_NEW_TAGS)) {
	    Rbc_Tree tree;

	    if (Rbc_TreeCmdGetToken(interp, Rbc_TreeName(tvPtr->tree), 
			&tree) == TCL_OK) {
		Rbc_TreeShareTagTable(tree, tvPtr->tree);
	    }
	}
    }

    if (Rbc_ObjConfigModified(rbcTreeViewSpecs, "-font", "-color", 
	(char *)NULL)) {
    ...
    int *yPtr)
{
    ...
    *yPtr += entryPtr->height;
	...
	bottomPtr = entryPtr;
	for (childPtr = Rbc_TreeViewFirstChild(entryPtr, ENTRY_HIDDEN); 
	     childPtr != NULL; 
	     childPtr = Rbc_TreeViewNextSibling(childPtr, ENTRY_HIDDEN)){
...
ComputeFlatLayout(TreeView *tvPtr)
{
	...
	if (tvPtr->flatArr == NULL) {
	    ...
	    tvPtr->flatArr = RbcCalloc((count + 1), sizeof(TreeViewEntry *));
	...
	tvPtr->levelInfo = RbcCalloc(tvPtr->depth + 2, sizeof(LevelInfo));
...
ComputeTreeLayout(TreeView *tvPtr)
{
	...
	for (entryPtr = tvPtr->rootPtr; entryPtr != NULL; 
		...
		if (tvPtr->flags & TV_HIDE_LEAVES) {
		    ...
		    if (Rbc_TreeViewFirstChild(entryPtr, ENTRY_HIDDEN) 
			!= NULL) {
			entryPtr->flags |= ENTRY_HAS_BUTTON;
		    }
		} else if (!Rbc_TreeIsLeaf(entryPtr->node)) {
		    entryPtr->flags |= ENTRY_HAS_BUTTON;
		}
	    }
 ...
 */
void
Rbc_TreeViewComputeLayout(TreeView *tvPtr)
{
    Rbc_ChainLink *linkPtr;
    TreeViewColumn *columnPtr;
    TreeViewEntry *entryPtr;
...
ComputeVisibleEntries(TreeView *tvPtr)
{
    ...
    xOffset = Rbc_AdjustViewport(tvPtr->xOffset, tvPtr->worldWidth,
	VPORTWIDTH(tvPtr), tvPtr->xScrollUnits, tvPtr->scrollMode);
    yOffset = Rbc_AdjustViewport(tvPtr->yOffset, 
	...
	tvPtr->visibleArr = RbcCalloc(nSlots, sizeof(TreeViewEntry *));
	...
	while ((entryPtr->worldY + entryPtr->height) <= tvPtr->yOffset) {
	    for (entryPtr = Rbc_TreeViewLastChild(entryPtr, ENTRY_HIDDEN);
		 entryPtr != NULL; 
		 entryPtr = Rbc_TreeViewPrevSibling(entryPtr, ENTRY_HIDDEN)) {
    ...
    tvPtr->xOffset = Rbc_AdjustViewport(tvPtr->xOffset, 
	tvPtr->worldWidth, VPORTWIDTH(tvPtr), tvPtr->xScrollUnits, 
	tvPtr->scrollMode);
    tvPtr->yOffset = Rbc_AdjustViewport(tvPtr->yOffset,
	tvPtr->worldHeight, VPORTHEIGHT(tvPtr), tvPtr->yScrollUnits,
	tvPtr->scrollMode);
 ...
 */
TreeViewIcon
Rbc_TreeViewGetEntryIcon(TreeView *tvPtr, TreeViewEntry *entryPtr)
{
    TreeViewIcon *icons;
    TreeViewIcon icon;
    ...
    int y)			
{
    ...
    isFocused = ((entryPtr == tvPtr->focusPtr) && 
		 (tvPtr->flags & TV_FOCUS));
    selected = Rbc_TreeViewEntryIsSelected(tvPtr, entryPtr);
	...
	font = entryPtr->font;
	if (font == NULL) {
	    font = Rbc_TreeViewGetStyleFont(tvPtr, stylePtr);
	}
	normalColor = entryPtr->color;
	if (normalColor == NULL) {
	    normalColor = Rbc_TreeViewGetStyleFg(tvPtr, stylePtr);
	}
	activeColor = (selected) ? SELECT_FG(tvPtr) : normalColor;
	gc = entryPtr->gc;
	if (gc == NULL) {
	    gc = Rbc_TreeViewGetStyleGC(stylePtr);
	}
	Rbc_SetDrawTextStyle(&ts, font, gc, normalColor, activeColor, 
	...
	Rbc_DrawTextLayout(tvPtr->tkwin, drawable, entryPtr->textPtr, 
		&ts, x, y);
    }
    return entryHeight;
}
    ...
    Drawable drawable)		/* Pixmap or window to draw into. */
{
    ...
    x = SCREENX(tvPtr, entryPtr->worldX);
    y = SCREENY(tvPtr, entryPtr->worldY);
    if (!Rbc_TreeViewDrawIcon(tvPtr, entryPtr, drawable, x, y)) {
    ...
    Drawable drawable)		/* Pixmap or window to draw into. */
{
    ...
    if ((Rbc_TreeNodeParent(entryPtr->node) != NULL) && 
	(tvPtr->lineWidth > 0)) {
	/*
	 ...
	 * sign).
	 */
	Rbc_TreeViewDrawButton(tvPtr, entryPtr, drawable, x + entryPtr->buttonX,
		y + entryPtr->buttonY);
    }
    x += ICONWIDTH(level);
    ...
    int x)
{
    ...
    cx = x;
    if (columnPtr->position == Rbc_ChainGetLength(tvPtr->colChainPtr)) {
    ...
    Rbc_Fill3DRectangle(tvPtr->tkwin, drawable, border, cx + 1, 
    ...
    int x;
{
    ...
    for (p = tvPtr->visibleArr; *p != NULL; p++) {
	if (Rbc_TreeViewEntryIsSelected(tvPtr, *p)) {
	    int y;

	    y = SCREENY(tvPtr, (*p)->worldY) - 1;
	    Rbc_Fill3DRectangle(tvPtr->tkwin, drawable, selBorder, x, y, 
    ...
    Drawable drawable;
{
	...
	Rbc_Draw3DRectangle(tvPtr->tkwin, drawable, tvPtr->border,
...
DisplayTreeView(ClientData clientData)	/* Information about widget. */
{
	...
	if (tvPtr->flags & TV_XSCROLL) {
		...
		Rbc_UpdateScrollbar(tvPtr->interp, tvPtr->xScrollCmdPrefix,
	...
	if (tvPtr->flags & TV_YSCROLL) {
		...
		Rbc_UpdateScrollbar(tvPtr->interp, tvPtr->yScrollCmdPrefix,
	...
	Rbc_TreeViewDrawRule(tvPtr, tvPtr->resizeColumnPtr, drawable);
    }
    {
	register TreeViewEntry **p;
	...
	for (linkPtr = Rbc_ChainFirstLink(tvPtr->colChainPtr); 
	    ...
	    Rbc_Fill3DRectangle(tvPtr->tkwin, drawable, border, x, 0,
		...
		for (p = tvPtr->visibleArr; *p != NULL; p++) {
		    ...
		    if (Rbc_TreeViewEntryIsSelected(tvPtr, entryPtr)) {
			Rbc_Fill3DRectangle(tvPtr->tkwin, drawable, selBorder, 
		    ...
		    valuePtr = Rbc_TreeViewFindValue(entryPtr, columnPtr);
		    if (valuePtr != NULL) {
			Rbc_TreeViewDrawValue(tvPtr, entryPtr, valuePtr, 
		...
		Rbc_Draw3DRectangle(tvPtr->tkwin, drawable, border, x, 0, 
 ...
 */
void
Rbc_TreeViewSelectCmdProc(ClientData clientData) 
{
    TreeView *tvPtr = clientData;
    ...
    Tcl_Obj *CONST *objv;	/* Argument strings. */
{
    ...
    if (Rbc_ConfigureComponentFromObj(interp, tvPtr->tkwin, "button", "Button",
	 rbcTreeViewButtonSpecs, 0, (Tcl_Obj **)NULL, (char *)tvPtr, 0) 
	!= TCL_OK) {
	goto error;
    }
