...
#define BITMAP_THREAD_KEY	"RBC Bitmap Data"
 ...
 */
typedef struct {
    Tcl_HashTable bitmapTable;	/* Hash table of bitmap data keyed by 
...
typedef struct {
    ...
    Rbc_Pad padX, padY;		/* Padding around the text */
} BitmapInfo;
...
#define rbc_width 40
#define rbc_height 40
static unsigned char rbc_bits[] =
{
...
#define bigrbc_width 64
#define bigrbc_height 64
static unsigned char bigrbc_bits[] =
{
    ...
    unsigned char **bitsPtr)	/* Pointer to converted array of data */
{
    ...
    *bitsPtr = NULL;
    srcBits = Rbc_GetBitmapData(Tk_Display(tkwin), bitmap, width, height,
	&bytesPerRow);
    if (srcBits == NULL) {
    ...
    bits = ckalloc(sizeof(unsigned char) * arraySize);
    assert(bits);
    count = 0;
    ...
    unsigned char **bitsPtr;	/* data array (output) */
{
    ...
    bits = RbcCalloc(sizeof(unsigned char), arraySize);
    if (bits == NULL) {
    ...
    unsigned char **bitsPtr;
{
	...
	*nextline = '\0';
	...
	Tcl_AppendResult(interp, Rbc_Itoa(width), " x ", (char *)NULL);
	Tcl_AppendResult(interp, Rbc_Itoa(height), "\"", (char *)NULL);
	return -1;
    }
    *widthPtr = width;
    *heightPtr = height;
    ...
    BitmapData *destPtr)	/* Destination bitmap. */
{
    ...
    Rbc_GetBoundingBox(srcPtr->width, srcPtr->height, theta, &rotWidth, 
    ...
    bits = RbcCalloc(arraySize, sizeof(unsigned char));
    if (bits == NULL) {
    ...
    char **argv;		/* Argument list */
{
    ...
    textPtr = Rbc_GetTextLayout(argv[3], &ts);
    bitmap = Rbc_CreateTextBitmap(tkwin, textPtr, &ts, &width, &height);
    ckfree(textPtr);
    if (bitmap == None) {
	...
	result = ScaleRotateData(interp, &srcData, theta, info.scale, 
		 &destData);
	ckfree(bits);		/* Free the un-transformed data array. */
	if (result != TCL_OK) {
	    return TCL_ERROR;
	}
	bits = destData.bits;
    ...
    hPtr = Tcl_CreateHashEntry(&(dataPtr->bitmapTable), argv[2], &isNew);
    Tcl_SetHashValue(hPtr, bits);
    return result;
}
    ...
    char **argv;		/* Argument list */
{
	...
	result = ScaleRotateData(interp, &srcData, theta, info.scale, 
		 &destData);
	ckfree(bits);		/* Free the array of un-transformed data. */
	if (result != TCL_OK) {
	    return TCL_ERROR;
	}
	bits = destData.bits;
    ...
    char **argv;		/* Argument list */
{
    ...
    Rbc_SetBooleanResult(interp, (bitmap != None));
    return TCL_OK;
}
    ...
    char **argv;		/* Argument list */
{
    ...
    Tcl_SetResult(interp, Rbc_Itoa(height), TCL_VOLATILE);
    Tk_FreeBitmap(dataPtr->display, bitmap);
    return TCL_OK;
}
    ...
    char **argv;		/* Argument list */
{
    ...
    Tk_SizeOfBitmap(dataPtr->display, bitmap, &width, &height);
    Tcl_SetResult(interp, Rbc_Itoa(width), TCL_VOLATILE);
    Tk_FreeBitmap(dataPtr->display, bitmap);
    return TCL_OK;
}
    ...
    char **argv;		/* Argument list */
{
    ...
    Tk_SizeOfBitmap(dataPtr->display, bitmap, &width, &height);
    Tcl_DStringInit(&dString);
    Tcl_DStringAppendElement(&dString, Rbc_Itoa(width));
    Tcl_DStringAppendElement(&dString, Rbc_Itoa(height));
 ...
 *--------------------------------------------------------------
 */
static Rbc_OpSpec bitmapOps[] =
{
    {"compose", 1, (Rbc_Op)ComposeOp, 4, 0, "bitmapName text ?flags?",},
    {"data", 2, (Rbc_Op)DataOp, 3, 3, "bitmapName",},
    {"define", 2, (Rbc_Op)DefineOp, 4, 0, "bitmapName data ?flags?",},
    {"exists", 1, (Rbc_Op)ExistsOp, 3, 3, "bitmapName",},
    {"height", 1, (Rbc_Op)HeightOp, 3, 3, "bitmapName",},
    {"source", 1, (Rbc_Op)SourceOp, 3, 3, "bitmapName",},
    {"width", 1, (Rbc_Op)WidthOp, 3, 3, "bitmapName",},
};
static int nBitmapOps = sizeof(bitmapOps) / sizeof(Rbc_OpSpec);
    ...
    char **argv;
{
    ...
    proc = Rbc_GetOp(interp, nBitmapOps, bitmapOps, RBC_OP_ARG1, argc, argv,0);
    if (proc == NULL) {
	return TCL_ERROR;
    }
    ...
    Tcl_Interp *interp;
{
    ...
    for (hPtr = Tcl_FirstHashEntry(&(dataPtr->bitmapTable), &cursor);
	 hPtr != NULL; hPtr = Tcl_NextHashEntry(&cursor)) {
	bits = (unsigned char *)Tcl_GetHashValue(hPtr);
	ckfree(bits);
    }
    Tcl_DeleteHashTable(&(dataPtr->bitmapTable));
    ...
    Tcl_Interp *interp;
{
    ...
    if (dataPtr == NULL) {
	dataPtr = ckalloc(sizeof(BitmapInterpData));
	assert(dataPtr);
	...
	Tcl_SetAssocData(interp, BITMAP_THREAD_KEY, BitmapInterpDeleteProc, 
		 dataPtr);
	Tcl_InitHashTable(&(dataPtr->bitmapTable), TCL_STRING_KEYS);
    }
    return dataPtr;
}
    ...
    Tcl_Interp *interp;
{
    ...
    /* Define the RBC logo bitmaps */
    Tk_DefineBitmap(interp, Tk_GetUid("bigRBC"), (char *)bigrbc_bits,
	bigrbc_width, bigrbc_height);
    Tk_DefineBitmap(interp, Tk_GetUid("RBC"), (char *)rbc_bits,
	rbc_width, rbc_height);
    Tcl_ResetResult(interp);
    return TCL_OK;
}
