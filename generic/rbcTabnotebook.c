 ...
 * rbcTabnotebook.c --
 *
 *	This module implements a tab notebook widget for the RBC toolkit.
 *
...
extern Tk_CustomOption rbcPositiveCountOption;
...
typedef struct {
    ...
    Rbc_Pad padX, padY;		/* Padding around embedded widget */
     ...
     */
    Rbc_Uid command;		/* Command (malloc-ed) invoked when the tab
				 * is selected */
    Rbc_Uid data;		/* This value isn't used in C code.
				 * It may be used by clients in Tcl bindings
				 * to associate extra data (other than the
				 * label or name) with the tab. */

    Rbc_ChainLink *linkPtr;	/* Pointer to where the tab resides in the
				 * list of tabs. */
    Rbc_Uid perfCommand;		/* Command (malloc-ed) invoked when the tab
				 * is selected */
    GC textGC;
    GC backGC;
...
static Tk_ConfigSpec tabConfigSpecs[] =
{
	...
	DEF_TAB_PAD, Tk_Offset(Tab, padX), 0, &rbcPadOption},
    {TK_CONFIG_CUSTOM, "-pady", "padY", "PadY",
	DEF_TAB_PAD, Tk_Offset(Tab, padY), 0, &rbcPadOption},
	...
	TK_CONFIG_NULL_OK, &rbcShadowOption},
...
struct NotebookStruct {
    ...
    Rbc_Chain *chainPtr;	/* List of tab entries. Used to
    ...
    Rbc_BindTable bindTable;	/* Tab binding information */
...
static Tk_ConfigSpec configSpecs[] =
{
	...
	TK_CONFIG_DONT_SET_DEFAULT, &rbcPositiveCountOption},
    {TK_CONFIG_CUSTOM, "-tile", "tile", "Tile",
...
static Rbc_BindPickProc PickTab;
    ...
    char *tagName;
{
    ...
    hPtr = Tcl_CreateHashEntry(&(nbPtr->tagTable), tagName, &isNew);
    assert(hPtr);
    return Tcl_GetHashKey(&(nbPtr->tagTable), hPtr);
}
    ...
    char *name;
{
    ...
    hPtr = Tcl_CreateHashEntry(&(nbPtr->imageTable), name, &isNew);
    if (isNew) {
	Tk_Image tkImage;
	int width, height;
	...
	imagePtr = ckalloc(sizeof(struct TabImageStruct));
	...
	imagePtr = (struct TabImageStruct *)Tcl_GetHashValue(hPtr);
	imagePtr->refCount++;
    }
    return imagePtr;
}
    ...
    Tcl_FreeProc **freeProcPtr;	/* Memory deallocation scheme to use */
{
    ...
    return Tcl_GetHashKey(&(nbPtr->imageTable), (*imagePtr)->hashPtr);
}
    ...
    Tab *tabPtr;
{
    ...
    count = 0;
    for (linkPtr = Rbc_ChainFirstLink(nbPtr->chainPtr); linkPtr != NULL;
	linkPtr = Rbc_ChainNextLink(linkPtr)) {
	t2Ptr = Rbc_ChainGetValue(linkPtr);
	if (t2Ptr == tabPtr) {
	    return count;
	}
	count++;
    ...
    Tab *tabPtr;
{
    int tier;
    Tab *prevPtr;
    Rbc_ChainLink *linkPtr, *lastPtr;

    nbPtr->focusPtr = nbPtr->selectPtr = tabPtr;
    Rbc_SetFocusItem(nbPtr->bindTable, nbPtr->focusPtr, NULL);

    tier = tabPtr->tier;
    for (linkPtr = Rbc_ChainPrevLink(tabPtr->linkPtr); linkPtr != NULL;
	linkPtr = lastPtr) {
	lastPtr = Rbc_ChainPrevLink(linkPtr);
	prevPtr = Rbc_ChainGetValue(linkPtr);
    ...
    for (linkPtr = Rbc_ChainFirstLink(nbPtr->chainPtr); linkPtr != NULL;
	linkPtr = Rbc_ChainNextLink(linkPtr)) {
	tabPtr = Rbc_ChainGetValue(linkPtr);
    ...
    Tab *tabPtr;
{
    ...
    linkPtr = Rbc_ChainPrevLink(tabPtr->linkPtr);
    if (linkPtr != NULL) {
	Tab *newPtr;

	newPtr = Rbc_ChainGetValue(linkPtr);
    ...
    Tab *tabPtr;
{
    ...
    linkPtr = Rbc_ChainNextLink(tabPtr->linkPtr);
    if (linkPtr != NULL) {
	Tab *newPtr;
    ...
    int allowNull;	/* Allow NULL tabPtr */
{
	...
	linkPtr = Rbc_ChainGetNthLink(nbPtr->chainPtr, position);
	if (linkPtr == NULL) {
	...
	tabPtr = (Tab *)Rbc_GetCurrentItem(nbPtr->bindTable);
	...
	linkPtr = Rbc_ChainLastLink(nbPtr->chainPtr);
	if (linkPtr != NULL) {
	    tabPtr = Rbc_ChainGetValue(linkPtr);
	}
    } else if (c == '@') {
	...
	if (Rbc_GetXY(nbPtr->interp, nbPtr->tkwin, string, &x, &y) 
	    != TCL_OK) {
	    return TCL_ERROR;
	}
	tabPtr = (Tab *)PickTab(nbPtr, x, y, NULL);
	...
	hPtr = Tcl_FindHashEntry(&(nbPtr->tabTable), string);
	if (hPtr != NULL) {
	    tabPtr = (Tab *)Tcl_GetHashValue(hPtr);
	}
    }
    *tabPtrPtr = tabPtr;
    ...
    Tab *tabPtr;
{
    ...
    if (tabPtr->text != NULL) {
	Rbc_FreeUid(tabPtr->text);
    }
    hPtr = Tcl_FindHashEntry(&(nbPtr->tabTable), tabPtr->name);
    assert(hPtr);
    Tcl_DeleteHashEntry(hPtr);
	...
	Rbc_FreeUid(tabPtr->command);
    }
    if (tabPtr->linkPtr != NULL) {
	Rbc_ChainDeleteLink(nbPtr->chainPtr, tabPtr->linkPtr);
    }
    if (tabPtr->tags != NULL) {
	Rbc_FreeUid(tabPtr->tags);
    }
    Rbc_DeleteBindings(nbPtr->bindTable, tabPtr);
    ckfree(tabPtr);
}
    ...
    Notebook *nbPtr;
{
    ...
    tabPtr = RbcCalloc(1, sizeof(Tab));
    assert(tabPtr);
    tabPtr->nbPtr = nbPtr;
    sprintf(string, "tab%d", nbPtr->nextId++);
    tabPtr->name = RbcStrdup(string);
    tabPtr->text = Rbc_GetUid(string);
    tabPtr->fill = FILL_NONE;
    tabPtr->anchor = TK_ANCHOR_CENTER;
    tabPtr->container = NULL;
    tabPtr->state = STATE_NORMAL;
    hPtr = Tcl_CreateHashEntry(&(nbPtr->tabTable), string, &isNew);
    Tcl_SetHashValue(hPtr, tabPtr);
    return tabPtr;
}
    ...
    Tab *tabPtr;
{
	...
	Rbc_GetTextExtents(&ts, tabPtr->text, &labelWidth, &labelHeight);
	Rbc_GetBoundingBox(labelWidth, labelHeight, nbPtr->defTabStyle.rotate,
     ...
     */
    if (tabPtr->tile != NULL) {
	Rbc_SetTileChangedProc(tabPtr->tile, TileChangedProc, nbPtr);
    }
    if (tabPtr->flags & TAB_VISIBLE) {
	EventuallyRedraw(nbPtr);
    }
    ...
    Rbc_List list;
{
	...
	Rbc_ListAppend(list, MakeTag(nbPtr, "Perforation"), 0);
    } else if (context == TAB_LABEL) {
	Rbc_ListAppend(list, MakeTag(nbPtr, tabPtr->name), 0);
	if (tabPtr->tags != NULL) {
	    int nNames;
	    char **names;
	    register char **p;
		...
		for (p = names; *p != NULL; p++) {
		    Rbc_ListAppend(list, MakeTag(nbPtr, *p), 0);
		}
		ckfree(names);
	    }
	}
    ...
    DestroyData dataPtr;	/* Pointer to the widget record. */
{
    ...
    if (nbPtr->tile != NULL) {
	Rbc_FreeTile(nbPtr->tile);
    }
    if (nbPtr->defTabStyle.activeGC != NULL) {
	Rbc_FreePrivateGC(nbPtr->display, nbPtr->defTabStyle.activeGC);
    }
    for (linkPtr = Rbc_ChainFirstLink(nbPtr->chainPtr); linkPtr != NULL;
	linkPtr = Rbc_ChainNextLink(linkPtr)) {
	tabPtr = Rbc_ChainGetValue(linkPtr);
    ...
    Rbc_ChainDestroy(nbPtr->chainPtr);
    Rbc_DestroyBindingTable(nbPtr->bindTable);
    Tcl_DeleteHashTable(&(nbPtr->tabTable));
    Tcl_DeleteHashTable(&(nbPtr->tagTable));
    ...
    Tk_Window tkwin;
{
    ...
    nbPtr = RbcCalloc(1, sizeof(Notebook));
    assert(nbPtr);
    ...
    nbPtr->overlap = 0;
    nbPtr->tearoff = TRUE;
    nbPtr->bindTable = Rbc_CreateBindingTable(interp, tkwin, nbPtr, PickTab, 
	GetTags);
    nbPtr->chainPtr = Rbc_ChainCreate();
    Tcl_InitHashTable(&(nbPtr->tabTable), TCL_STRING_KEYS);
    Tcl_InitHashTable(&(nbPtr->imageTable), TCL_STRING_KEYS);
    Tcl_InitHashTable(&(nbPtr->tagTable), TCL_STRING_KEYS);
#if (TK_MAJOR_VERSION > 4)
    Rbc_SetWindowInstanceData(tkwin, nbPtr);
#endif
    return nbPtr;
}
    ...
    int flags;
{
    ...
    if (Rbc_ConfigModified(configSpecs, "-width", "-height", "-side", "-gap",
	    "-slant", (char *)NULL)) {
	nbPtr->flags |= (TNB_LAYOUT | TNB_SCROLL);
    }
     ...
     */
    if (nbPtr->tile != NULL) {
	Rbc_SetTileChangedProc(nbPtr->tile, TileChangedProc, nbPtr);
    }
    /*
     * GC for active line.
     */
    ...
    newGC = Rbc_GetPrivateGC(nbPtr->tkwin, gcMask, &gcValues);
    if (LineIsDashed(nbPtr->defTabStyle.dashes)) {
	nbPtr->defTabStyle.dashes.offset = 2;
	Rbc_SetDashes(nbPtr->display, newGC, &(nbPtr->defTabStyle.dashes));
    }
    if (nbPtr->defTabStyle.activeGC != NULL) {
	Rbc_FreePrivateGC(nbPtr->display, 
    ...
    if (Rbc_ConfigModified(configSpecs, "-font", "-*foreground", "-rotate",
	    "-*background", "-side", (char *)NULL)) {
	Rbc_ChainLink *linkPtr;
	Tab *tabPtr;

	for (linkPtr = Rbc_ChainFirstLink(nbPtr->chainPtr); 
    ...
    char **argv;
{
	...
	for (hPtr = Tcl_FirstHashEntry(&(nbPtr->tagTable), &cursor);
	    hPtr != NULL; hPtr = Tcl_NextHashEntry(&cursor)) {
	    tagName = Tcl_GetHashKey(&(nbPtr->tagTable), hPtr);
	    Tcl_AppendElement(interp, tagName);
	}
	return TCL_OK;
    }
    return Rbc_ConfigureBindings(interp, nbPtr->bindTable,
    ...
    char **argv;
{
	...
	Tab *tabPtr;
	Rbc_ChainLink *linkPtr, *nextLinkPtr;
	...
	while (linkPtr != NULL) {
	    nextLinkPtr = Rbc_ChainNextLink(linkPtr);
	    tabPtr = Rbc_ChainGetValue(linkPtr);
	    DestroyTab(nbPtr, tabPtr);
	    linkPtr = nextLinkPtr;
	    if (tabPtr == lastPtr) {
    ...
    char **argv;
{
    ...
    if (tabPtr != NULL) {
	Tcl_SetResult(interp, Rbc_Itoa(TabIndex(nbPtr, tabPtr)), 
		TCL_VOLATILE);
    }
    return TCL_OK;
}
...
{
    Tab *tabPtr;
    Rbc_ChainLink *linkPtr, *beforeLinkPtr;
    char c;
	...
	if (position < 0) {
	    beforeLinkPtr = Rbc_ChainFirstLink(nbPtr->chainPtr);
	} else if (position > Rbc_ChainGetLength(nbPtr->chainPtr)) {
	    beforeLinkPtr = NULL;
	} else {
	    beforeLinkPtr = Rbc_ChainGetNthLink(nbPtr->chainPtr, position);
	}
    } else {
	Tab *beforePtr;
    ...
    lastNotebookInstance = nbPtr;
    if (Rbc_ConfigureWidgetComponent(interp, nbPtr->tkwin, tabPtr->name,
	...
	Rbc_ChainAppendLink(nbPtr->chainPtr, linkPtr);
    } else {
	Rbc_ChainLinkBefore(nbPtr->chainPtr, linkPtr, beforeLinkPtr);
    }
    tabPtr->linkPtr = linkPtr;
    Rbc_ChainSetValue(linkPtr, tabPtr);
    Tcl_SetResult(interp, tabPtr->name, TCL_VOLATILE);
    return TCL_OK;
    ...
    Tcl_DString *resultPtr;
{
	...
	if (*p == '%') {
		...
		*p = '%';
		...
		break;
	    case 'i':		/* Tab Index */
		string = Rbc_Itoa(TabIndex(nbPtr, tabPtr));
		break;
	    case 'n':		/* Tab name */
		string = tabPtr->name;
		break;
    ...
    char **argv;
{
    ...
    Rbc_ChainUnlinkLink(nbPtr->chainPtr, tabPtr->linkPtr);
    if (before) {
	Rbc_ChainLinkBefore(nbPtr->chainPtr, tabPtr->linkPtr, 
	    linkPtr->linkPtr);
    } else {
	Rbc_ChainLinkAfter(nbPtr->chainPtr, tabPtr->linkPtr, 
	    linkPtr->linkPtr);
    }
    nbPtr->flags |= (TNB_LAYOUT | TNB_SCROLL);
    EventuallyRedraw(nbPtr);
    ...
    char **argv;
{
	...
	RenumberTiers(nbPtr, tabPtr);
	Rbc_PickCurrentItem(nbPtr->bindTable);
    }
    ...
    char **argv;
{
    ...
    if (Rbc_GetScrollInfo(interp, argc - 2, argv + 2, &(nbPtr->scrollOffset),
	    nbPtr->worldWidth, width, nbPtr->scrollUnits, 
	    RBC_SCROLL_MODE_CANVAS) != TCL_OK) {
	return TCL_ERROR;
    }
    nbPtr->flags |= TNB_SCROLL;
    EventuallyRedraw(nbPtr);
...
#define TEAR_OFF_TAB_SIZE	5
    ...
    Rbc_RelinkWindow(tabPtr->tkwin, tabPtr->container, x, y);
    ...
    DestroyData dataPtr;
{
	...
	if (tabPtr->tkwin != NULL) {
	    ...
	    Rbc_RelinkWindow(tabPtr->tkwin, nbPtr->tkwin, rect.x, rect.y);
	    if (tabPtr == nbPtr->selectPtr) {
 ...
 *----------------------------------------------------------------------
 */
static Rbc_OpSpec tabOps[] =
{
    {"cget", 2, (Rbc_Op)TabCgetOp, 5, 5, "nameOrIndex option",},
    {"configure", 2, (Rbc_Op)TabConfigureOp, 4, 0,
	"nameOrIndex ?option value?...",},
    {"dockall", 1, (Rbc_Op)TabDockallOp, 3, 3, "" }, 
    {"names", 1, (Rbc_Op)TabNamesOp, 3, 0, "?pattern...?",},
    {"tearoff", 1, (Rbc_Op)TabTearoffOp, 4, 5, "index ?parent?",},
};

static int nTabOps = sizeof(tabOps) / sizeof(Rbc_OpSpec);
    ...
    char **argv;
{
    ...
    proc = Rbc_GetOp(interp, nTabOps, tabOps, RBC_OP_ARG2, argc, argv, 0);
    if (proc == NULL) {
	return TCL_ERROR;
    }
 ...
 *----------------------------------------------------------------------
 */
static Rbc_OpSpec perforationOps[] =
{
    {"activate", 1, (Rbc_Op)PerforationActivateOp, 4, 4, "boolean" }, 
    {"invoke", 1, (Rbc_Op)PerforationInvokeOp, 3, 3, "",},
};

static int nPerforationOps = sizeof(perforationOps) / sizeof(Rbc_OpSpec);
    ...
    char **argv;
{
    ...
    proc = Rbc_GetOp(interp, nPerforationOps, perforationOps, RBC_OP_ARG2, 
		     argc, argv, 0);
    if (proc == NULL) {
	return TCL_ERROR;
    }
...
#define SCAN_DRAGTO	2
	...
	offset = Rbc_AdjustViewport(offset, nbPtr->worldWidth,
	    VPORTWIDTH(nbPtr), nbPtr->scrollUnits, RBC_SCROLL_MODE_CANVAS);
    ...
    char **argv;
{
	...
	if (tabPtr->worldX < left) {
		...
		nextPtr = Rbc_ChainGetValue(linkPtr);
...
SizeOp(nbPtr, interp, argc, argv)
    ...
    char **argv;		/* Not used. */
{
    Tcl_SetResult(interp, Rbc_Itoa(Rbc_ChainGetLength(nbPtr->chainPtr)),
	TCL_VOLATILE);
    return TCL_OK;
}

    ...
    int nTabs;
{
	...
	linkPtr = Rbc_ChainFirstLink(nbPtr->chainPtr);
	count = 1;
	while (linkPtr != NULL) {
    ...
    Notebook *nbPtr;
{
	...
	linkPtr = Rbc_ChainFirstLink(nbPtr->chainPtr);
	if (linkPtr != NULL) {
	    nbPtr->selectPtr = Rbc_ChainGetValue(linkPtr);
	}
    }
    ...
    Notebook *nbPtr;
{
    ...
    nbPtr->nVisible = 0;
    if (Rbc_ChainGetLength(nbPtr->chainPtr) == 0) {
	return;
    }
    nVisibleTabs = 0;
    if (nbPtr->flags & TNB_STATIC) {
    ...
    int nPoints;
{
	...
	Tk_Fill3DPolygon(nbPtr->tkwin, drawable, border, pointArr, nPoints,
	    borderWidth, relief);
#endif
	Rbc_TilePolygon(nbPtr->tkwin, drawable, tabPtr->tile, pointArr, 
			nPoints);
#ifdef notdef
    ...
    Drawable drawable;
{
	...
	Rbc_SetDrawTextStyle(&ts, GETATTR(tabPtr, font), tabPtr->textGC,
	...
	Rbc_DrawText(nbPtr->tkwin, drawable, tabPtr->text, &ts, tx, ty);
    }
}
    ...
    Drawable drawable;
{
	...
	Rbc_Fill3DRectangle(nbPtr->tkwin, drawable, perfBorder,
    ...
    Drawable drawable;
{
	...
	Rbc_Draw3DRectangle(nbPtr->tkwin, drawable, nbPtr->border,
    ...
    ClientData clientData;	/* Information about widget. */
{
	...
	width = VPORTWIDTH(nbPtr);
	nbPtr->scrollOffset = Rbc_AdjustViewport(nbPtr->scrollOffset,
	    nbPtr->worldWidth, width, nbPtr->scrollUnits, 
	    RBC_SCROLL_MODE_CANVAS);
	if (nbPtr->scrollCmdPrefix != NULL) {
	    Rbc_UpdateScrollbar(nbPtr->interp, nbPtr->scrollCmdPrefix,
     ...
     */
    if (nbPtr->tile != NULL) {
	Rbc_SetTileOrigin(nbPtr->tkwin, nbPtr->tile, 0, 0);
	Rbc_TileRectangle(nbPtr->tkwin, drawable, nbPtr->tile, 0, 0,
	    Tk_Width(nbPtr->tkwin), height);
    } else {
	Rbc_Fill3DRectangle(nbPtr->tkwin, drawable, nbPtr->border, 0, 0,
	...
	for (i = 0; i < Rbc_ChainGetLength(nbPtr->chainPtr); i++) {
    ...
    ClientData clientData;
{
     ...
     */
    if (nbPtr->tile != NULL) {
	Rbc_SetTileOrigin(tkwin, nbPtr->tile, 0, 0);
	Rbc_TileRectangle(tkwin, drawable, nbPtr->tile, 0, 0, Tk_Width(tkwin), 
		Tk_Height(tkwin));
    } else {
	Rbc_Fill3DRectangle(tkwin, drawable, nbPtr->border, 0, 0, 
	...
	Rbc_Draw3DRectangle(tkwin, drawable, nbPtr->border, 0, 0,
 ...
 * --------------------------------------------------------------
 */
static Rbc_OpSpec notebookOps[] =
{
    {"activate", 1, (Rbc_Op)ActivateOp, 3, 3, "index",},
    {"bind", 1, (Rbc_Op)BindOp, 2, 5, "index ?sequence command?",},
    {"cget", 2, (Rbc_Op)CgetOp, 3, 3, "option",},
    {"configure", 2, (Rbc_Op)ConfigureOp, 2, 0, "?option value?...",},
    {"delete", 1, (Rbc_Op)DeleteOp, 2, 0, "first ?last?",},
    {"focus", 1, (Rbc_Op)FocusOp, 3, 3, "index",},
    {"highlight", 1, (Rbc_Op)ActivateOp, 3, 3, "index",},
    {"id", 2, (Rbc_Op)IdOp, 3, 3, "index",},
    {"index", 3, (Rbc_Op)IndexOp, 3, 5, "string",},
    {"insert", 3, (Rbc_Op)InsertOp, 3, 0, "index ?option value?",},
    {"invoke", 3, (Rbc_Op)InvokeOp, 3, 3, "index",},
    {"move", 1, (Rbc_Op)MoveOp, 5, 5, "name after|before index",},
    {"nearest", 1, (Rbc_Op)NearestOp, 4, 4, "x y",},
    {"perforation", 1, (Rbc_Op)PerforationOp, 2, 0, "args",},
    {"scan", 2, (Rbc_Op)ScanOp, 5, 5, "dragto|mark x y",},
    {"see", 3, (Rbc_Op)SeeOp, 3, 3, "index",},
    {"select", 3, (Rbc_Op)SelectOp, 3, 3, "index",},
    {"size", 2, (Rbc_Op)SizeOp, 2, 2, "",},
    {"tab", 1, (Rbc_Op)TabOp, 2, 0, "oper args",},
    {"view", 1, (Rbc_Op)ViewOp, 2, 5,
	"?moveto fract? ?scroll number what?",},
};

static int nNotebookOps = sizeof(notebookOps) / sizeof(Rbc_OpSpec);
    ...
    char **argv;		/* Vector of argument strings. */
{
    ...
    proc = Rbc_GetOp(interp, nNotebookOps, notebookOps, RBC_OP_ARG1, argc, 
	argv, 0);
    if (proc == NULL) {
	return TCL_ERROR;
    }
    ...
    char **argv;		/* Argument strings. */
{
     ...
     * that the user could set the variable "rbc_library" within the
     * script.
     */
    if (!Tcl_GetCommandInfo(interp, "::rbc::Tabnotebook::ObjInit", &cmdInfo)) {
	static char initCmd[] = 
	    "source [file join $::rbc::library tabnotebook.tcl]";
    ...
    if (Tcl_VarEval(interp, "::rbc::Tabnotebook::ObjInit ", argv[1], (char *)NULL)
	!= TCL_OK) {
        goto error;
    }
...
Rbc_TabnotebookInit(interp)
    Tcl_Interp *interp;
{
  if (Tcl_CreateCommand(interp, "::rbc::tabnotebook", NotebookCmd, 
