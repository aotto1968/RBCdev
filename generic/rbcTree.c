...
static Rbc_TreeApplyProc SizeApplyProc;
static Tcl_IdleProc NotifyIdleProc;

#define TREE_THREAD_KEY		"RBC Tree Data"
#define TREE_MAGIC		((unsigned int) 0x46170277)
#define TREE_DESTROYED		(1<<0)

typedef struct Rbc_TreeNodeStruct Node;
typedef struct Rbc_TreeClientStruct TreeClient;
typedef struct Rbc_TreeObjectStruct TreeObject;
typedef struct Rbc_TreeValueStruct TreeValue;
 ...
 *	field (Rbc_TreeKey) and the value (Tcl_Obj) containing the
 *	actual data representations.
 * 
 */
struct Rbc_TreeValueStruct {
    Rbc_TreeKey key;		/* String identifying the data field */
    Tcl_Obj *objPtr;		/* Data representation. */
    Rbc_Tree owner;		/* Non-NULL if privately owned. */
    Rbc_TreeValue next;		/* Next value in the chain. */
};
...
static void TreeDestroyValues _ANSI_ARGS_((Rbc_TreeNode node));

static TreeValue *TreeFindValue _ANSI_ARGS_((Rbc_TreeNode node,
	Rbc_TreeKey key));
static TreeValue *TreeCreateValue _ANSI_ARGS_((Rbc_TreeNode node,
	Rbc_TreeKey key, int *newPtr));

static int TreeDeleteValue _ANSI_ARGS_((Rbc_TreeNode node, 
	Rbc_TreeValue value));

static TreeValue *TreeFirstValue _ANSI_ARGS_((Rbc_TreeNode, 
	Rbc_TreeKeySearch *searchPtr));

static TreeValue *TreeNextValue _ANSI_ARGS_((Rbc_TreeKeySearch *srchPtr));
...
#if (SIZEOF_VOID_P == 8)
static Tcl_Hash HashOneWord _ANSI_ARGS_((uint64_t mask, unsigned int downshift,
	CONST void *key));

#endif /* SIZEOF_VOID_P == 8 */

/*
 * The hash table below is used to keep track of all the Rbc_TreeKeys
 * created so far.
 */
static Tcl_HashTable keyTable;
 ...
 *	than the Tcl thread routines so RBC can work with pre-8.0 
...
GetTreeInterpData(Tcl_Interp *interp)
{
    ...
    if (dataPtr == NULL) {
	dataPtr = ckalloc(sizeof(TreeInterpData));
	assert(dataPtr);
...
NewNode(TreeObject *treeObjPtr, CONST char *name, int inode)
{
    ...
    nodePtr = Rbc_PoolAllocItem(treeObjPtr->nodePool, sizeof(Node));
    nodePtr->inode = inode;
    ...
    nodePtr->label = NULL;
    if (name != NULL) {
	nodePtr->label = Rbc_TreeGetKey(name);
    }
    treeObjPtr->nNodes++;
    return nodePtr;
}
 ...
 */
static void
ReleaseTagTable(Rbc_TreeTagTable *tablePtr)
{
    tablePtr->refCount--;
	...
	for(hPtr = Tcl_FirstHashEntry(&tablePtr->tagTable, &cursor); 
	    hPtr != NULL; hPtr = Tcl_NextHashEntry(&cursor)) {
	    Rbc_TreeTagEntry *tPtr;

	    tPtr = Tcl_GetHashValue(hPtr);
	    Tcl_DeleteHashTable(&tPtr->nodeTable);
	    ckfree(tPtr);
	}
...
FreeNode(TreeObject *treeObjPtr, Node *nodePtr)
{
    ...
    UnlinkNode(nodePtr);
    treeObjPtr->nNodes--;
    hPtr = Tcl_FindHashEntry(&treeObjPtr->nodeTable, (char *)nodePtr->inode);
    assert(hPtr);
    Tcl_DeleteHashEntry(hPtr);
    Rbc_PoolFreeItem(treeObjPtr->nodePool, (char *)nodePtr);
}
...
NewTreeObject(TreeInterpData *dataPtr, Tcl_Interp *interp, CONST char *treeName)
{
    ...
    treeObjPtr = RbcCalloc(1, sizeof(TreeObject));
    if (treeObjPtr == NULL) {
	Tcl_AppendResult(interp, "can't allocate tree", (char *)NULL);
	return NULL;
    }
    treeObjPtr->name = RbcStrdup(treeName);
    treeObjPtr->interp = interp;
    treeObjPtr->valuePool = Rbc_PoolCreate(RBC_FIXED_SIZE_ITEMS);
    treeObjPtr->nodePool = Rbc_PoolCreate(RBC_FIXED_SIZE_ITEMS);
    treeObjPtr->clients = Rbc_ChainCreate();
    treeObjPtr->depth = 1;
    treeObjPtr->notifyFlags = 0;
    Tcl_InitHashTable(&treeObjPtr->nodeTable, TCL_ONE_WORD_KEYS);

    hPtr = Tcl_CreateHashEntry(&treeObjPtr->nodeTable, (char *)0, &isNew);
    treeObjPtr->root = NewNode(treeObjPtr, treeName, 0);
    Tcl_SetHashValue(hPtr, treeObjPtr->root);

    treeObjPtr->tablePtr = &dataPtr->treeTable;
    treeObjPtr->hashPtr = Tcl_CreateHashEntry(treeObjPtr->tablePtr, treeName, 
	&isNew);
    Tcl_SetHashValue(treeObjPtr->hashPtr, treeObjPtr);
    ...
    CONST char *treeName)
{
    ...
    name = Rbc_GetQualifiedName(nsPtr, treeName, &dString);
...
GetTreeObject(Tcl_Interp *interp, CONST char *name, int flags)
{
    ...
    treeObjPtr = NULL;
    if (Rbc_ParseQualifiedName(interp, name, &nsPtr, &treeName) != TCL_OK) {
...
DestroyTreeObject(TreeObject *treeObjPtr)
{
    ...
    for (linkPtr = Rbc_ChainFirstLink(treeObjPtr->clients); linkPtr != NULL;
	linkPtr = Rbc_ChainNextLink(linkPtr)) {
	clientPtr = Rbc_ChainGetValue(linkPtr);
	Rbc_ChainDestroy(clientPtr->events);
	Rbc_ChainDestroy(clientPtr->traces);
	ckfree(clientPtr);
    }
    Rbc_ChainDestroy(treeObjPtr->clients);

    TeardownTree(treeObjPtr, treeObjPtr->root);
    Rbc_PoolDestroy(treeObjPtr->nodePool);
    Rbc_PoolDestroy(treeObjPtr->valuePool);
    Tcl_DeleteHashTable(&treeObjPtr->nodeTable);

    if (treeObjPtr->hashPtr != NULL) {
	/* Remove the entry from the global tree table. */
	Tcl_DeleteHashEntry(treeObjPtr->hashPtr); 
    ...
    Tcl_Interp *interp)
{
    ...
    for (hPtr = Tcl_FirstHashEntry(&dataPtr->treeTable, &cursor);
	 hPtr != NULL; hPtr = Tcl_NextHashEntry(&cursor)) {
	treeObjPtr = (TreeObject *)Tcl_GetHashValue(hPtr);
    ...
    Rbc_TreeNotifyEvent *eventPtr)
{
    ...
    for (linkPtr = Rbc_ChainFirstLink(clientPtr->events); 
	linkPtr != NULL; linkPtr = nextPtr) {
	nextPtr = Rbc_ChainNextLink(linkPtr);
	notifyPtr = Rbc_ChainGetValue(linkPtr);
    ...
    int eventFlag)
{
     ...
     * been created.
     */
    for (linkPtr = Rbc_ChainFirstLink(treeObjPtr->clients);
...
static void
FreeValue(Node *nodePtr, TreeValue *valuePtr)
{
    if (valuePtr->objPtr != NULL) {
	Tcl_DecrRefCount(valuePtr->objPtr);
    }
    Rbc_PoolFreeItem(nodePtr->treeObject->valuePool, valuePtr);
}
    ...
    CONST char *string;		/* String to convert. */
{
    ...
    hPtr = Tcl_CreateHashEntry(&keyTable, string, &isNew);
    return (Rbc_TreeKey)Tcl_GetHashKey(&keyTable, hPtr);
}
				 ...
				 * where to insert the new node. */
{
	...
	hPtr = Tcl_CreateHashEntry(&treeObjPtr->nodeTable,(char *)inode, 
		   &isNew);
    } while (!isNew);
    nodePtr = NewNode(treeObjPtr, name, inode);
    Tcl_SetHashValue(hPtr, nodePtr);
 ...
 *----------------------------------------------------------------------
 *
 * Rbc_TreeCreateNodeWithId --
 *
 *	Like Rbc_TreeCreateNode, but provides a specific id to use
				 ...
				 * where to insert the new node. */
{
    ...
    treeObjPtr = parentPtr->treeObject;
    hPtr = Tcl_CreateHashEntry(&treeObjPtr->nodeTable,(char *)inode, &isNew);
    if (!isNew) {
	return NULL;
    }
    nodePtr = NewNode(treeObjPtr, name, inode);
    ...
    Node *beforePtr)
{
    ...
    if (Rbc_TreeIsAncestor(nodePtr, parentPtr)) {
	return TCL_ERROR;
    }
    UnlinkNode(nodePtr);
    /* 
...
int
Rbc_TreeDeleteNode(TreeClient *clientPtr, Node *nodePtr)
{
    TreeObject *treeObjPtr = nodePtr->treeObject;
    Node *childPtr, *nextPtr;
    ...
    for (childPtr = nodePtr->first; childPtr != NULL; childPtr = nextPtr) {
	nextPtr = childPtr->next;
	Rbc_TreeDeleteNode(clientPtr, childPtr);
    }
    /* 
...
Rbc_TreeNode
Rbc_TreeGetNode(TreeClient *clientPtr, unsigned int inode)
{
    TreeObject *treeObjPtr = clientPtr->treeObject;
    Tcl_HashEntry *hPtr;

    hPtr = Tcl_FindHashEntry(&treeObjPtr->nodeTable, (char *)inode);
    if (hPtr != NULL) {
	return (Rbc_TreeNode)Tcl_GetHashValue(hPtr);
    }
    return NULL;
}
    ...
    ClientData clientData)
{
    ...
    tracePtr = RbcCalloc(1, sizeof (TraceHandler));
    assert(tracePtr);
    tracePtr->linkPtr = Rbc_ChainAppend(clientPtr->traces, tracePtr);
    if (keyPattern != NULL) {
	tracePtr->keyPattern = RbcStrdup(keyPattern);
    }
    if (tagName != NULL) {
	tracePtr->withTag = RbcStrdup(tagName);
    }
    ...
    return (Rbc_TreeTrace)tracePtr;
}

void
Rbc_TreeDeleteTrace(Rbc_TreeTrace trace)
{
    TraceHandler *tracePtr = (TraceHandler *)trace;

    Rbc_ChainDeleteLink(tracePtr->clientPtr->traces, tracePtr->linkPtr);
    if (tracePtr->keyPattern != NULL) {
	ckfree(tracePtr->keyPattern);
    }
...
void
Rbc_TreeRelabelNode(TreeClient *clientPtr, Node *nodePtr, CONST char *string)
{
    nodePtr->label = Rbc_TreeGetKey(string);
    /* 
     * Issue callbacks to each client indicating that a new node has
     * been created.
     */
...
void
Rbc_TreeRelabelNode2(Node *nodePtr, CONST char *string)
{
    nodePtr->label = Rbc_TreeGetKey(string);
}
 ...
 */
Rbc_TreeNode
Rbc_TreeFindChild(Node *parentPtr, CONST char *string)
{
    Rbc_TreeKey label;
    register Node *nodePtr;
    
    label = Rbc_TreeGetKey(string);
 ...
 */
int
Rbc_TreeNodePosition(Node *nodePtr)
{
    Node *parentPtr;
    int count;
 ...
 */
Rbc_TreeNode
Rbc_TreePrevNode(Node *rootPtr, Node *nodePtr)
{
    Node *prevPtr;
 ...
 */
Rbc_TreeNode
Rbc_TreeNextNode(Node *rootPtr, Node *nodePtr)
{
    Node *nextPtr;
...
int
Rbc_TreeIsBefore(Node *n1Ptr, Node *n2Ptr)
{
    int depth;
    register int i;
    Node *nodePtr;
...
CallTraces(
    ...
    unsigned int flags)
{
    Rbc_ChainLink *l1Ptr, *l2Ptr;
    TreeClient *clientPtr;
    TraceHandler *tracePtr;	

    for(l1Ptr = Rbc_ChainFirstLink(treeObjPtr->clients); 
	l1Ptr != NULL; l1Ptr = Rbc_ChainNextLink(l1Ptr)) {
	clientPtr = Rbc_ChainGetValue(l1Ptr);
	for(l2Ptr = Rbc_ChainFirstLink(clientPtr->traces); 
	    l2Ptr != NULL; l2Ptr = Rbc_ChainNextLink(l2Ptr)) {
	    tracePtr = Rbc_ChainGetValue(l2Ptr);
	    if ((tracePtr->keyPattern != NULL) && 
		(!Tcl_StringMatch(key, tracePtr->keyPattern))) {
		continue;	/* Key pattern doesn't match. */
	    }
	    if ((tracePtr->withTag != NULL) && 
		(!Rbc_TreeHasTag(clientPtr, nodePtr, tracePtr->withTag))) {
...
int
Rbc_TreeValueExistsByKey(clientPtr, nodePtr, key)
    TreeClient *clientPtr;
    Node *nodePtr;
    Rbc_TreeKey key;
{
...
int
Rbc_TreeSetValueByKey(
    Tcl_Interp *interp,
    TreeClient *clientPtr,
    Node *nodePtr,		/* Node to be updated. */
    Rbc_TreeKey key,		/* Identifies the field key. */
...
int
Rbc_TreeUnsetValueByKey(
    Tcl_Interp *interp,
    TreeClient *clientPtr,
    Node *nodePtr,		/* Node to be updated. */
    Rbc_TreeKey key)		/* Name of field in node. */
{
    TreeObject *treeObjPtr = nodePtr->treeObject;
    TreeValue *valuePtr;
    ...
    Tcl_Obj **objPtrPtr)
{
    ...
    if (left != NULL) {
	*left = *right = '\0';
	result = Rbc_TreeGetArrayValue(interp, clientPtr, nodePtr, string, 
		left + 1, objPtrPtr);
	*left = '(', *right = ')';
    } else {
	result = Rbc_TreeGetValueByKey(interp, clientPtr, nodePtr, 
		Rbc_TreeGetKey(string), objPtrPtr);
    }
    return result;
}
				 ...
				 * is deleted. */
{
    ...
    if (left != NULL) {
	*left = *right = '\0';
	result = Rbc_TreeSetArrayValue(interp, clientPtr, nodePtr, string, 
		left + 1, valueObjPtr);
	*left = '(', *right = ')';
    } else {
	result = Rbc_TreeSetValueByKey(interp, clientPtr, nodePtr, 
			       Rbc_TreeGetKey(string), valueObjPtr);
    }
    return result;
}
    ...
    CONST char *string)		/* String identifying the field in node. */
{
    ...
    if (left != NULL) {
	*left = *right = '\0';
	result = Rbc_TreeUnsetArrayValue(interp, clientPtr, nodePtr, string, 
		left + 1);
	*left = '(', *right = ')';
    } else {
	result = Rbc_TreeUnsetValueByKey(interp, clientPtr, nodePtr, 
		Rbc_TreeGetKey(string));
    }
    return result;
}
...
int
Rbc_TreeValueExists(TreeClient *clientPtr, Node *nodePtr, CONST char *string)
{
    char *left, *right;
    int result;
    ...
    if (left != NULL) {
	*left = *right = '\0';
	result = Rbc_TreeArrayValueExists(clientPtr, nodePtr, string, left + 1);
	*left = '(', *right = ')';
    } else {
	result = Rbc_TreeValueExistsByKey(clientPtr, nodePtr, 
		Rbc_TreeGetKey(string));
    }
    return result;
}
...
Rbc_TreeKey
Rbc_TreeNextKey(TreeClient *clientPtr, Rbc_TreeKeySearch *iterPtr)
{
    TreeValue *valuePtr;
...
int
Rbc_TreeIsAncestor(Node *n1Ptr, Node *n2Ptr)
{
    if (n2Ptr != NULL) {
	n2Ptr = n2Ptr->parent;
	while (n2Ptr != NULL) {
    ...
    Rbc_TreeCompareNodesProc *proc)
{
    ...
    nodeArr = ckalloc((nNodes + 1) * sizeof(Node *));
    if (nodeArr == NULL) {
    ...
    *p = NULL;
	...
	UnlinkNode(*p);
	LinkBefore(nodePtr, *p, (Rbc_TreeNode)NULL);
    }
    ckfree(nodeArr);
...
Rbc_TreeApply(
    ...
    Rbc_TreeApplyProc *proc,	/* Procedure to call for each node. */
				 ...
				 * proc. */
{
	...
	/* 
	 * Get the next link in the chain before calling Rbc_TreeApply
	 * recursively.  This is because the apply callback may delete
	 * the node and its link.  
	 */

	nextPtr = childPtr->next;
	result = Rbc_TreeApply(childPtr, proc, clientData);
	TEST_RESULT(result);
    }
    ...
    int order)			/* Order of traversal. */
{
	...
	if (childPtr != NULL) {
	    result = Rbc_TreeApplyDFS(childPtr, proc, clientData, order);
	    TEST_RESULT(result);
	    childPtr = childPtr->next;
	}
	 ...
	 * Rbc_TreeApply recursively.  This is because the 
...
int
Rbc_TreeApplyBFS(nodePtr, proc, clientData)
    Node *nodePtr;		/* Root node of subtree. */
    Rbc_TreeApplyProc *proc;	/* Procedure to call for each node. */
    ClientData clientData;	/* One-word of data passed when calling
				 * proc. */
{
	...
	nextPtr = Rbc_ChainNextLink(linkPtr);
	Rbc_ChainDeleteLink(queuePtr, linkPtr);
	linkPtr = nextPtr;
    }
    Rbc_ChainDestroy(queuePtr);
    return TCL_OK;
...
NewTreeClient(TreeObject *treeObjPtr)
{
    ...
    clientPtr = RbcCalloc(1, sizeof(TreeClient));
    if (clientPtr != NULL) {
	Rbc_TreeTagTable *tablePtr;

	clientPtr->magic = TREE_MAGIC;
	clientPtr->linkPtr = Rbc_ChainAppend(treeObjPtr->clients, clientPtr);
	clientPtr->events = Rbc_ChainCreate();
	clientPtr->traces = Rbc_ChainCreate();
	clientPtr->treeObject = treeObjPtr;
	clientPtr->root = treeObjPtr->root;
	tablePtr = ckalloc(sizeof(Rbc_TreeTagTable));
...
void
Rbc_TreeReleaseToken(TreeClient *clientPtr)
{
    TreeObject *treeObjPtr;
    Rbc_ChainLink *linkPtr;
    EventHandler *notifyPtr;
    ...
    for (linkPtr = Rbc_ChainFirstLink(clientPtr->traces); linkPtr != NULL;
	 linkPtr = Rbc_ChainNextLink(linkPtr)) {
	...
	Rbc_ChainDeleteLink(treeObjPtr->clients, clientPtr->linkPtr);
	if (Rbc_ChainGetLength(treeObjPtr->clients) == 0) {
	    DestroyTreeObject(treeObjPtr);
	}
    }
    clientPtr->magic = 0;
...
int
Rbc_TreeSize(Node *nodePtr)
{
    int sum;

    sum = 0;
    Rbc_TreeApply(nodePtr, SizeApplyProc, &sum);
    return sum;
}


void
Rbc_TreeCreateEventHandler(
    TreeClient *clientPtr,
    unsigned int mask,
    Rbc_TreeNotifyEventProc *proc,
    ClientData clientData)
{
    ...
    if (linkPtr == NULL) {
	notifyPtr = ckalloc(sizeof (EventHandler));
	assert(notifyPtr);
	linkPtr = Rbc_ChainAppend(clientPtr->events, notifyPtr);
    }
    if (proc == NULL) {
	Rbc_ChainDeleteLink(clientPtr->events, linkPtr);
	ckfree(notifyPtr);
    } else {
	notifyPtr->proc = proc;
	notifyPtr->clientData = clientData;
 ...
 */
char *
Rbc_TreeNodePath(Node *nodePtr, Tcl_DString *resultPtr)
{
    ...
    CONST char *elemName)
{
    ...
    if (Rbc_GetArrayFromObj((Tcl_Interp *)NULL, valuePtr->objPtr, &tablePtr) 
	!= TCL_OK) {
	return FALSE;
    }
    hPtr = Tcl_FindHashEntry(tablePtr, elemName);
    return (hPtr != NULL);
}

int
Rbc_TreeGetArrayValue(
    Tcl_Interp *interp,
    TreeClient *clientPtr,
    Node *nodePtr,
    CONST char *arrayName,
    ...
    Tcl_Obj **valueObjPtrPtr)
{
    ...
    if (Rbc_GetArrayFromObj(interp, valuePtr->objPtr, &tablePtr) != TCL_OK) {
	return TCL_ERROR;
    }
    hPtr = Tcl_FindHashEntry(tablePtr, elemName);
    if (hPtr == NULL) {
    ...
    *valueObjPtrPtr = (Tcl_Obj *)Tcl_GetHashValue(hPtr);
    ...
    Tcl_Obj *valueObjPtr)	/* New value of element. */
{
    ...
    flags = TREE_TRACE_WRITE;
    if (isNew) {
	valuePtr->objPtr = Rbc_NewArrayObj(0, (Tcl_Obj **)NULL);
	Tcl_IncrRefCount(valuePtr->objPtr);
	flags |= TREE_TRACE_CREATE;
    ...
    hPtr = Tcl_CreateHashEntry(tablePtr, elemName, &isNew);
    assert(hPtr);
	...
	oldValueObjPtr = (Tcl_Obj *)Tcl_GetHashValue(hPtr);
    ...
    Tcl_Obj *listObjPtr)
{
    ...
    tablePtr = (Tcl_HashTable *)valuePtr->objPtr;
...
int
Rbc_TreeTagTableIsShared(TreeClient *clientPtr)
{
    return (clientPtr->tagTablePtr->refCount > 1);
}   

void
Rbc_TreeClearTags(TreeClient *clientPtr, Rbc_TreeNode node)
{
    Tcl_HashEntry *hPtr, *h2Ptr;
    Tcl_HashSearch cursor;
    
    for (hPtr = Tcl_FirstHashEntry(&clientPtr->tagTablePtr->tagTable, &cursor); 
	hPtr != NULL; hPtr = Tcl_NextHashEntry(&cursor)) {
	Rbc_TreeTagEntry *tPtr;
    ...
    CONST char *tagName)
{
	...
	(node == Rbc_TreeRootNode(clientPtr))) {
	return TRUE;
    }
    hPtr = Tcl_FindHashEntry(&clientPtr->tagTablePtr->tagTable, tagName);
    if (hPtr == NULL) {
	return FALSE;
    }
    tPtr = Tcl_GetHashValue(hPtr);
    hPtr = Tcl_FindHashEntry(&tPtr->nodeTable, (char *)node);
    if (hPtr == NULL) {
	return FALSE;
    }
    return TRUE;
    ...
    CONST char *tagName)
{
    ...
    hPtr = Tcl_CreateHashEntry(tablePtr, tagName, &isNew);
    assert(hPtr);
    if (isNew) {

	tPtr = ckalloc(sizeof(Rbc_TreeTagEntry));
	Tcl_InitHashTable(&tPtr->nodeTable, TCL_ONE_WORD_KEYS);
	Tcl_SetHashValue(hPtr, tPtr);
	tPtr->hashPtr = hPtr;
	tPtr->tagName = Tcl_GetHashKey(tablePtr, hPtr);
    } else {
	tPtr = Tcl_GetHashValue(hPtr);
    }
    hPtr = Tcl_CreateHashEntry(&tPtr->nodeTable, (char *)node, &isNew);
    assert(hPtr);
    if (isNew) {
...
void
Rbc_TreeForgetTag(TreeClient *clientPtr, CONST char *tagName)
{
    if ((strcmp(tagName, "all") != 0) && (strcmp(tagName, "root") != 0)) {
	Tcl_HashEntry *hPtr;
 ...
 *----------------------------------------------------------------------
 *
 * Rbc_TreeTagHashTable --
 *
 *---------------------------------------------------------------------- 
 */
Tcl_HashTable *
Rbc_TreeTagHashTable(TreeClient *clientPtr, CONST char *tagName)
{
    Tcl_HashEntry *hPtr;
...
Tcl_HashEntry *
Rbc_TreeFirstTag(TreeClient *clientPtr, Tcl_HashSearch *cursorPtr)
{
    return Tcl_FirstHashEntry(&clientPtr->tagTablePtr->tagTable, cursorPtr);
}
...
{
    TreeValue **newBucketPtr, **oldBuckets;
    register TreeValue **bucketPtr, **endPtr;
    register TreeValue *valuePtr, *nextPtr;
    unsigned int downshift;
    unsigned long mask;
    TreeValue **buckets;
    size_t nBuckets;

    oldBuckets = (TreeValue **)nodePtr->values;
    ...
    nodePtr->logSize += 2;
    nBuckets = (1 << nodePtr->logSize);
    buckets = RbcCalloc(nBuckets, sizeof(TreeValue *));
    ...
    nodePtr->values = (TreeValue *)buckets;
    ckfree(oldBuckets);
}
...
ConvertValues(Node *nodePtr)
{
    ...
    unsigned int mask;
    int downshift;
    TreeValue *valuePtr, *nextPtr, **bucketPtr;
 ...
 */
static int
TreeDeleteValue(Node *nodePtr, Rbc_TreeValue value)
{
    TreeValue *valuePtr = value;
    register TreeValue *prevPtr;
	...
	bucketPtr = (TreeValue **)nodePtr->values + RANDOM_INDEX(valuePtr->key);
...
TreeDestroyValues(Node *nodePtr)
{
	...
	buckets = (TreeValue **)nodePtr->values;
	nBuckets = (1 << nodePtr->logSize);
 ...
 *	Rbc_TreeNextValue will return all of the values in the table,
 *	one at a time.
 *
 * Side effects:
 *	None.
...
TreeFirstValue(
    Node *nodePtr,
    Rbc_TreeKeySearch *searchPtr) /* Place to store information about
 ...
 *	Rbc_TreeFirstValue, this procedure may be called to return
				   ...
				   * Rbc_TreeFirstValue. */
{
	...
	buckets = (TreeValue **)searchPtr->node->values;
...
TreeFindValue(
    Node *nodePtr,
    Rbc_TreeKey key)		/* Key to use to find matching entry. */
{
    register TreeValue *valuePtr;
    TreeValue *bucket;
	...
	mask = (1 << nodePtr->logSize) - 1;
	downshift = DOWNSHIFT_START - nodePtr->logSize;
	bucket = ((TreeValue **)(nodePtr->values))[RANDOM_INDEX((void *)key)];
    } else {
...
TreeCreateValue(
    Node *nodePtr,
    Rbc_TreeKey key,		/* Key to use to find or create matching
				 * entry. */
				 ...
				 * entry was created. */
{
	...
	mask = nBuckets - 1;
	downshift = DOWNSHIFT_START - nodePtr->logSize;
	bucketPtr = (TreeValue **)nodePtr->values + RANDOM_INDEX((void *)key);
	...
	*newPtr = FALSE;
	...
	/* TreeValue not found. Add a new value to the bucket. */
	*newPtr = TRUE;
	valuePtr = Rbc_PoolAllocItem(nodePtr->treeObject->valuePool, 
		sizeof(TreeValue));
	valuePtr->key = key;
	valuePtr->owner = NULL;
	valuePtr->next = *bucketPtr;
	...
	*newPtr = FALSE;
	...
	/* TreeValue not found. Add a new value to the list. */
	*newPtr = TRUE;
	valuePtr = Rbc_PoolAllocItem(nodePtr->treeObject->valuePool, 
		sizeof(TreeValue));
	valuePtr->key = key;
