 ...
 * rbcContainer.c --
 *
 *	This module implements a container widget for the RBC toolkit.
 *
    ...
    Window window;
{
	...
	for (i = 0; i < nChildren; i++) {
	     ...
	     */
	    Rbc_ChainPrepend(chainPtr, (ClientData)children[i]);
	}
	if (children != NULL) {
	    XFree((char *)children);
	}
    ...
    SearchInfo *searchPtr;
{
	...
	for (linkPtr = Rbc_ChainFirstLink(chainPtr); linkPtr != NULL;
	    ...
	    child = (Window)Rbc_ChainGetValue(linkPtr);
    ...
    int offset;			/* Offset to field in structure */
{
	...
	root = RootWindow(cntrPtr->display, Tk_ScreenNumber(cntrPtr->tkwin));
	if (Rbc_ReparentWindow(cntrPtr->display, *winPtr, root, 
    ...
    int flags;
{
    ...
    if (Rbc_ConfigModified(configSpecs, "-window", "-name", "-command", 
			   (char *)NULL)) {
	cntrPtr->flags &= ~CONTAINER_MAPPED;
	if (cntrPtr->adopted != None) {
	    if (Rbc_ReparentWindow(cntrPtr->display, cntrPtr->adopted,
    ...
    char **argv;		/* Argument strings. */
{
    ...
    cntrPtr = RbcCalloc(1, sizeof(Container));
    assert(cntrPtr);
    cntrPtr->tkwin = tkwin;
    ...
    ClientData clientData;	/* Information about widget. */
{
	...
	tkToplevel = Rbc_Toplevel(cntrPtr->tkwin);
	window = Rbc_GetRealWindowId(tkToplevel);
	...
	if (cntrPtr->borderWidth > 0) {
	    Rbc_Draw3DRectangle(cntrPtr->tkwin, drawable, cntrPtr->border,
	...
	Rbc_Fill3DRectangle(cntrPtr->tkwin, drawable, cntrPtr->border,
	    cntrPtr->highlightWidth, cntrPtr->highlightWidth,
	    Tk_Width(cntrPtr->tkwin) - 2 * cntrPtr->highlightWidth,
	    Tk_Height(cntrPtr->tkwin) - 2 * cntrPtr->highlightWidth,
	    cntrPtr->borderWidth, cntrPtr->relief);
 ...
 * --------------------------------------------------------------
 */
static Rbc_OpSpec opSpecs[] =
{
    {"cget", 2, (Rbc_Op)CgetOp, 3, 3, "option",},
    {"configure", 2, (Rbc_Op)ConfigureOp, 2, 0, "?option value?...",},
#ifndef WIN32
    {"find", 1, (Rbc_Op)FindOp, 3, 4, "?-command|-name? pattern",},
#endif /*WIN32*/
#ifdef notdef
    {"send", 1, (Rbc_Op)SendOp, 4, 4, "window string",},
#endif
};

static int nSpecs = sizeof(opSpecs) / sizeof(Rbc_OpSpec);
    ...
    char **argv;		/* Vector of argument strings. */
{
    ...
    proc = Rbc_GetOp(interp, nSpecs, opSpecs, RBC_OP_ARG1, argc, argv, 0);
    if (proc == NULL) {
	return TCL_ERROR;
    }
