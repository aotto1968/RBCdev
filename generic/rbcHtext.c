 ...
 * rbcHtext.c --
 *
 *	This module implements a hypertext widget for the RBC toolkit.
 *
...
typedef struct {
    ...
    Rbc_Chain *chainPtr;	/* Chain of embedded widgets on the line of 
				 * text */
} Line;
...
typedef struct {
    ...
    Tcl_HashTable widgetTable;	/* Table of embedded widgets. */
...
typedef struct {
    ...
    Rbc_Pad padX, padY;		/* Extra padding to frame around */
    ...
    int prevSize;
{
	...
	*arrayPtr = NULL;
    ...
    newPtr = RbcCalloc(elemSize, newSize);
    if (newPtr == NULL) {
	return TCL_ERROR;
    }
    ...
    int *indexPtr;
{
    ...
    if (Rbc_GetXY(htPtr->interp, htPtr->tkwin, string, &x, &y) != TCL_OK) {
	return TCL_ERROR;
    }
    ...
    if (Rbc_ChainGetLength(linePtr->chainPtr) > 0) {
	Rbc_ChainLink *linkPtr;
	int deltaX;
	EmbeddedWidget *winPtr;

	for (linkPtr = Rbc_ChainFirstLink(linePtr->chainPtr); linkPtr != NULL;
	    linkPtr = Rbc_ChainNextLink(linkPtr)) {
	    winPtr = Rbc_ChainGetValue(linkPtr);
    ...
    XEvent *eventPtr;		/* Information about event. */
{
	...
	hPtr = Tcl_FindHashEntry(&htPtr->widgetTable, (char *)winPtr->tkwin);
    ...
    Tk_Window tkwin;		/* Not used. */
{
    ...
    Tk_DeleteEventHandler(winPtr->tkwin, StructureNotifyMask,
	EmbeddedWidgetEventProc, winPtr);
    hPtr = Tcl_FindHashEntry(&(winPtr->htPtr->widgetTable), 
			     (char *)winPtr->tkwin);
    Tcl_DeleteHashEntry(hPtr);
    winPtr->cavityWidth = winPtr->cavityHeight = 0;
    winPtr->tkwin = NULL;
    ...
    Tk_Window tkwin;		/* Path name of embedded widget  */
{
    ...
    hPtr = Tcl_FindHashEntry(&(htPtr->widgetTable), (char *)tkwin);
    if (hPtr != NULL) {
	return (EmbeddedWidget *) Tcl_GetHashValue(hPtr);
    }
    return NULL;
}
    ...
    char *name;			/* Name of embedded widget */
{
    ...
    hPtr = Tcl_CreateHashEntry(&(htPtr->widgetTable), (char *)tkwin, &isNew);
    ...
    winPtr = RbcCalloc(1, sizeof(EmbeddedWidget));
    assert(winPtr);
    winPtr->flags = 0;
    winPtr->tkwin = tkwin;
    winPtr->htPtr = htPtr;
    ...
    HText *htPtr;
{
    ...
    linePtr->textEnd = -1;
    linePtr->baseline = 0;
    linePtr->chainPtr = Rbc_ChainCreate();
    ...
    Line *linePtr;
{
    ...
    Rbc_ChainDestroy(linePtr->chainPtr);
}
    ...
    DestroyData dataPtr;	/* Info about hypertext widget. */
{
	...
	ckfree(htPtr->lineArr);
    }
    Tcl_DeleteHashTable(&(htPtr->widgetTable));
    ckfree(htPtr);
}
...
#define fileno	 _fileno
    ...
    buffer = ckalloc(sizeof(char) * fileSize);
    if (buffer == NULL) {
	fclose(f);
    ...
    int nBytes;
{
    ...
    cmdArr = ckalloc(sizeof(char) * (nBytes + 1));

    textArr = ckalloc(sizeof(char) * (nBytes + 1));
    if (htPtr->charArr != NULL) {
	ckfree(htPtr->charArr);
    }
			         ...
			         * already have values for some fields. */
{
    ...
    if (Rbc_ConfigModified(configSpecs, "-font", "-linespacing", "-file",
    ...
    if (htPtr->tile != NULL) {
	Rbc_SetTileChangedProc(htPtr->tile, TileChangedProc, htPtr);
    }
     ...
     */
    if (Rbc_ConfigModified(configSpecs, "-file", "-text", (char *)NULL)) {
	int result;
    ...
    Segment *segPtr;
{
	...
	width = (selEnd == linePtr->textEnd)
	    ...
	    nextX - lastX;
	Rbc_Fill3DRectangle(htPtr->tkwin, draw, htPtr->selBorder,
    ...
    int deltaY;			/* Change from previous Y coordinate */
{
	...
	if (htPtr->tileOffsetPage) {
	    Rbc_SetTSOrigin(htPtr->tkwin, htPtr->tile, x, y);
	} else {
	    Rbc_SetTileOrigin(htPtr->tkwin, htPtr->tile, 0, 0);
	}
	Rbc_TileRectangle(htPtr->tkwin, pixmap, htPtr->tile, 0, 0, width, 
		height);
    } else {
    ...
    ClientData clientData;	/* Information about widget. */
{
	...
	htPtr->xOffset = Rbc_AdjustViewport(htPtr->pendingX,
	    htPtr->worldWidth, width,
	    htPtr->xScrollUnits, RBC_SCROLL_MODE_LISTBOX);
	htPtr->yOffset = Rbc_AdjustViewport(htPtr->pendingY,
	    htPtr->worldHeight, height,
	    htPtr->yScrollUnits, RBC_SCROLL_MODE_LISTBOX);
	if (htPtr->xScrollCmdPrefix != NULL) {
	    Rbc_UpdateScrollbar(htPtr->interp, htPtr->xScrollCmdPrefix,
		(double)htPtr->xOffset / htPtr->worldWidth,
		(double)(htPtr->xOffset + width) / htPtr->worldWidth);
	}
	if (htPtr->yScrollCmdPrefix != NULL) {
	    Rbc_UpdateScrollbar(htPtr->interp, htPtr->yScrollCmdPrefix,
  ...
  //if (htPtr->nLines) {
	...
	for (i = first; i <= last; i++) {
	    ...
	    for (linkPtr = Rbc_ChainFirstLink(htPtr->lineArr[i].chainPtr);
    ...
    char **argv;
{
    ...
    Tcl_SetResult(htPtr->interp, Rbc_Itoa(line), TCL_VOLATILE);
    return TCL_OK;
}
    ...
    char **argv;
{
    ...
    if (Rbc_GetScrollInfo(interp, argc - 2, argv + 2, &(htPtr->pendingX),
	    worldWidth, width, htPtr->xScrollUnits, RBC_SCROLL_MODE_LISTBOX) 
	!= TCL_OK) {
	return TCL_ERROR;
    }
    htPtr->flags |= TEXT_DIRTY;
    ...
    char **argv;
{
    ...
    if (Rbc_GetScrollInfo(interp, argc - 2, argv + 2, &(htPtr->pendingY),
	    worldHeight, height, htPtr->yScrollUnits, RBC_SCROLL_MODE_LISTBOX)
	!= TCL_OK) {
	return TCL_ERROR;
    }
    htPtr->flags |= TEXT_DIRTY;
    ...
    char **argv;		/* Argument strings. */
{
    ...
    Rbc_ChainAppend(linePtr->chainPtr, winPtr);
    ...
    char **argv;
{
    ...
    for (hPtr = Tcl_FirstHashEntry(&(htPtr->widgetTable), &cursor);
	hPtr != NULL; hPtr = Tcl_NextHashEntry(&cursor)) {
	winPtr = (EmbeddedWidget *)Tcl_GetHashValue(hPtr);
	if (winPtr->tkwin == NULL) {
	    fprintf(stderr, "window `%s' is null\n",
		Tk_PathName(Tcl_GetHashKey(&(htPtr->widgetTable), hPtr)));
	    continue;
	}
    ...
    char **argv;
{
    ...
    if (Rbc_GetXY(interp, htPtr->tkwin, argv[3], &x, &y) != TCL_OK) {
	return TCL_ERROR;
    }
    c = argv[2][0];
    length = strlen(argv[2]);
    ...
    char **argv;
{
    ...
    *endPtr = saved;
    ...
    if (match > 0) {
	Tcl_AppendElement(interp, Rbc_Itoa(matchStart));
	Tcl_AppendElement(interp, Rbc_Itoa(matchEnd));
    } else {
	Tcl_ResetResult(interp);
    }
    return TCL_OK;
    ...
    char **argv;
{
    ...
    Tcl_SetResult(interp, Rbc_Itoa(tindex), TCL_VOLATILE);
    return TCL_OK;
}
...
static Rbc_OpSpec textOps[] =
{
    {"append", 1, (Rbc_Op)AppendOp, 3, 0, "window ?option value?...",},
    {"cget", 2, (Rbc_Op)CgetOp, 3, 3, "?window? option",},
    {"configure", 2, (Rbc_Op)ConfigureOp, 2, 0,
	"?window? ?option value?...",},
    {"gotoline", 2, (Rbc_Op)GotoOp, 2, 3, "?line?",},
    {"index", 1, (Rbc_Op)IndexOp, 3, 3, "string",},
    {"linepos", 1, (Rbc_Op)LinePosOp, 3, 3, "string",},
    {"range", 2, (Rbc_Op)RangeOp, 2, 4, "?from? ?to?",},
    {"scan", 2, (Rbc_Op)ScanOp, 4, 4, "oper @x,y",},
    {"search", 3, (Rbc_Op)SearchOp, 3, 5, "pattern ?from? ?to?",},
    {"selection", 3, (Rbc_Op)SelectOp, 3, 5, "oper ?index?",},
    {"windows", 6, (Rbc_Op)WindowsOp, 2, 3, "?pattern?",},
    {"xview", 1, (Rbc_Op)XViewOp, 2, 5,
	"?moveto fract? ?scroll number what?",},
    {"yview", 1, (Rbc_Op)YViewOp, 2, 5,
	"?moveto fract? ?scroll number what?",},
};
static int nTextOps = sizeof(textOps) / sizeof(Rbc_OpSpec);
    ...
    char **argv;		/* Argument strings. */
{
    ...
    proc = Rbc_GetOp(interp, nTextOps, textOps, RBC_OP_ARG1, argc, argv, 0);
    if (proc == NULL) {
	return TCL_ERROR;
    }
    Tcl_Preserve(htPtr);
    ...
    CONST84 char **argv;	/* Argument strings. */
{
    ...
    htPtr = RbcCalloc(1, sizeof(HText));
    assert(htPtr);
    ...
    Tcl_InitHashTable(&(htPtr->widgetTable), TCL_ONE_WORD_KEYS);

    /* Reset number of lines allocated */
    if (ResizeArray((char **)&(htPtr->lineArr), sizeof(Line), 3,0) != TCL_OK) {
	Tcl_AppendResult(interp, "can't reallocate array of lines", (char *)NULL);
	return TCL_ERROR;
    }
    htPtr->nLines = 0;
