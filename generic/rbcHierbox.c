...
extern Tk_CustomOption rbcUidOption;
 ...
 */
struct TreeStruct {
    Rbc_Uid nameId;		/* String identifying the node within this
    ...
    Rbc_Chain *chainPtr;	/* Pointer to list of child nodes. The list
				 * isn't allocated until the child nodes are
				 * to be inserted. */

    Rbc_ChainLink *linkPtr;	/* List link containing this node in parent.
...
struct EntryStruct {
    ...
    Rbc_Uid dataUid;		/* This value isn't used in C code.
				 * It may be used in Tcl bindings to
				 * associate extra data (other than
				 * the label, image, or text) with the
				 * entry. */

    Rbc_Uid tags;		/* List of binding tags for this
				 * entry.  Rbc_Uid, not a string,
				 * because in the typical case most
				 * entries will have the same
				 * bintags. */
    Tcl_HashEntry *hashPtr;	/* Points the hash entry containing this
				 * entry.  This is how the node index is
				 * stored--as the key of the hash entry. */
    Hierbox *hboxPtr;

    Rbc_Uid openCmd, closeCmd;	/* Tcl commands to invoke when entries
...
static Tk_ConfigSpec entryConfigSpecs[] =
{
	...
	TK_CONFIG_NULL_OK, &rbcUidOption},
	...
	TK_CONFIG_NULL_OK | TK_CONFIG_COLOR_ONLY, &rbcShadowOption},
    {TK_CONFIG_CUSTOM, "-labelshadow", "labelShadow", "LabelShadow",
	DEF_ENTRY_SHADOW_MONO, Tk_Offset(Entry, labelShadow),
	TK_CONFIG_NULL_OK | TK_CONFIG_MONO_ONLY, &rbcShadowOption},
...
struct HierboxStruct {
    ...
    Rbc_Tile tile;		/* Tiled background */
    ...
    XColor *focusColor;
    Rbc_Dashes focusDashes;	/* Dash on-off value. */
    ...
    int scrollMode;		/* Selects mode of scrolling: either
				 * RBC_SCROLL_MODE_HIERBOX, 
				 * RBC_SCROLL_MODE_LISTBOX, or
				 * RBC_SCROLL_MODE_CANVAS. */
    ...
    Rbc_BindTable bindTable;	/* Binding information for entries. */
    Rbc_BindTable buttonBindTable; /* Binding information for buttons. */
};
...
static Tk_ConfigSpec buttonConfigSpecs[] =
{
	...
	Tk_Offset(Hierbox, button.reqSize), 0, &rbcDistanceOption},
    {TK_CONFIG_END, (char *)NULL, (char *)NULL, (char *)NULL,
	(char *)NULL, 0, 0}
};
...
static Rbc_ChainCompareProc CompareNodesByTclCmd, CompareNodesByName;
static Rbc_BindPickProc PickButton, PickEntry;
    ...
    char *name;
{
    ...
    hPtr = Tcl_CreateHashEntry(&(hboxPtr->imageTable), (char *)name, &isNew);
    if (isNew) {
	Tk_Image tkImage;
	int width, height;
	...
	imagePtr = ckalloc(sizeof(struct CachedImageStruct));
	imagePtr->tkImage = tkImage;
	imagePtr->hashPtr = hPtr;
	imagePtr->refCount = 1;
	imagePtr->width = width, imagePtr->height = height;
	Tcl_SetHashValue(hPtr, imagePtr);
    } else {
	imagePtr = (struct CachedImageStruct *)Tcl_GetHashValue(hPtr);
	imagePtr->refCount++;
    }
    return imagePtr;
}
    ...
    int offset;			/* offset to field in structure */
{
	...
	if (nNames > 0) {
	    ...
	    imageArr = ckalloc(sizeof(CachedImage *) * (nNames + 1));
	    assert(imageArr);
	    for (i = 0; i < nNames; i++) {
    ...
    Tcl_FreeProc **freeProcPtr;	/* Memory deallocation scheme to use */
{
	...
	for (imagePtr = *imagePtrPtr; *imagePtr != NULL; imagePtr++) {
	    Tcl_DStringAppendElement(&dString, 
			     Rbc_NameOfImage((*imagePtr)->tkImage));
	}
    }
    ...
    unsigned int flags;
{
	...
	if (!(flags & APPLY_OPEN_ONLY) ||
	    ...
	    for (linkPtr = Rbc_ChainFirstLink(rootPtr->chainPtr); 
		...
		treePtr = Rbc_ChainGetValue(linkPtr);
    ...
    unsigned int mask;
{
    ...
    linkPtr = Rbc_ChainPrevLink(treePtr->linkPtr);
    if (linkPtr == NULL) {
	...
	linkPtr = Rbc_ChainLastLink(treePtr->chainPtr);
	if (linkPtr == NULL) {
	    break;		/* Found a leaf. */
	}
	treePtr = Rbc_ChainGetValue(linkPtr);
    ...
    unsigned int mask;
{
	...
	linkPtr = Rbc_ChainFirstLink(treePtr->chainPtr);
	if (linkPtr != NULL) {
	...
	linkPtr = Rbc_ChainNextLink(treePtr->linkPtr);
	if (linkPtr != NULL) {
	    return Rbc_ChainGetValue(linkPtr);
	}
    ...
    register Tree *t1Ptr, *t2Ptr;
{
    ...
    for (linkPtr = Rbc_ChainFirstLink(t1Ptr->parentPtr->chainPtr); linkPtr != NULL;
    ...
    Tree *treePtr;
{
    ...
    hPtr = Tcl_CreateHashEntry(&(hboxPtr->selectTable), (char *)treePtr, 
       &isNew);
    if (isNew) {
	Rbc_ChainLink *linkPtr;

	linkPtr = Rbc_ChainAppend(&(hboxPtr->selectChain), treePtr);
    ...
    Tree *treePtr;
{
    ...
    hPtr = Tcl_FindHashEntry(&(hboxPtr->selectTable), (char *)treePtr);
    if (hPtr != NULL) {
	Rbc_ChainLink *linkPtr;

	linkPtr = (Rbc_ChainLink *)Tcl_GetHashValue(hPtr);
	Rbc_ChainDeleteLink(&(hboxPtr->selectChain), linkPtr);
	Tcl_DeleteHashEntry(hPtr);
    }
}
...
ClearSelection(hboxPtr)
    Hierbox *hboxPtr;
{
    Tcl_DeleteHashTable(&(hboxPtr->selectTable));
    Tcl_InitHashTable(&(hboxPtr->selectTable), TCL_ONE_WORD_KEYS);
    Rbc_ChainReset(&(hboxPtr->selectChain));
    EventuallyRedraw(hboxPtr);
    ...
    Tree *rootPtr;
{
    ...
    for (linkPtr = Rbc_ChainFirstLink(&(hboxPtr->selectChain)); 
	 linkPtr != NULL; linkPtr = nextPtr) {
	nextPtr = Rbc_ChainNextLink(linkPtr);
	treePtr = Rbc_ChainGetValue(linkPtr);
    ...
    Tcl_DString *resultPtr;
{
    ...
    level = treePtr->level;
    nameArr = ckalloc((level + 1) * sizeof(char *));
    assert(nameArr);
    for (i = level; i >= 0; i--) {
    ...
    Tree *nodePtr;		/* The node to be inserted. */
{
	...
	parentPtr->chainPtr = Rbc_ChainCreate();
    }
    linkPtr = Rbc_ChainNewLink();
    if (position == APPEND) {
	Rbc_ChainAppendLink(parentPtr->chainPtr, linkPtr);
    } else {
	Rbc_ChainLink *beforePtr;

	beforePtr = Rbc_ChainGetNthLink(parentPtr->chainPtr, position);
	Rbc_ChainLinkBefore(parentPtr->chainPtr, linkPtr, beforePtr);
    }
    nodePtr->level = parentPtr->level + 1;
    nodePtr->parentPtr = parentPtr;
    nodePtr->linkPtr = linkPtr;
    Rbc_ChainSetValue(linkPtr, nodePtr);
}
    ...
    DestroyData data;
{
	...
	Rbc_FreeUid(treePtr->nameId);
    }
    if (treePtr->chainPtr != NULL) {
	Rbc_ChainDestroy(treePtr->chainPtr);
    }
    ...
    char *name;			/* Name identifier for the new node. */
{
    ...
    entryPtr = RbcCalloc(1, sizeof(Entry));
    assert(entryPtr);
    ...
    entryPtr->labelText = RbcStrdup(name);
    ...
    treePtr = RbcCalloc(1, sizeof(Tree));
    assert(treePtr);
    treePtr->nameId = Rbc_GetUid(name);
    treePtr->entryPtr = entryPtr;

    /* Generate a unique node serial number. */
    do {
	serial = hboxPtr->nextSerial++;
	hPtr = Tcl_CreateHashEntry(&(hboxPtr->nodeTable), (char *)serial,
	    &isNew);
    } while (!isNew);
    Tcl_SetHashValue(hPtr, treePtr);
    ...
    char *name;
{
    ...
    nameId = Rbc_FindUid(name);
    if (nameId != NULL) {
	register Tree *treePtr;
	Rbc_ChainLink *linkPtr;

	/* The component identifier must already exist if the node exists. */
	for (linkPtr = Rbc_ChainFirstLink(parentPtr->chainPtr); linkPtr != NULL;
	    linkPtr = Rbc_ChainNextLink(linkPtr)) {
	    treePtr = Rbc_ChainGetValue(linkPtr);
    ...
    char *string;
{
	...
	if (Tcl_GetInt(NULL, string, &serial) == TCL_OK) {
	    ...
	    hPtr = Tcl_FindHashEntry(&(hboxPtr->nodeTable), (char *)serial);
	    if (hPtr != NULL) {
    ...
    Tree *nodePtr;
{
    ...
    serial = (int)Tcl_GetHashKey(&(hboxPtr->nodeTable),
    ...
    Tree **treePtrPtr;
{
	...
	if (!(hboxPtr->flags & HIERBOX_DIRTY)) {
	    nodePtr = (Tree *) Rbc_GetCurrentItem(hboxPtr->bindTable);
	    if (nodePtr == NULL) {
		nodePtr = (Tree *)Rbc_GetCurrentItem(hboxPtr->buttonBindTable);
	    }
	}
	...
	if (fromPtr->linkPtr != NULL) {
	    ...
	    linkPtr = Rbc_ChainNextLink(fromPtr->linkPtr);
	    if (linkPtr != NULL) {
		nodePtr = Rbc_ChainGetValue(linkPtr);
	    }
	}
	...
	if (fromPtr->linkPtr != NULL) {
	    ...
	    linkPtr = Rbc_ChainPrevLink(fromPtr->linkPtr);
	    if (linkPtr != NULL) {
		nodePtr = Rbc_ChainGetValue(linkPtr);
	    }
	}
	...
	if (Rbc_GetXY(hboxPtr->interp, hboxPtr->tkwin, string, &x, &y)
	    == TCL_OK) {
    ...
    Tree *treePtr;
{
    ...
    *sumPtr += Rbc_ChainGetLength(treePtr->chainPtr);
    return TCL_OK;
}
...
CompareNodesByName(link1PtrPtr, link2PtrPtr)
    Rbc_ChainLink **link1PtrPtr, **link2PtrPtr;
{
    Tree *t1Ptr, *t2Ptr;

    t1Ptr = Rbc_ChainGetValue(*link1PtrPtr);
    t2Ptr = Rbc_ChainGetValue(*link2PtrPtr);
    ...
    Tree *treePtr;
{
	...
	if (hboxPtr->sortCmd != NULL) {
	    hierBox = hboxPtr;
	    Rbc_ChainSort(treePtr->chainPtr, CompareNodesByTclCmd);
	} else {
	    Rbc_ChainSort(treePtr->chainPtr, CompareNodesByName);
	}
    }
    return TCL_OK;
}
    ...
    Tree *treePtr;
{
	...
	if (IsAncestor(treePtr, hboxPtr->focusPtr)) {
	    ...
	    Rbc_SetFocusItem(hboxPtr->bindTable, hboxPtr->focusPtr, NULL);
	}
    }
    return TCL_OK;
}
    ...
    Tree *treePtr;
{
	...
	Rbc_ChainDeleteLink(treePtr->parentPtr->chainPtr, treePtr->linkPtr);
    ...
    Rbc_DeleteBindings(hboxPtr->bindTable, treePtr);
    Rbc_DeleteBindings(hboxPtr->buttonBindTable, treePtr);
    ...
    Rbc_List list;
{
    ...
    Rbc_ListAppend(list, (char *)object, 0);
    treePtr = (Tree *) object;
	...
	if (Tcl_SplitList((Tcl_Interp *)NULL, treePtr->entryPtr->tags, &nNames,
		...
		Rbc_ListAppend(list, Tk_GetUid(*p), 0);
	    }
	    ckfree(names);
	}
    }
    ...
    int flags;
{
	...
	ts.font = font;
	Rbc_GetTextExtents(&ts, entryPtr->labelText, &width, &height);
    }
	...
	Rbc_GetTextExtents(&ts, entryPtr->dataText, &width, &height);
	width += 2 * LABEL_PADX;
	height += 2 * LABEL_PADY;
    }
    entryWidth += width;
    ...
    Tk_Window tkwin;
{
    ...
    hboxPtr = RbcCalloc(1, sizeof(Hierbox));
    assert(hboxPtr);
    ...
    hboxPtr->scrollMode = RBC_SCROLL_MODE_HIERBOX;
    ...
    Rbc_ChainInit(&(hboxPtr->selectChain));
    Tcl_InitHashTable(&(hboxPtr->selectTable), TCL_ONE_WORD_KEYS);
    Tcl_InitHashTable(&(hboxPtr->nodeTable), TCL_ONE_WORD_KEYS);
    Tcl_InitHashTable(&(hboxPtr->imageTable), TCL_STRING_KEYS);
    hboxPtr->bindTable = Rbc_CreateBindingTable(interp, tkwin, hboxPtr, 
	PickEntry, GetTags);
    ...
    DestroyData dataPtr;	/* Pointer to the widget record. */
{
	...
	Rbc_FreePrivateGC(hboxPtr->display, hboxPtr->focusGC);
    }
    if (hboxPtr->tile != NULL) {
	Rbc_FreeTile(hboxPtr->tile);
    }
    if (hboxPtr->visibleArr != NULL) {
	ckfree(hboxPtr->visibleArr);
    }
    if (hboxPtr->levelInfo != NULL) {
	ckfree(hboxPtr->levelInfo);
    }
    ...
    Tcl_DeleteHashTable(&(hboxPtr->nodeTable));
    Rbc_ChainReset(&(hboxPtr->selectChain));
    Tcl_DeleteHashTable(&(hboxPtr->selectTable));
    Rbc_DestroyBindingTable(hboxPtr->bindTable);
    Rbc_DestroyBindingTable(hboxPtr->buttonBindTable);
    ckfree(hboxPtr);
}
    ...
    int flags;
{
    ...
    if (Rbc_ConfigModified(configSpecs, "-font", "-linespacing", "-width",
    ...
    newGC = Rbc_GetPrivateGC(hboxPtr->tkwin, gcMask, &gcValues);
    if (LineIsDashed(hboxPtr->focusDashes)) {
	hboxPtr->focusDashes.offset = 2;
	Rbc_SetDashes(hboxPtr->display, newGC, &(hboxPtr->focusDashes));
    }
    if (hboxPtr->focusGC != NULL) {
	Rbc_FreePrivateGC(hboxPtr->display, hboxPtr->focusGC);
    }
    ...
    if (hboxPtr->tile != NULL) {
	Rbc_SetTileChangedProc(hboxPtr->tile, TileChangedProc, hboxPtr);
    }
    ConfigureButtons(hboxPtr);
    ...
    LayoutInfo *infoPtr;
{
    ...
    if ((entryPtr->flags & BUTTON_SHOW) || ((entryPtr->flags & BUTTON_AUTO) &&
	    (Rbc_ChainGetLength(treePtr->chainPtr) > 0))) {
	...
	bottomPtr = treePtr;
	for (linkPtr = Rbc_ChainFirstLink(treePtr->chainPtr); linkPtr != NULL;
	    linkPtr = Rbc_ChainNextLink(linkPtr)) {
	    treePtr = Rbc_ChainGetValue(linkPtr);
	    if (treePtr->entryPtr->flags & ENTRY_MAPPED) {
		ResetCoordinates(hboxPtr, treePtr, infoPtr);
    ...
    Hierbox *hboxPtr;
{
    ...
    hboxPtr->levelInfo = RbcCalloc(hboxPtr->depth + 2, sizeof(LevelInfo));
    ...
    Hierbox *hboxPtr;
{
    ...
    hboxPtr->xOffset = Rbc_AdjustViewport(hboxPtr->xOffset, hboxPtr->worldWidth,
	VPORTWIDTH(hboxPtr), hboxPtr->xScrollUnits, hboxPtr->scrollMode);
    hboxPtr->yOffset = Rbc_AdjustViewport(hboxPtr->yOffset,
    ...
    hboxPtr->visibleArr = RbcCalloc(nSlots, sizeof(Tree *));
	...
	for (linkPtr = Rbc_ChainLastLink(treePtr->chainPtr); linkPtr != NULL;
    ...
    Tree *treePtr;
{
    ...
    ts.shadow.offset = entryPtr->labelShadow.offset;
    textPtr = Rbc_GetTextLayout(entryPtr->labelText, &ts);
    ...
    Drawable drawable;		/* Pixmap or window to draw into. */
{
    ...
    Rbc_Fill3DRectangle(hboxPtr->tkwin, drawable, border, x, y,
    ...
    Drawable drawable;		/* Pixmap or window to draw into. */
{
	...
	Rbc_SetDrawTextStyle(&ts, font, entryPtr->dataGC, colorPtr,
	...
	Rbc_DrawText(hboxPtr->tkwin, drawable, entryPtr->dataText, &ts, x, y);
    }
}
    ...
    Drawable drawable;		/* Pixmap or window to draw into. */
{
    ...
    Rbc_Fill3DRectangle(hboxPtr->tkwin, drawable, hboxPtr->border, x, y, width, 
    ...
    y += FOCUS_WIDTH;
    if (isSelected) {
	Rbc_Fill3DRectangle(hboxPtr->tkwin, drawable, hboxPtr->selBorder,
	...
	Rbc_SetDrawTextStyle(&ts, font, entryPtr->labelGC, normalColor,
	...
	Rbc_DrawText(hboxPtr->tkwin, drawable, entryPtr->labelText, &ts,
	    x, y);
    }
    ...
    Drawable drawable;
{
	...
	Rbc_Draw3DRectangle(hboxPtr->tkwin, drawable, hboxPtr->border,
    ...
    ClientData clientData;	/* Information about widget. */
{
	...
	ComputeVisibleEntries(hboxPtr);
	Rbc_PickCurrentItem(hboxPtr->bindTable);
	Rbc_PickCurrentItem(hboxPtr->buttonBindTable);

	width = VPORTWIDTH(hboxPtr);
	height = VPORTHEIGHT(hboxPtr);
	if (hboxPtr->flags & HIERBOX_XSCROLL) {
	    if (hboxPtr->xScrollCmdPrefix != NULL) {
		Rbc_UpdateScrollbar(hboxPtr->interp, hboxPtr->xScrollCmdPrefix,
	...
	if (hboxPtr->flags & HIERBOX_YSCROLL) {
		...
		Rbc_UpdateScrollbar(hboxPtr->interp, hboxPtr->yScrollCmdPrefix,
	...
	if (hboxPtr->scrollTile) {
	    Rbc_SetTSOrigin(hboxPtr->tkwin, hboxPtr->tile, -hboxPtr->xOffset,
		-hboxPtr->yOffset);
	} else {
	    Rbc_SetTileOrigin(hboxPtr->tkwin, hboxPtr->tile, 0, 0);
	}
	Rbc_TileRectangle(hboxPtr->tkwin, drawable, hboxPtr->tile, 0, 0,
	    Tk_Width(hboxPtr->tkwin), Tk_Height(hboxPtr->tkwin));
    } else {
	Rbc_Fill3DRectangle(hboxPtr->tkwin, drawable, hboxPtr->border, 0, 0,
	...
	if (hboxPtr->activePtr != NULL) {
	    ...
	    Rbc_Fill3DRectangle(hboxPtr->tkwin, drawable, hboxPtr->activeBorder, 
    ...
    char **argv;		/* Argument strings. */
{
    ...
    if (Rbc_ConfigureWidgetComponent(interp, tkwin, "button", "Button",
    ...
    char **argv;
{
    ...
    return Rbc_ConfigureBindings(interp, hboxPtr->buttonBindTable, object,
	argc - 4, argv + 4);
}
 ...
 *----------------------------------------------------------------------
 */
static Rbc_OpSpec buttonOperSpecs[] =
{
    {"activate", 1, (Rbc_Op)ButtonActivateOp, 4, 4, "node",},
    {"bind", 1, (Rbc_Op)ButtonBindOp, 4, 6,
	"tagName ?sequence command?",},
    {"cget", 2, (Rbc_Op)ButtonCgetOp, 4, 4, "option",},
    {"configure", 2, (Rbc_Op)ButtonConfigureOp, 3, 0,
	"?option value?...",},
    {"highlight", 1, (Rbc_Op)ButtonActivateOp, 4, 4, "node",},
};

static int nButtonSpecs = sizeof(buttonOperSpecs) / sizeof(Rbc_OpSpec);
    ...
    char **argv;
{
    ...
    proc = Rbc_GetOp(interp, nButtonSpecs, buttonOperSpecs, RBC_OP_ARG2, argc,
	 argv, 0);
    if (proc == NULL) {
	return TCL_ERROR;
    }
    ...
    char **argv;
{
    ...
    return Rbc_ConfigureBindings(interp, hboxPtr->bindTable, item, argc - 3, 
	argv + 3);
}
    ...
    char **argv;
{
    ...
    Rbc_SetBooleanResult(interp, IsHidden(treePtr));
    return TCL_OK;
}
    ...
    char **argv;
{
    ...
    Rbc_SetBooleanResult(interp, IsBefore(n1Ptr, n2Ptr));
    return TCL_OK;
}
    ...
    int *indexPtr;
{
	...
	*indexPtr = editPtr->selLast;
	...
	if (Rbc_GetXY(interp, hboxPtr->tkwin, string, &x, &y) != TCL_OK) {
	    return TCL_ERROR;
	}
    ...
    char **argv;
{
    ...
    Tcl_SetResult(interp, Rbc_Itoa(nChars), TCL_VOLATILE);
    return TCL_OK;
}
    ...
    char **argv;
{
    ...
    string = ckalloc(sizeof(char) * (newSize + 1));
    ...
    char **argv;
{
    ...
    p = string = ckalloc(sizeof(char) * (newSize + 1));
    ...
    char **argv;
{
    ...
    Rbc_SetBooleanResult(interp, (treePtr->entryPtr->flags & ENTRY_OPEN));
    return TCL_OK;
}
    ...
    char **argv;
{
	...
	Rbc_ChainLink *firstPtr, *lastPtr;
	int first, last;
	int nNodes;

	if ((Rbc_GetPosition(interp, argv[4], &first) != TCL_OK) ||
	    (Rbc_GetPosition(interp, argv[5], &last) != TCL_OK)) {
	    return TCL_ERROR;
	}
	nNodes = Rbc_ChainGetLength(parentPtr->chainPtr);
	if (nNodes == 0) {
	    return TCL_OK;
	}
	...
	firstPtr = Rbc_ChainGetNthLink(parentPtr->chainPtr, first);
	lastPtr = Rbc_ChainGetNthLink(parentPtr->chainPtr, last);
	if (first > last) {
	    for ( /*empty*/ ; lastPtr != NULL;
		lastPtr = Rbc_ChainPrevLink(lastPtr)) {
		nodePtr = Rbc_ChainGetValue(lastPtr);
		Tcl_AppendElement(interp, NodeToString(hboxPtr, nodePtr));
		if (lastPtr == firstPtr) {
		    break;
		}
		...
		firstPtr = Rbc_ChainNextLink(firstPtr)) {
		nodePtr = Rbc_ChainGetValue(firstPtr);
		Tcl_AppendElement(interp, NodeToString(hboxPtr, nodePtr));
		if (firstPtr == lastPtr) {
		    break;
		}
    ...
    char **argv;
{
    ...
    *sumPtr = 0;
    ...
    Tcl_SetResult(interp, Rbc_Itoa(*sumPtr), TCL_VOLATILE);
    return TCL_OK;
}
...
static Rbc_OpSpec entryOperSpecs[] =
{
    {"activate", 1, (Rbc_Op)ActivateOpOp, 4, 4, "node",},
    {"cget", 2, (Rbc_Op)CgetOpOp, 5, 5, "node option",},
    {"children", 2, (Rbc_Op)ChildrenOpOp, 4, 6, "node first last",},
    {"configure", 2, (Rbc_Op)ConfigureOpOp, 4, 0,
	"node ?node...? ?option value?...",},
    {"delete", 1, (Rbc_Op)DeleteOpOp, 6, 6, "node first last"},
    {"highlight", 1, (Rbc_Op)ActivateOpOp, 4, 4, "node",},
    {"index", 3, (Rbc_Op)IndexOpOp, 6, 6, "index"},
    {"insert", 3, (Rbc_Op)InsertOpOp, 6, 6, "node index string"},
    {"isbefore", 3, (Rbc_Op)IsBeforeOpOp, 5, 5, "node node",},
    {"ishidden", 3, (Rbc_Op)IsHiddenOpOp, 4, 4, "node",},
    {"isopen", 3, (Rbc_Op)IsOpenOpOp, 4, 4, "node",},
    {"size", 1, (Rbc_Op)SizeOpOp, 4, 5, "?-recurse? node",},
};
static int nEntrySpecs = sizeof(entryOperSpecs) / sizeof(Rbc_OpSpec);
    ...
    char **argv;
{
    ...
    proc = Rbc_GetOp(interp, nEntrySpecs, entryOperSpecs, RBC_OP_ARG2, argc, 
	argv, 0);
    if (proc == NULL) {
	return TCL_ERROR;
    }
    ...
    char **argv;
{
    ...
    nextProc = NextNode;
    optionList = Rbc_ListCreate(TCL_STRING_KEYS);
	...
	if ((c == 'e') && (length > 2) &&
	    ...
	    node = Rbc_ListGetNode(optionList, argv[i]);
	    if (node == NULL) {
		node = Rbc_ListCreateNode(optionList, argv[i]);
		Rbc_ListAppendNode(optionList, node);
	    }
	    Rbc_ListSetValue(node, argv[i + 1]);
	    i++;
	}
    }
	...
	for (node = Rbc_ListFirstNode(optionList); node != NULL;
	    node = Rbc_ListNextNode(node)) {
	    option = (char *)Rbc_ListGetKey(node);
	    Tcl_ResetResult(interp);
	    if (Tk_ConfigureValue(interp, hboxPtr->tkwin, entryConfigSpecs,
		    (char *)treePtr->entryPtr, option, 0) != TCL_OK) {
		goto error;	/* This shouldn't happen. */
	    }
	    pattern = (char *)Rbc_ListGetValue(node);
    ...
    int *nonMatchPtr;		/* returns: inverted search indicator */
{
	...
	(Rbc_ListGetLength(optionList) > 0)) {
	/*
    ...
    char **argv;
{
    ...
    if (Rbc_GetPosition(hboxPtr->interp, argv[2], &position) != TCL_OK) {
	return TCL_ERROR;
    }
    argc -= 3, argv += 3;
    ...
    char **argv;
{
    Tree *treePtr;
    Rbc_ChainLink *linkPtr;
    Rbc_ChainLink *firstPtr, *lastPtr, *nextPtr;
	...
	firstPtr = Rbc_ChainFirstLink(treePtr->chainPtr);
	lastPtr = Rbc_ChainLastLink(treePtr->chainPtr);
	break;
	 ...
	 */
	{
	    ...
	    if (Rbc_GetPosition(interp, argv[3], &position) != TCL_OK) {
		return TCL_ERROR;
	    }
	    if (position >= Rbc_ChainGetLength(treePtr->chainPtr)) {
		return TCL_OK;	/* Bad first index */
	    }
	    if (position == APPEND) {
		linkPtr = Rbc_ChainLastLink(treePtr->chainPtr);
	    } else {
		linkPtr = Rbc_ChainGetNthLink(treePtr->chainPtr, position);
	    }
	 ...
	 */
	{
	    ...
	    if ((Rbc_GetPosition(interp, argv[3], &first) != TCL_OK) ||
		(Rbc_GetPosition(interp, argv[4], &last) != TCL_OK)) {
		return TCL_ERROR;
	    }
	    nEntries = Rbc_ChainGetLength(treePtr->chainPtr);
	    if (nEntries == 0) {
		return TCL_OK;
	    }
	    if (first == APPEND) {
	    ...
	    firstPtr = Rbc_ChainGetNthLink(treePtr->chainPtr, first);
	    lastPtr = Rbc_ChainGetNthLink(treePtr->chainPtr, last);
	}
	break;
    }
...
#define MOVE_AFTER	(1<<2)
    ...
    Rbc_ChainUnlinkLink(srcPtr->parentPtr->chainPtr, srcPtr->linkPtr);
    switch (action) {
    case MOVE_INTO:
	Rbc_ChainLinkBefore(destPtr->chainPtr, srcPtr->linkPtr,
	    (Rbc_ChainLink *) NULL);
	parentPtr = destPtr;
	break;

    case MOVE_BEFORE:
	Rbc_ChainLinkBefore(parentPtr->chainPtr, srcPtr->linkPtr,
	    destPtr->linkPtr);
	break;

    case MOVE_AFTER:
	Rbc_ChainLinkAfter(parentPtr->chainPtr, srcPtr->linkPtr,
	    destPtr->linkPtr);
	break;
    }
    ...
    char **argv;
{
	...
	Rbc_SetBooleanResult(interp, bool);
    }
    return TCL_OK;
}
    ...
    char **argv;
{
    ...
    for (linkPtr = Rbc_ChainLastLink(&(hboxPtr->selectChain)); 
	 linkPtr != NULL; linkPtr = nextPtr) {
    ...
    char **argv;
{
    ...
    bool = (Rbc_ChainGetLength(&(hboxPtr->selectChain)) > 0);
    Rbc_SetBooleanResult(interp, bool);
    return TCL_OK;
}
 ...
 *----------------------------------------------------------------------
 */
static Rbc_OpSpec selectionOperSpecs[] =
{
    {"anchor", 1, (Rbc_Op)AnchorOpOp, 4, 4, "index",},
    {"clear", 5, (Rbc_Op)SelectOpOp, 4, 5, "firstIndex ?lastIndex?",},
    {"clearall", 6, (Rbc_Op)ClearallOpOp, 3, 3, "",},
    {"includes", 2, (Rbc_Op)IncludesOpOp, 4, 4, "index",},
    {"mark", 1, (Rbc_Op)MarkOpOp, 4, 4, "index",},
    {"present", 1, (Rbc_Op)PresentOpOp, 3, 3, "",},
    {"set", 1, (Rbc_Op)SelectOpOp, 4, 5, "firstIndex ?lastIndex?",},
    {"toggle", 1, (Rbc_Op)SelectOpOp, 4, 5, "firstIndex ?lastIndex?",},
};
static int nSelectionSpecs = sizeof(selectionOperSpecs) / sizeof(Rbc_OpSpec);
    ...
    char **argv;
{
    ...
    proc = Rbc_GetOp(interp, nSelectionSpecs, selectionOperSpecs, RBC_OP_ARG2,
	argc, argv, 0);
    if (proc == NULL) {
	return TCL_ERROR;
    }
    ...
    char **argv;
{
    ...
    if (Rbc_GetScrollInfo(interp, argc - 2, argv + 2, &(hboxPtr->xOffset),
    ...
    char **argv;
{
    ...
    if (Rbc_GetScrollInfo(interp, argc - 2, argv + 2, &(hboxPtr->yOffset),
 ...
 * --------------------------------------------------------------
 */
static Rbc_OpSpec operSpecs[] =
{
    {"bbox", 2, (Rbc_Op)BboxOp, 2, 0, "index...",},
    {"bind", 2, (Rbc_Op)BindOp, 3, 5, "tagName ?sequence command?",},
    {"button", 2, (Rbc_Op)ButtonOp, 2, 0, "args",},
    {"cget", 2, (Rbc_Op)CgetOp, 3, 3, "option",},
    {"close", 2, (Rbc_Op)CloseOp, 2, 0, "?-recurse? index...",},
    {"configure", 2, (Rbc_Op)ConfigureOp, 2, 0, "?option value?...",},
    {"curselection", 2, (Rbc_Op)CurselectionOp, 2, 2, "",},
    {"delete", 1, (Rbc_Op)DeleteOp, 2, 0, "?-recurse? index ?index...?",},
    {"entry", 1, (Rbc_Op)EntryOp, 2, 0, "oper args",},
    {"find", 2, (Rbc_Op)FindOp, 2, 0, "?flags...? ?firstIndex lastIndex?",},
    {"focus", 2, (Rbc_Op)FocusOp, 3, 3, "index",},
    {"get", 1, (Rbc_Op)GetOp, 2, 0, "?-full? index ?index...?",},
    {"hide", 1, (Rbc_Op)HideOp, 2, 0, "?-exact? ?-glob? ?-regexp? ?-nonmatching? ?-name string? ?-full string? ?-data string? ?--? ?index...?",},
    {"index", 3, (Rbc_Op)IndexOp, 3, 5, "?-at index? string",},
    {"insert", 3, (Rbc_Op)InsertOp, 3, 0, "?-at index? position label ?label...? ?option value?",},
    {"move", 1, (Rbc_Op)MoveOp, 5, 5, "index into|before|after index",},
    {"nearest", 1, (Rbc_Op)NearestOp, 4, 5, "x y ?varName?",},
    {"open", 1, (Rbc_Op)OpenOp, 2, 0, "?-recurse? index...",},
    {"range", 1, (Rbc_Op)RangeOp, 4, 5, "?-open? firstIndex lastIndex",},
    {"scan", 2, (Rbc_Op)ScanOp, 5, 5, "dragto|mark x y",},
    {"see", 3, (Rbc_Op)SeeOp, 3, 0, "?-anchor anchor? index",},
    {"selection", 3, (Rbc_Op)SelectionOp, 2, 0, "oper args",},
    {"show", 2, (Rbc_Op)ShowOp, 2, 0, "?-exact? ?-glob? ?-regexp? ?-nonmatching? ?-name string? ?-full string? ?-data string? ?--? ?index...?",},
    {"sort", 2, (Rbc_Op)SortOp, 2, 0, "?-recurse? ?-command string? index...",},
    {"toggle", 1, (Rbc_Op)ToggleOp, 3, 3, "index",},
    {"xview", 1, (Rbc_Op)XViewOp, 2, 5, "?moveto fract? ?scroll number what?",},
    {"yview", 1, (Rbc_Op)YViewOp, 2, 5, "?moveto fract? ?scroll number what?",},
};

static int nSpecs = sizeof(operSpecs) / sizeof(Rbc_OpSpec);
    ...
    char **argv;		/* Vector of argument strings. */
{
    ...
    proc = Rbc_GetOp(interp, nSpecs, operSpecs, RBC_OP_ARG1, argc, argv, 0);
    if (proc == NULL) {
	return TCL_ERROR;
    }
    Tcl_Preserve(hboxPtr);
 ...
 *----------------------------------------------------------------------
 */
static Rbc_OpSpec entrySelectionOperSpecs[] =
{
    {"adjust", 2, (Rbc_Op)AdjustOpOp, 5, 5, "index",},
    {"clear", 2, (Rbc_Op)ClearOpOp, 4, 4, "",},
    {"from", 1, (Rbc_Op)FromOpOp, 5, 5, "index",},
    {"present", 1, (Rbc_Op)PresentOpOp, 4, 4, "",},
    {"range", 1, (Rbc_Op)RangeOpOp, 4, 5, "firstIndex lastIndex",},
    {"to", 1, (Rbc_Op)ToOpOp, 5, 5, "index",},
};
static int nEntrySelectionSpecs =
sizeof(entrySelectionOperSpecs) / sizeof(Rbc_OpSpec);
