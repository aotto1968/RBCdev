...
/* Textbox Procedures */
static Rbc_ConfigSpec textboxConfigSpecs[] =
{
    {RBC_CONFIG_BORDER, "-background", "background", "Background",
	DEF_TEXTBOX_BACKGROUND, Rbc_Offset(Textbox, border), 0},
	...
	DEF_TEXTBOX_CURSOR, Rbc_Offset(Textbox, cursor), 
	RBC_CONFIG_NULL_OK},
    {RBC_CONFIG_DISTANCE, "-borderwidth", "borderWidth", "BorderWidth",
	DEF_TEXTBOX_BORDERWIDTH, Rbc_Offset(Textbox, borderWidth),
	RBC_CONFIG_DONT_SET_DEFAULT},
    {RBC_CONFIG_BORDER, "-buttonbackground", "buttonBackground", 
	"ButtonBackground", DEF_TEXTBOX_BUTTON_BACKGROUND,
	Rbc_Offset(Textbox, buttonBorder), 0},
    {RBC_CONFIG_RELIEF, "-buttonrelief", "buttonRelief", "ButtonRelief",
	DEF_TEXTBOX_BUTTON_RELIEF, Rbc_Offset(Textbox, buttonRelief),
	RBC_CONFIG_DONT_SET_DEFAULT},
    {RBC_CONFIG_DISTANCE, "-buttonborderwidth", "buttonBorderWidth", 
	"ButtonBorderWidth", DEF_TEXTBOX_BUTTON_BORDERWIDTH, 
	Rbc_Offset(Textbox, buttonBorderWidth),
	RBC_CONFIG_DONT_SET_DEFAULT},
    {RBC_CONFIG_BOOLEAN, "-exportselection", "exportSelection",
	"ExportSelection", DEF_TEXTBOX_EXPORT_SELECTION, 
	Rbc_Offset(Textbox, exportSelection), 
	RBC_CONFIG_DONT_SET_DEFAULT},
    {RBC_CONFIG_RELIEF, "-relief", "relief", "Relief",
	DEF_TEXTBOX_RELIEF, Rbc_Offset(Textbox, relief), 0},
    {RBC_CONFIG_BORDER, "-selectbackground", "selectBackground", "Background",
	DEF_TEXTBOX_SELECT_BG_MONO, Rbc_Offset(Textbox, selBorder),
	RBC_CONFIG_MONO_ONLY},
    {RBC_CONFIG_BORDER, "-selectbackground", "selectBackground", "Background",
	DEF_TEXTBOX_SELECT_BACKGROUND, Rbc_Offset(Textbox, selBorder),
	RBC_CONFIG_COLOR_ONLY},
    {RBC_CONFIG_DISTANCE, "-selectborderwidth", "selectBorderWidth", 
        "BorderWidth", DEF_TEXTBOX_SELECT_BORDERWIDTH, 
	Rbc_Offset(Textbox, selBorderWidth), 
	RBC_CONFIG_DONT_SET_DEFAULT},
    {RBC_CONFIG_COLOR, "-selectforeground", "selectForeground", "Foreground",

	DEF_TEXTBOX_SELECT_FG_MONO, Rbc_Offset(Textbox, selFgColor),
	RBC_CONFIG_MONO_ONLY},
    {RBC_CONFIG_COLOR, "-selectforeground", "selectForeground", "Foreground",
	DEF_TEXTBOX_SELECT_FOREGROUND, Rbc_Offset(Textbox, selFgColor),
	RBC_CONFIG_COLOR_ONLY},
    {RBC_CONFIG_RELIEF, "-selectrelief", "selectRelief", "Relief",
	DEF_TEXTBOX_SELECT_RELIEF, Rbc_Offset(Textbox, selRelief),
	RBC_CONFIG_DONT_SET_DEFAULT},
    {RBC_CONFIG_END, (char *)NULL, (char *)NULL, (char *)NULL, (char *)NULL, 
	0, 0}
};
    ...
    Textbox *tbPtr;
{
    ...
    ts.font = tbPtr->font;
    textPtr = Rbc_GetTextLayout(tbPtr->string, &ts);
    if (tbPtr->textPtr != NULL) {
	ckfree(tbPtr->textPtr);
    }
    ...
    int nBytes;
{
    ...
    newText = ckalloc(sizeof(char) * (newSize + 1));
    ...
    int firstPos, lastPos;
{
    ...
    newText = ckalloc(sizeof(char) * newSize);
    p = newText;
    if (firstPos > 0) {
    ...
    TreeViewColumn *columnPtr;
{
    ...
    tbPtr->string = RbcStrdup(string);
    tbPtr->gc = Rbc_TreeViewGetStyleGC(stylePtr);
    tbPtr->font = Rbc_TreeViewGetStyleFont(tvPtr, stylePtr);
    tbPtr->selFirst = tbPtr->selLast = -1;
    UpdateLayout(tbPtr);
    Tk_MapWindow(tbPtr->tkwin);
    EventuallyRedraw(tbPtr);
    ...
    int *indexPtr;
{
	...
	*indexPtr = 0;
	...
	if (Rbc_GetXY(interp, tbPtr->tkwin, string, &x, &y) != TCL_OK) {
	    return TCL_ERROR;
	}
    ...
    DestroyData data;
{
    ...
    Rbc_FreeObjOptions(textboxConfigSpecs, (char *)tbPtr, 
	tbPtr->display, 0);
...
int
Rbc_TreeViewTextbox(tvPtr, entryPtr, columnPtr)
    TreeView *tvPtr;
    TreeViewEntry *entryPtr;
    TreeViewColumn *columnPtr;
{
    ...
    tbPtr = RbcCalloc(1, sizeof(Textbox));
    assert(tbPtr);
    ...
    Rbc_SetWindowInstanceData(tkwin, tbPtr);
#endif
    ...
    if (Rbc_ConfigureWidgetFromObj(tvPtr->interp, tkwin, textboxConfigSpecs, 0, 
    ...
    ClientData clientData;
{
    ...
    Rbc_Fill3DRectangle(tbPtr->tkwin, drawable, tbPtr->border, 0, 0,
	Tk_Width(tbPtr->tkwin), Tk_Height(tbPtr->tkwin), 
	...
	if (selLength > 0) {	/* The selection itself */
	    ...
	    x2 += x;
	    width = (x2 - x1) + 1;
	    Rbc_Fill3DRectangle(tbPtr->tkwin, drawable, tbPtr->selBorder,
    ...
    Rbc_Fill3DRectangle(tbPtr->tkwin, drawable, tbPtr->buttonBorder, 
	       buttonX, buttonY, buttonWidth, buttonHeight,
	       tbPtr->buttonBorderWidth, tbPtr->buttonRelief); 
    
    buttonX += buttonWidth / 2;
    buttonY = tbPtr->height / 2;
    Rbc_DrawArrow(tbPtr->display, drawable, tbPtr->gc, buttonX,
		  buttonY, STD_ARROW_HEIGHT, ARROW_DOWN);
#endif
    Rbc_Draw3DRectangle(tbPtr->tkwin, drawable, tbPtr->border, 0, 0,
	Tk_Width(tbPtr->tkwin), Tk_Height(tbPtr->tkwin), 
	tbPtr->borderWidth, tbPtr->relief);
    ...
    Tcl_Obj *CONST *objv;	/* Not used. */
{
	...
	if (entryPtr->labelUid != NULL) {
	    Rbc_TreeViewFreeUid(tvPtr, entryPtr->labelUid);
	}
	if (tbPtr->string == NULL) {
	    entryPtr->labelUid = Rbc_TreeViewGetUid(tvPtr, "");
	} else {
	    entryPtr->labelUid = Rbc_TreeViewGetUid(tvPtr, tbPtr->string);
	}
    } else {
	TreeViewColumn *columnPtr;
	Tcl_Obj *objPtr;
    ...
    if (tvPtr != NULL) {
	Rbc_TreeViewConfigureEntry(tvPtr, entryPtr, 0, NULL, 
		RBC_CONFIG_OBJV_ONLY);
...
CgetOp(tbPtr, interp, objc, objv)
    ...
    Tcl_Obj *CONST *objv;
{
    return Rbc_ConfigureValueFromObj(interp, tbPtr->tkwin, 
...
{
    if (objc == 2) {
	return Rbc_ConfigureInfoFromObj(interp, tbPtr->tkwin, 
		textboxConfigSpecs, (char *)tbPtr, (Tcl_Obj *)NULL, 0);
    } else if (objc == 3) {
	return Rbc_ConfigureInfoFromObj(interp, tbPtr->tkwin, 
		textboxConfigSpecs, (char *)tbPtr, objv[3], 0);
    }
    if (Rbc_ConfigureWidgetFromObj(interp, tbPtr->tkwin, 
...
{
    {"adjust", 1, (Rbc_Op)SelectionAdjustOp, 4, 4, "index",},
    {"clear", 1, (Rbc_Op)SelectionClearOp, 3, 3, "",},
    {"from", 1, (Rbc_Op)SelectionFromOp, 4, 4, "index"},
    {"present", 1, (Rbc_Op)SelectionPresentOp, 3, 3, ""},
    {"range", 1, (Rbc_Op)SelectionRangeOp, 5, 5, "start end",},
    {"to", 1, (Rbc_Op)SelectionToOp, 4, 4, "index"},
};
 ...
 *----------------------------------------------------------------------
 */
static Rbc_OpSpec comboOps[] =
{
    {"apply", 1, (Rbc_Op)ApplyOp, 2, 2, "",},
    {"cancel", 2, (Rbc_Op)CancelOp, 2, 2, "",},
    {"cget", 2, (Rbc_Op)CgetOp, 3, 3, "value",},
    {"configure", 2, (Rbc_Op)ConfigureOp, 2, 0, "?option value...?",},
    {"delete", 1, (Rbc_Op)DeleteOp, 3, 4, "first ?last?"},
    {"icursor", 2, (Rbc_Op)IcursorOp, 3, 3, "index"},
    {"index", 3, (Rbc_Op)IndexOp, 3, 3, "index"},
    {"insert", 3, (Rbc_Op)InsertOp, 4, 4, "index string"},
    {"selection", 1, (Rbc_Op)SelectionOp, 2, 0, "args"},
};
static int nComboOps = sizeof(comboOps) / sizeof(Rbc_OpSpec);
    ...
    Tcl_Obj *CONST *objv;
{
    ...
    proc = Rbc_GetOpFromObj(interp, nComboOps, comboOps, RBC_OP_ARG1, objc, 
	objv, 0);
    if (proc == NULL) {
	return TCL_ERROR;
    }
