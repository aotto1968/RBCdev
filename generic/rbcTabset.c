 ...
 * rbcTabset.c --
 *
 *	This module implements a tabset widget for the RBC toolkit.
 *
    ...
    char *tagName;
{
    ...
    hPtr = Tcl_CreateHashEntry(&(setPtr->tagTable), tagName, &isNew);
    assert(hPtr);
    return Tcl_GetHashKey(&(setPtr->tagTable), hPtr);
}
    ...
    char *name;
{
	...
	imagePtr = Tcl_GetHashValue(hPtr);
	imagePtr->refCount++;
    }
    return imagePtr;
}
    ...
    Tab *tabPtr;
{
    ...
    count = 0;
    for (linkPtr = Rbc_ChainFirstLink(setPtr->chainPtr); linkPtr != NULL;
    ...
    Tab *tabPtr;
{
    ...
    Rbc_SetFocusItem(setPtr->bindTable, setPtr->focusPtr, NULL);
    ...
    char *string;
{
    ...
    hPtr = Tcl_FindHashEntry(&(setPtr->tabTable), string);
    return (hPtr != NULL);
}
    ...
    int allowNull;	/* Allow NULL tabPtr */
{
	...
	linkPtr = Rbc_ChainGetNthLink(setPtr->chainPtr, position);
	if (linkPtr == NULL) {
	...
	tabPtr = (Tab *)Rbc_GetCurrentItem(setPtr->bindTable);
	...
	linkPtr = Rbc_ChainLastLink(setPtr->chainPtr);
	if (linkPtr != NULL) {
	    tabPtr = Rbc_ChainGetValue(linkPtr);
	}
    } else if (c == '@') {
	...
	if (Rbc_GetXY(setPtr->interp, setPtr->tkwin, string, &x, &y) 
	    != TCL_OK) {
	    return TCL_ERROR;
	}
	tabPtr = PickTab(setPtr, x, y, NULL);
    ...
    Tab *tabPtr;
{
    ...
    hPtr = Tcl_FindHashEntry(&(setPtr->tabTable), tabPtr->name);
    assert(hPtr);
    Tcl_DeleteHashEntry(hPtr);
    ...
    if (tabPtr->linkPtr != NULL) {
	Rbc_ChainDeleteLink(setPtr->chainPtr, tabPtr->linkPtr);
    }
    if (tabPtr->tags != NULL) {
	Rbc_FreeUid(tabPtr->tags);
    }
    Rbc_DeleteBindings(setPtr->bindTable, tabPtr);
    ckfree(tabPtr);
}
    ...
    char *name;
{
    ...
    tabPtr->name = RbcStrdup(name);
    tabPtr->text = Rbc_GetUid(name);
    tabPtr->fill = FILL_NONE;
    tabPtr->anchor = TK_ANCHOR_CENTER;
    tabPtr->container = NULL;
    tabPtr->state = STATE_NORMAL;
    hPtr = Tcl_CreateHashEntry(&(setPtr->tabTable), name, &isNew);
    ...
    Tab *tabPtr;
{
	...
	Rbc_GetBoundingBox(labelWidth, labelHeight, setPtr->defTabStyle.rotate,
     ...
     */
    if (tabPtr->tile != NULL) {
	Rbc_SetTileChangedProc(tabPtr->tile, TileChangedProc, setPtr);
    }
    if (tabPtr->flags & TAB_VISIBLE) {
	EventuallyRedraw(setPtr);
    }
    ...
    Rbc_List list;
{
	...
	Rbc_ListAppend(list, MakeTag(setPtr, "Perforation"), 0);
    } else if (context == TAB_LABEL) {
	Rbc_ListAppend(list, MakeTag(setPtr, tabPtr->name), 0);
	if (tabPtr->tags != NULL) {
	    int nNames;
	    char **names;
	    register char **p;
		...
		for (p = names; *p != NULL; p++) {
		    Rbc_ListAppend(list, MakeTag(setPtr, *p), 0);
		}
		ckfree(names);
	    }
	}
    ...
    DestroyData dataPtr;	/* Pointer to the widget record. */
{
    ...
    if (setPtr->tile != NULL) {
	Rbc_FreeTile(setPtr->tile);
    }
    if (setPtr->defTabStyle.activeGC != NULL) {
	Rbc_FreePrivateGC(setPtr->display, setPtr->defTabStyle.activeGC);
    }
    for (linkPtr = Rbc_ChainFirstLink(setPtr->chainPtr); linkPtr != NULL;
	linkPtr = Rbc_ChainNextLink(linkPtr)) {
	tabPtr = Rbc_ChainGetValue(linkPtr);
    ...
    Rbc_ChainDestroy(setPtr->chainPtr);
    Rbc_DestroyBindingTable(setPtr->bindTable);
    Tcl_DeleteHashTable(&(setPtr->tabTable));
    Tcl_DeleteHashTable(&(setPtr->tagTable));
    ...
    Tk_Window tkwin;
{
    ...
    setPtr = RbcCalloc(1, sizeof(Tabset));
    assert(setPtr);
    ...
    setPtr->overlap = 0;
    setPtr->tearoff = TRUE;
    setPtr->bindTable = Rbc_CreateBindingTable(interp, tkwin, setPtr, PickTab, 
	GetTags);
    setPtr->chainPtr = Rbc_ChainCreate();
    Tcl_InitHashTable(&(setPtr->tabTable), TCL_STRING_KEYS);
    Tcl_InitHashTable(&(setPtr->imageTable), TCL_STRING_KEYS);
    Tcl_InitHashTable(&(setPtr->tagTable), TCL_STRING_KEYS);
#if (TK_MAJOR_VERSION > 4)
    Rbc_SetWindowInstanceData(tkwin, setPtr);
#endif
    return setPtr;
}
    ...
    int flags;
{
     ...
     */
    if (setPtr->tile != NULL) {
	Rbc_SetTileChangedProc(setPtr->tile, TileChangedProc, setPtr);
    }
    /*
     * GC for active line.
     */
    ...
    newGC = Rbc_GetPrivateGC(setPtr->tkwin, gcMask, &gcValues);
    if (LineIsDashed(setPtr->defTabStyle.dashes)) {
	setPtr->defTabStyle.dashes.offset = 2;
	Rbc_SetDashes(setPtr->display, newGC, &(setPtr->defTabStyle.dashes));
    }
    if (setPtr->defTabStyle.activeGC != NULL) {
	Rbc_FreePrivateGC(setPtr->display, setPtr->defTabStyle.activeGC);
    }
    ...
    char **argv;
{
	...
	for (hPtr = Tcl_FirstHashEntry(&(setPtr->tagTable), &cursor);
	    hPtr != NULL; hPtr = Tcl_NextHashEntry(&cursor)) {
	    tagName = Tcl_GetHashKey(&(setPtr->tagTable), hPtr);
	    Tcl_AppendElement(interp, tagName);
	}
	return TCL_OK;
    }
    return Rbc_ConfigureBindings(interp, setPtr->bindTable,
...
#define SEARCH_INDICES	2
	...
	Tcl_SetResult(interp, Rbc_Itoa(TabIndex(setPtr, tabPtr)), 
		TCL_VOLATILE);
    }
    return TCL_OK;
}
    ...
    char **argv;
{
	...
	if (position < 0) {
	    beforeLinkPtr = Rbc_ChainFirstLink(setPtr->chainPtr);
	} else if (position > Rbc_ChainGetLength(setPtr->chainPtr)) {
	    beforeLinkPtr = NULL;
	} else {
	    beforeLinkPtr = Rbc_ChainGetNthLink(setPtr->chainPtr, position);
	}
    } else {
	Tab *beforePtr;
	...
	count = i - start;
	options = argv + start;
	if (Rbc_ConfigureWidgetComponent(interp, setPtr->tkwin, tabPtr->name,
	...
	if (beforeLinkPtr == NULL) {
	    Rbc_ChainAppendLink(setPtr->chainPtr, linkPtr);
	} else {
	    Rbc_ChainLinkBefore(setPtr->chainPtr, linkPtr, beforeLinkPtr);
	}
    ...
    Tcl_DString *resultPtr;
{
	...
	if (*p == '%') {
		...
		*p = '%';
		...
		break;
	    case 'i':		/* Tab Index */
		string = Rbc_Itoa(TabIndex(setPtr, tabPtr));
		break;
	    case 'n':		/* Tab name */
		string = tabPtr->name;
		break;
    ...
    char **argv;
{
    ...
    Rbc_ChainUnlinkLink(setPtr->chainPtr, tabPtr->linkPtr);
    if (before) {
	Rbc_ChainLinkBefore(setPtr->chainPtr, tabPtr->linkPtr, linkPtr->linkPtr);
    } else {
	Rbc_ChainLinkAfter(setPtr->chainPtr, tabPtr->linkPtr, linkPtr->linkPtr);
    }
    setPtr->flags |= (TABSET_LAYOUT | TABSET_SCROLL);
    EventuallyRedraw(setPtr);
    return TCL_OK;
    ...
    char **argv;
{
	...
	RenumberTiers(setPtr, tabPtr);
	Rbc_PickCurrentItem(setPtr->bindTable);
    }
    ...
    char **argv;
{
    ...
    if (Rbc_GetScrollInfo(interp, argc - 2, argv + 2, &(setPtr->scrollOffset),
    ...
    DestroyData dataPtr;
{
	...
	if (tabPtr->tkwin != NULL) {
	    ...
	    Rbc_RelinkWindow(tabPtr->tkwin, setPtr->tkwin, rect.x, rect.y);
	    if (tabPtr == setPtr->selectPtr) {
...
TabPageHeight(setPtr, interp, argc, argv)
    ...
    char **argv;		/* Not used. */
{
    Tcl_SetResult(interp, Rbc_Itoa(VPORTHEIGHT(setPtr)), TCL_VOLATILE);
    return TCL_OK;
}
...
TabPageWidth(setPtr, interp, argc, argv)
    ...
    char **argv;		/* Not used. */
{
    Tcl_SetResult(interp, Rbc_Itoa(VPORTWIDTH(setPtr)), TCL_VOLATILE);
    return TCL_OK;
}
...
static Rbc_OpSpec tabOps[] =
{
    ...
    {"pageheight", 5, (Rbc_Op)TabPageHeight, 3, 3, "", },
    {"pagewidth", 5, (Rbc_Op)TabPageWidth, 3, 3, "", },
...
#define SCAN_DRAGTO	2
	...
	offset = Rbc_AdjustViewport(offset, setPtr->worldWidth,
	    VPORTWIDTH(setPtr), setPtr->scrollUnits, RBC_SCROLL_MODE_CANVAS);
...
SizeOp(setPtr, interp, argc, argv)
    ...
    char **argv;		/* Not used. */
{
    Tcl_SetResult(interp, Rbc_Itoa(Rbc_ChainGetLength(setPtr->chainPtr)),
	TCL_VOLATILE);
    return TCL_OK;
}

    ...
    int nTabs;
{
	...
	linkPtr = Rbc_ChainFirstLink(setPtr->chainPtr);
	count = 1;
	while (linkPtr != NULL) {
    ...
    Tabset *setPtr;
{
	...
	linkPtr = Rbc_ChainFirstLink(setPtr->chainPtr);
	if (linkPtr != NULL) {
	    setPtr->selectPtr = Rbc_ChainGetValue(linkPtr);
	}
    }
	...
	if (nTiers > setPtr->reqTiers) {
	    ...
	    x = 0;
	    nTiers = 1;
	    for (linkPtr = Rbc_ChainFirstLink(setPtr->chainPtr);
    ...
    Tabset *setPtr;
{
    ...
    setPtr->nVisible = 0;
    if (Rbc_ChainGetLength(setPtr->chainPtr) == 0) {
	return;
    }
    nVisibleTabs = 0;
    if (setPtr->flags & TABSET_STATIC) {
    ...
    int nPoints;
{
    ...
    if (tabPtr->tile != NULL) {
	Rbc_TilePolygon(setPtr->tkwin, drawable, tabPtr->tile, pointArr,
	    nPoints);
    ...
    Drawable drawable;
{
	...
	Rbc_DrawText(setPtr->tkwin, drawable, tabPtr->text, &ts, tx, ty);
    }
}
    ...
    Drawable drawable;
{
	...
	Rbc_Fill3DRectangle(setPtr->tkwin, drawable, perfBorder,
    ...
    Drawable drawable;
{
	...
	Rbc_Draw3DRectangle(setPtr->tkwin, drawable, setPtr->border,
    ...
    ClientData clientData;	/* Information about widget. */
{
	...
	width = VPORTWIDTH(setPtr);
	setPtr->scrollOffset = Rbc_AdjustViewport(setPtr->scrollOffset,
	    setPtr->worldWidth, width, setPtr->scrollUnits,
	    RBC_SCROLL_MODE_CANVAS);
	if (setPtr->scrollCmdPrefix != NULL) {
	    Rbc_UpdateScrollbar(setPtr->interp, setPtr->scrollCmdPrefix,
     ...
     */
    if (setPtr->tile != NULL) {
	Rbc_SetTileOrigin(setPtr->tkwin, setPtr->tile, 0, 0);
	Rbc_TileRectangle(setPtr->tkwin, drawable, setPtr->tile, 0, 0,
	    Tk_Width(setPtr->tkwin), height);
    } else {
	Rbc_Fill3DRectangle(setPtr->tkwin, drawable, setPtr->border, 0, 0,
	...
	for (i = 0; i < Rbc_ChainGetLength(setPtr->chainPtr); i++) {
    ...
    ClientData clientData;
{
     ...
     */
    if (setPtr->tile != NULL) {
	Rbc_SetTileOrigin(tkwin, setPtr->tile, 0, 0);
	Rbc_TileRectangle(tkwin, drawable, setPtr->tile, 0, 0,
	    Tk_Width(tkwin), Tk_Height(tkwin));
    } else {
	Rbc_Fill3DRectangle(tkwin, drawable, setPtr->border, 0, 0,
	...
	Rbc_Draw3DRectangle(tkwin, drawable, setPtr->border, 0, 0,
...
static int nTabsetOps = sizeof(tabsetOps) / sizeof(Rbc_OpSpec);
    ...
    char **argv;		/* Vector of argument strings. */
{
    ...
    proc = Rbc_GetOp(interp, nTabsetOps, tabsetOps, RBC_OP_ARG1, 
	argc, argv, 0);
    if (proc == NULL) {
	return TCL_ERROR;
    }
    ...
    char **argv;		/* Argument strings. */
{
     ...
     * script.
     */
    if (!Tcl_GetCommandInfo(interp, "::rbc::Tabset::ObjInit", &cmdInfo)) {
	static char initCmd[] = "source [file join $::rbc::library tabset.tcl]";
    ...
    if (Tcl_VarEval(interp, "::rbc::Tabset::ObjInit ", argv[1], (char *)NULL)
	!= TCL_OK) {
      goto error;
    }
...
Rbc_TabsetInit(interp)
    Tcl_Interp *interp;
{
    if (Tcl_CreateCommand(interp, "::rbc::tabset", TabsetCmd,
