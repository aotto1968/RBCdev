 ...
 * Todo:
 *	Test on win95
 *	Does terminating rbcwish kill child processes?
...
extern void Rbc_MapPid(HANDLE hProcess, DWORD pid);
...
SetupHandlers(ClientData clientData, int flags)
{
	...
	pipePtr = Rbc_ChainGetValue(linkPtr);
...
CheckHandlers(ClientData clientData, int flags)
{
	...
	if (queueEvent) {
	    ...
	    pipePtr->flags |= PIPE_PENDING;
	    eventPtr = ckalloc(sizeof(PipeEvent));
	    assert(eventPtr);
...
CreatePipeHandler(HANDLE hFile, int flags)
{
    ...
    pipePtr = RbcCalloc(1, sizeof(PipeHandler));
    assert(pipePtr);
	...
	pipePtr->buffer = RbcCalloc(1, PIPE_BUFSIZ);
...
GetPipeHandler(HANDLE hPipe)
{
    ...
    for (linkPtr = Rbc_ChainFirstLink(&pipeChain); linkPtr != NULL;
	linkPtr = Rbc_ChainNextLink(linkPtr)) {
	pipePtr = Rbc_ChainGetValue(linkPtr);
    ...
    DWORD *pidPtr)		/* (out) Id of child process. */
{
    ...
    *hProcessPtr = INVALID_HANDLE_VALUE;
	 ...
	 * as a sink.
	 */
	if ((Rbc_GetPlatformId() == VER_PLATFORM_WIN32_WINDOWS)
	    && (applType == APPL_DOS)) {
				 ...
				 * but it will never get any data. */
{
	...
	*errPipePtr = -1;
    ...
    procArr = ckalloc((unsigned)((cmdCount + 1) * sizeof(Process)));
    assert(procArr);
    thisInput = hStdin;
    if (argc == 0) {
    ...
    ClientData clientData)
{
    ...
    EnterCriticalSection(&pipeCriticalSection);
    Rbc_ChainAppend(&pipeChain, pipePtr);
    LeaveCriticalSection(&pipeCriticalSection);
}
 ...
 */
void
Rbc_DeleteFileHandler(int fd)	/* Descriptor or handle of file */
{
    PipeHandler *pipePtr;
    Rbc_ChainLink *linkPtr;
    HANDLE hPipe;
...
#if KILL_DEBUG
    PurifyPrintf("Rbc_DeleteFileHandler(%d)\n", fd);
#endif
    hPipe = (HANDLE) fd;
    EnterCriticalSection(&pipeCriticalSection);
	...
	pipePtr = Rbc_ChainGetValue(linkPtr);
	if ((pipePtr->hPipe == hPipe) && !(pipePtr->flags & PIPE_DELETED)) {
	    Rbc_ChainDeleteLink(&pipeChain, linkPtr);
	    DeletePipeHandler(pipePtr);
	    break;
	}
    }
    LeaveCriticalSection(&pipeCriticalSection);
#if KILL_DEBUG
    PurifyPrintf("Rbc_DeleteFileHandler: done\n");
#endif
}
    ...
    unsigned int size)
{
...
#if ASYNC_DEBUG
    PurifyPrintf("Rbc_AsyncRead(f=%d)\n", f);
#endif
    pipePtr = GetPipeHandler((HANDLE) f);
    if ((pipePtr == NULL) || (pipePtr->flags & PIPE_DELETED)) {
	errno = EBADF;
#if ASYNC_DEBUG
	PurifyPrintf("Rbc_AsyncRead: bad file\n");
#endif
	return -1;
    }
    if (!PeekOnPipe(pipePtr, &nBytes)) {
#if ASYNC_DEBUG
	PurifyPrintf("Rbc_AsyncRead: pipe is drained (nBytes=%d).\n", nBytes);
#endif
    ...
    if (nBytes == -1) {
#if ASYNC_DEBUG
	PurifyPrintf("Rbc_AsyncRead: Error\n");
#endif
	return -1;
    }
    if (nBytes == 0) {
#if ASYNC_DEBUG
	PurifyPrintf("Rbc_AsyncRead: EOF\n");
#endif
	return 0;
    }
    count = pipePtr->end - pipePtr->start;
#if ASYNC_DEBUG
    PurifyPrintf("Rbc_AsyncRead: nBytes is %d, %d\n", nBytes, count);
#endif
...
#if ASYNC_DEBUG
	PurifyPrintf("Rbc_AsyncRead: signaling idle\n");
#endif
