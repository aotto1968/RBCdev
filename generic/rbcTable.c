...
#define TABLE_THREAD_KEY	"RBC Table Data"
...
static Rbc_Uid rowUid, columnUid;
...
static Tk_ConfigSpec entryConfigSpecs[] =
{
    ...
    {TK_CONFIG_CUSTOM, "-padx", (char *)NULL, (char *)NULL,
	(char *)NULL, Tk_Offset(Entry, padX), 0, &rbcPadOption},
    {TK_CONFIG_CUSTOM, "-pady", (char *)NULL, (char *)NULL,
	(char *)NULL, Tk_Offset(Entry, padY), 0, &rbcPadOption},
    {TK_CONFIG_CUSTOM, "-ipadx", (char *)NULL, (char *)NULL,
	(char *)NULL, Tk_Offset(Entry, ipadX), 0, &rbcDistanceOption},
    {TK_CONFIG_CUSTOM, "-ipady", (char *)NULL, (char *)NULL,
	(char *)NULL, Tk_Offset(Entry, ipadY), 0, &rbcDistanceOption},
...
#define STRING_SPACE 200
	...
	Tcl_DStringAppendElement(&buffer, Rbc_Itoa(limitsPtr->min));
    } else {
	...
	Tcl_DStringAppendElement(&buffer, Rbc_Itoa(limitsPtr->max));
    } else {
	Tcl_DStringAppendElement(&buffer, "");
    }
	...
	Tcl_DStringAppendElement(&buffer, Rbc_Itoa(limitsPtr->nom));
    } else {
	Tcl_DStringAppendElement(&buffer, "");
    }
    ...
    Tk_Window tkwin;		/* Widget associated with table entry */
{
    ...
    hPtr = Tcl_FindHashEntry(&(tablePtr->entryTable), (char *)tkwin);
    if (hPtr == NULL) {
	return NULL;
    }
    ...
    Tk_Window tkwin;
{
     ...
     */
    entryPtr->linkPtr = Rbc_ChainAppend(tablePtr->chainPtr, entryPtr);
    entryPtr->hashPtr = Tcl_CreateHashEntry(&(tablePtr->entryTable),
	(char *)tkwin, &dummy);
    Tcl_SetHashValue(entryPtr->hashPtr, entryPtr);
    ...
    Entry *entryPtr;
{
	...
	Rbc_ChainDeleteLink(entryPtr->row.chainPtr, entryPtr->row.linkPtr);
    }
    if (entryPtr->column.linkPtr != NULL) {
	Rbc_ChainDeleteLink(entryPtr->column.chainPtr,
	    entryPtr->column.linkPtr);
    }
    if (entryPtr->linkPtr != NULL) {
	Rbc_ChainDeleteLink(tablePtr->chainPtr, entryPtr->linkPtr);
    }
    ...
    Tcl_DString *resultPtr;
{
	...
	Tcl_DStringAppend(resultPtr, Rbc_Itoa(entryPtr->ipadX), -1);
    }
    if (entryPtr->ipadY != ENTRY_DEF_PAD) {
	Tcl_DStringAppend(resultPtr, " -ipady ", -1);
	Tcl_DStringAppend(resultPtr, Rbc_Itoa(entryPtr->ipadY), -1);
    }
    if (entryPtr->row.span != ENTRY_DEF_SPAN) {
	Tcl_DStringAppend(resultPtr, " -rowspan ", -1);
	Tcl_DStringAppend(resultPtr, Rbc_Itoa(entryPtr->row.span), -1);
    }
    if (entryPtr->column.span != ENTRY_DEF_SPAN) {
	Tcl_DStringAppend(resultPtr, " -columnspan ", -1);
	Tcl_DStringAppend(resultPtr, Rbc_Itoa(entryPtr->column.span), -1);
    }
	...
	Tcl_DStringAppend(resultPtr, Rbc_NameOfFill(entryPtr->fill), -1);
    }
...
CreateRowColumn()
{
    ...
    rcPtr = ckalloc(sizeof(RowColumn));
    ...
    int *numberPtr;
{
    ...
    if ((n < 0) || (n >= Rbc_ChainGetLength(infoPtr->chainPtr))) {
    ...
    int n;
{
    ...
    for (i = Rbc_ChainGetLength(infoPtr->chainPtr); i <= n; i++) {
	rcPtr = CreateRowColumn();
	rcPtr->index = i;
	rcPtr->linkPtr = Rbc_ChainAppend(infoPtr->chainPtr, (ClientData)rcPtr);
    }
    linkPtr = Rbc_ChainGetNthLink(infoPtr->chainPtr, n);
    if (linkPtr == NULL) {
	return NULL;
    }
    ...
    RowColumn *rcPtr;
{
	...
	for (linkPtr = Rbc_ChainFirstLink(tablePtr->chainPtr); linkPtr != NULL;
	    linkPtr = nextPtr) {
    ...
    char **argv;
{
    ...
    nMatches = 0;
    for (linkPtr = Rbc_ChainFirstLink(infoPtr->chainPtr); linkPtr != NULL;
	linkPtr = Rbc_ChainNextLink(linkPtr)) {
	rcPtr = Rbc_ChainGetValue(linkPtr);
    ...
    Tcl_DString *resultPtr;
{
	...
	Tcl_DStringAppend(resultPtr, Rbc_Dtoa(interp, rcPtr->weight), -1);
    }
    ...
    char *pattern;
{
    ...
    Tcl_DStringInit(&dString);
    lastPtr = Rbc_ChainLastLink(infoPtr->chainPtr);
    for (linkPtr = Rbc_ChainFirstLink(infoPtr->chainPtr); linkPtr != NULL;
	linkPtr = Rbc_ChainNextLink(linkPtr)) {
	rcPtr = Rbc_ChainGetValue(linkPtr);
    ...
    int start, span;
{
    ...
    length = Rbc_ChainGetLength(infoPtr->chainPtr);
    for (i = length; i < (start + span); i++) {
	rcPtr = CreateRowColumn();
	rcPtr->index = i;
	rcPtr->linkPtr = Rbc_ChainAppend(infoPtr->chainPtr, (ClientData)rcPtr);
    }
    linkPtr = Rbc_ChainGetNthLink(infoPtr->chainPtr, start);
    return Rbc_ChainGetValue(linkPtr);
}

/*
...
/*LINTLIBRARY*/
int
Rbc_GetTable(dataPtr, interp, pathName, tablePtrPtr)
    ...
    Table **tablePtrPtr;
{
    ...
    hPtr = Tcl_FindHashEntry(&(dataPtr->tableTable), (char *)tkwin);
    if (hPtr == NULL) {
	Tcl_AppendResult(interp, "no table associated with widget \"",
	    pathName, "\"", (char *)NULL);
	return TCL_ERROR;
    }
    *tablePtrPtr = (Table *)Tcl_GetHashValue(hPtr);
    return TCL_OK;
}
    ...
    PartitionInfo *infoPtr;
{
	...
	for (node = Rbc_ListFirstNode(infoPtr->list); node != NULL;
	    node = Rbc_ListNextNode(node)) {
	    chainPtr = (Rbc_Chain *)Rbc_ListGetValue(node);
	    if (chainPtr != NULL) {
		Rbc_ChainDestroy(chainPtr);
	    }
	}
	Rbc_ListDestroy(infoPtr->list);
    }
    if (infoPtr->chainPtr != NULL) {
	Rbc_ChainLink *linkPtr;
	RowColumn *rcPtr;

	for (linkPtr = Rbc_ChainFirstLink(infoPtr->chainPtr);
	    linkPtr != NULL; linkPtr = Rbc_ChainNextLink(linkPtr)) {
	    rcPtr = Rbc_ChainGetValue(linkPtr);
	    ckfree(rcPtr);
	}
	Rbc_ChainDestroy(infoPtr->chainPtr);
    }
}
    ...
    DestroyData dataPtr;	/* Table structure */
{
    ...
    Rbc_ChainDestroy(tablePtr->chainPtr);

    DestroyPartitions(&(tablePtr->rowInfo));
    DestroyPartitions(&(tablePtr->columnInfo));
    Tcl_DeleteHashTable(&(tablePtr->entryTable));
    if (tablePtr->hashPtr != NULL) {
	Tcl_DeleteHashEntry(tablePtr->hashPtr);
    }
    ckfree(tablePtr);
}
    ...
    Entry *entryPtr;
{
     ...
     */
    if (entryPtr->column.linkPtr != NULL) {
	Rbc_ChainUnlinkLink(entryPtr->column.chainPtr,
	    entryPtr->column.linkPtr);
    }
    if (entryPtr->row.linkPtr != NULL) {
	Rbc_ChainUnlinkLink(entryPtr->row.chainPtr, entryPtr->row.linkPtr);
    }
    list = tablePtr->rowInfo.list;
    key = 0;			/* Initialize key to bogus span */
    for (node = Rbc_ListFirstNode(list); node != NULL;
	node = Rbc_ListNextNode(node)) {
	key = (int)Rbc_ListGetKey(node);
	if (entryPtr->row.span <= key) {
	    break;
	}
    }
    if (key != entryPtr->row.span) {
	Rbc_ListNode newNode;

	/*
	 * Create a new list (bucket) to hold entries of that size
	 * span and and link it into the list of buckets.
	 */
	newNode = Rbc_ListCreateNode(list, (char *)entryPtr->row.span);
	Rbc_ListSetValue(newNode, (char *)Rbc_ChainCreate());
	Rbc_ListLinkBefore(list, newNode, node);
	node = newNode;
    }
    chainPtr = (Rbc_Chain *) Rbc_ListGetValue(node);
    if (entryPtr->row.linkPtr == NULL) {
	entryPtr->row.linkPtr = Rbc_ChainAppend(chainPtr, entryPtr);
    } else {
	Rbc_ChainLinkBefore(chainPtr, entryPtr->row.linkPtr, NULL);
    }
	 ...
	 * span and and link it into the list of buckets.
	 */
	newNode = Rbc_ListCreateNode(list, (char *)entryPtr->column.span);
	Rbc_ListSetValue(newNode, (char *)Rbc_ChainCreate());
	Rbc_ListLinkBefore(list, newNode, node);
	node = newNode;
    }
    ...
    if (entryPtr->column.linkPtr == NULL) {
	entryPtr->column.linkPtr =
	    Rbc_ChainAppend(chainPtr, entryPtr);
    } else {
	Rbc_ChainLinkBefore(chainPtr, entryPtr->column.linkPtr, NULL);
    }
    ...
    Entry *entryPtr;
{
    ...
    startPtr = Rbc_ChainGetValue(linkPtr);
				 ...
				 * amount of space to remove */
{
	...
	for (linkPtr = Rbc_ChainFirstLink(chainPtr);
    ...
    Table *tablePtr;
{
    ...
    for (node = Rbc_ListFirstNode(infoPtr->list); node != NULL;
	node = Rbc_ListNextNode(node)) {
	chainPtr = Rbc_ChainGetValue(node);
    ...
    ClientData clientData;
{
     ...
     */
    if ((Rbc_ChainGetLength(tablePtr->chainPtr) == 0) ||
	(tablePtr->tkwin == NULL)) {
    ...
    for (linkPtr = Rbc_ChainFirstLink(tablePtr->columnInfo.chainPtr);
	linkPtr != NULL; linkPtr = Rbc_ChainNextLink(linkPtr)) {
    ...
    for (linkPtr = Rbc_ChainFirstLink(tablePtr->rowInfo.chainPtr);
	linkPtr != NULL; linkPtr = Rbc_ChainNextLink(linkPtr)) {
	rowPtr = Rbc_ChainGetValue(linkPtr);
				 ...
				 * the table */
{
    ...
    if (Rbc_GetTable(dataPtr, interp, argv[2], &tablePtr) != TCL_OK) {
	return TCL_ERROR;
    }
    tablePtr->flags |= REQUEST_LAYOUT;
    ...
    char **argv;
{
	...
	for (linkPtr = Rbc_ChainFirstLink(infoPtr->chainPtr); linkPtr != NULL;
		...
		matches++;
		DeleteRowColumn(tablePtr, infoPtr, rcPtr);
		Rbc_ChainDeleteLink(infoPtr->chainPtr, linkPtr);
	    }
	}
    }
...
{
    Table *tablePtr;
    Rbc_ChainLink *linkPtr, *nextPtr, *fromPtr;
    ...
    fromPtr = Rbc_ChainGetNthLink(infoPtr->chainPtr, from);
    rcPtr = Rbc_ChainGetValue(fromPtr);
    ...
    linkPtr = Rbc_ChainNextLink(fromPtr);
    for (i = from + 1; i <= to; i++) {
	nextPtr = Rbc_ChainNextLink(linkPtr);
	rcPtr = Rbc_ChainGetValue(linkPtr);
	DeleteRowColumn(tablePtr, infoPtr, rcPtr);
    ...
    char **argv;
{
	...
	for (hPtr = Tcl_FirstHashEntry(&(dataPtr->tableTable), &cursor);
...
{ 
    ...
    RowColumn *rcPtr;
    register int i;
    Rbc_ChainLink *beforePtr, *linkPtr;
    int linkBefore;
    ...
    beforePtr = Rbc_ChainGetNthLink(infoPtr->chainPtr, before);
    /*
	...
	Rbc_ChainSetValue(linkPtr, rcPtr);
	if (linkBefore) {
	    Rbc_ChainLinkBefore(infoPtr->chainPtr, linkPtr, beforePtr);
	} else {
	    Rbc_ChainLinkAfter(infoPtr->chainPtr, linkPtr, beforePtr);
	}
    ...
    char **argv;
{
    ...
    RowColumn *rcPtr;
    register int i;
    Rbc_ChainLink *afterPtr, *linkPtr;
    Entry *entryPtr;
    ...
    afterPtr = Rbc_ChainGetNthLink(infoPtr->chainPtr, number);
	...
	linkPtr = Rbc_ChainNewLink();
	Rbc_ChainSetValue(linkPtr, rcPtr);
	Rbc_ChainLinkAfter(infoPtr->chainPtr, linkPtr, afterPtr);
	rcPtr->linkPtr = linkPtr;
    }
    ...
    char **argv;
{
    ...
    if (Rbc_GetPixels(interp, tablePtr->tkwin, argv[3], PIXELS_ANY, &x)
	!= TCL_OK) {
	return TCL_ERROR;
    }
    if (Rbc_GetPixels(interp, tablePtr->tkwin, argv[4], PIXELS_ANY, &y)
	!= TCL_OK) {
	return TCL_ERROR;
    }
    ...
    Tcl_AppendElement(interp, Rbc_Itoa(rowPtr->index));
    Tcl_AppendElement(interp, Rbc_Itoa(columnPtr->index));
    return TCL_OK;
}
    ...
    char **argv;
{
    ...
    Tcl_DStringAppend(&dString, " \\\n", -1);
    lastPtr = Rbc_ChainLastLink(tablePtr->chainPtr);
    for (linkPtr = Rbc_ChainFirstLink(tablePtr->chainPtr); linkPtr != NULL;
	linkPtr = Rbc_ChainNextLink(linkPtr)) {
	entryPtr = Rbc_ChainGetValue(linkPtr);
	PrintEntry(entryPtr, &dString);
	...
	Tcl_DStringAppend(&dString, Rbc_Itoa(rcPtr->index), -1);
 ...
 * Table operations.
 *
 * The fields for Rbc_OpSpec are as follows:
 *
 *   - operation name
...
{
    {"arrange", 1, (Rbc_Op)ArrangeOp, 3, 3, "container",},
    {"cget", 2, (Rbc_Op)CgetOp, 4, 5,
	"container ?row|column|widget? option",},
    {"configure", 3, (Rbc_Op)ConfigureOp, 3, 0,
	"container ?row|column|widget?... ?option value?...",},
    {"containers", 3, (Rbc_Op)ContainersOp, 2, 4, "?switch? ?arg?",},
    {"delete", 1, (Rbc_Op)DeleteOp, 3, 0,
	"container row|column ?row|column?",},
    {"extents", 1, (Rbc_Op)ExtentsOp, 4, 4,
	"container row|column|widget",},
    {"forget", 1, (Rbc_Op)ForgetOp, 3, 0, "widget ?widget?...",},
    {"info", 3, (Rbc_Op)InfoOp, 3, 0,
	"container ?row|column|widget?...",},
    {"insert", 3, (Rbc_Op)InsertOp, 4, 6,
	"container ?-before|-after? row|column ?count?",},
    {"join", 1, (Rbc_Op)JoinOp, 5, 5, "container first last",},
    {"locate", 2, (Rbc_Op)LocateOp, 5, 5, "container x y",},
    {"save", 2, (Rbc_Op)SaveOp, 3, 3, "container",},
    {"search", 2, (Rbc_Op)SearchOp, 3, 0, "container ?switch arg?...",},
    {"split", 2, (Rbc_Op)SplitOp, 4, 5, "container row|column div",},
};
    ...
    char **argv;
{
	...
	if (Rbc_GetTable(clientData, interp, argv[1], &tablePtr) != TCL_OK) {
    ...
    Tcl_Interp *interp;
{
    ...
    Tcl_DeleteHashTable(&(dataPtr->tableTable));
     ...
     Tcl_Interp *interp;
{
    ...
    if (dataPtr == NULL) {
	dataPtr = ckalloc(sizeof(TableInterpData));
	assert(dataPtr);
	Tcl_SetAssocData(interp, TABLE_THREAD_KEY, TableInterpDeleteProc, 
		dataPtr);
	Tcl_InitHashTable(&(dataPtr->tableTable), TCL_ONE_WORD_KEYS);
    }
    return dataPtr;
}
    ...
    Tcl_Interp *interp;
{
    ...
    rowUid = (Rbc_Uid)Tk_GetUid("row");
    columnUid = (Rbc_Uid)Tk_GetUid("column");
    return TCL_OK;
}
