 ...
 */
extern int Rbc_CreatePipeline _ANSI_ARGS_((Tcl_Interp *interp, int argc,
...
static Rbc_SwitchParseProc StringToSignal;
static Rbc_SwitchCustom killSignalSwitch =
{
    StringToSignal, (Rbc_SwitchFreeProc *)NULL, (ClientData)0,
};

static Rbc_SwitchSpec switchSpecs[] = 
{
    {RBC_SWITCH_STRING, "-decodeoutput", 
         Rbc_Offset(BackgroundInfo, outputEncodingName), 0},
    {RBC_SWITCH_STRING, "-decodeerror", 
         Rbc_Offset(BackgroundInfo, errorEncodingName), 0},
    {RBC_SWITCH_BOOLEAN, "-echo", 
         Rbc_Offset(BackgroundInfo, sink2.echo), 0},
    {RBC_SWITCH_STRING, "-error", 
         Rbc_Offset(BackgroundInfo, sink2.doneVar), 0},
    {RBC_SWITCH_STRING, "-update", 
	Rbc_Offset(BackgroundInfo, sink1.updateVar), 0},
    {RBC_SWITCH_STRING, "-output", 
        Rbc_Offset(BackgroundInfo, sink1.doneVar), 0},
    {RBC_SWITCH_STRING, "-lasterror", 
	Rbc_Offset(BackgroundInfo, sink2.updateVar), 0},
    {RBC_SWITCH_STRING, "-lastoutput", 
	Rbc_Offset(BackgroundInfo, sink1.updateVar), 0},
    {RBC_SWITCH_LIST, "-onerror", 
	Rbc_Offset(BackgroundInfo, sink2.updateCmd), 0},
    {RBC_SWITCH_LIST, "-onoutput", 
	Rbc_Offset(BackgroundInfo, sink1.updateCmd), 0},
    {RBC_SWITCH_BOOLEAN, "-keepnewline", 
	Rbc_Offset(BackgroundInfo, keepNewline), 0},
    {RBC_SWITCH_BOOLEAN, "-check", 
	Rbc_Offset(BackgroundInfo, interval), 0},
    {RBC_SWITCH_CUSTOM, "-killsignal", 
	Rbc_Offset(BackgroundInfo, signalNum), 0, &killSignalSwitch},
    {RBC_SWITCH_BOOLEAN, "-linebuffered", 
	Rbc_Offset(BackgroundInfo, lineBuffered), 0},
    ...
    Sink *sinkPtr;
{
    ...
    arrayPtr = ckalloc(sizeof(unsigned char) * sinkPtr->size);
    if (arrayPtr == NULL) {
	return -1;
    }
...
FreeBackgroundInfo(bgPtr)
    BackgroundInfo *bgPtr;
{
    Rbc_FreeSwitches(switchSpecs, (char *)bgPtr, 0);
...
#else
    Tcl_DStringAppendElement(&dString, Rbc_Itoa(lastPid));
#endif
    Tcl_DStringAppendElement(&dString, Rbc_Itoa(code));
    switch(pcode) {
    case PROCESS_EXITED:
    ...
    char **argv;		/* Argument strings. */
{
    ...
    bgPtr->statVar = RbcStrdup(argv[1]);

    /* Try to clean up any detached processes */
    Tcl_ReapDetachedProcs();

    i = Rbc_ProcessSwitches(interp, switchSpecs, argc - 2, argv + 2, 
	(char *)bgPtr, RBC_SWITCH_ARGV_PARTIAL);
    if (i < 0) {
	FreeBackgroundInfo(bgPtr);
	return TCL_ERROR;
    }
    ...
    nProcs = Rbc_CreatePipeline(interp, argc - i, argv + i, &pidPtr,
