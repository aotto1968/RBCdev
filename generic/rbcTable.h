...
typedef struct {
    Tcl_HashTable tableTable;	/* Hash table of table structures keyed by 
...
typedef struct  {
	...
	Rbc_ChainLink *linkPtr;	/* Link to widget in the chain of spans */

	Rbc_Chain *chainPtr;	/* Pointer to the chain of spans. */
    } row, column;
    ...
    Rbc_Pad padX;		/* Extra padding placed left and right of the
				 * widget. */
    Rbc_Pad padY;		/* Extra padding placed above and below the
				 * widget */
    ...
    Rbc_ChainLink *linkPtr;	/* Pointer into list of entries. */

    Tcl_HashEntry *hashPtr;	/* Pointer into table of entries. */
...
struct RowColumnStruct {
    ...
    Rbc_Pad pad;		/* Pads the partition beyond its nominal
				 * size */
 ...
 */
#if defined(DEF_ARRAY_SIZE)
#  undef DEF_ARRAY_SIZE
#endif
...
typedef struct PartitionInfo {
    ...
    Rbc_Chain *chainPtr;
    Rbc_List list;		/* Linked list of bins of widgets
...
struct TableStruct {
    ...
    Rbc_Chain *chainPtr;	/* Chain of entries in the table. */

    Tcl_HashTable entryTable;	/* Table of entries.  Serves as a
    ...
    Tcl_HashEntry *hashPtr;	/* Used to delete the table from its
				 * hashtable. */
...
extern int Rbc_GetTable _ANSI_ARGS_((TableInterpData *dataPtr, 
