...
#
set rbcTabnotebook(activate) yes
...
bind Tabnotebook <ButtonPress-2> {
    set rbcTabnotebook(cursor) [%W cget -cursor]
    set rbcTabnotebook(activate) no
    %W configure -cursor hand1
    %W scan mark %x %y
}

bind Tabnotebook <ButtonRelease-2> {
    %W configure -cursor $rbcTabnotebook(cursor)
    set rbcTabnotebook(activate) yes
    %W activate @%x,%y
}
...
#
# ----------------------------------------------------------------------
bind Tabnotebook <KeyPress-Up> { rbc::SelectTab %W "up" }
bind Tabnotebook <KeyPress-Down> { rbc::SelectTab %W "down" }
bind Tabnotebook <KeyPress-Right> { rbc::SelectTab %W "right" }
bind Tabnotebook <KeyPress-Left> { rbc::SelectTab %W "left" }
...
#
# ----------------------------------------------------------------------
proc rbc::FindMatchingTab { widget key } {
...
#
# ----------------------------------------------------------------------
proc rbc::DestroyTearoff { widget tab } {
...
#
# ----------------------------------------------------------------------
proc rbc::CreateTearoff { widget tab rootX rootY } {
    ...
    wm protocol $top WM_DELETE_WINDOW [list rbc::DestroyTearoff $widget $tab]
...
#
# ----------------------------------------------------------------------
proc rbc::ToggleTearoff { widget x y index } {
    ...
    if { $container == "$widget" } {
	rbc::CreateTearoff $widget $tab $x $y
    } elseif { $container != "" } {
	rbc::DestroyTearoff $widget $tab
    }
}
...
#
# ----------------------------------------------------------------------
proc rbc::TabnotebookInit { widget } {
    $widget bind all <Enter> { 
	if { $rbcTabnotebook(activate) } {
	    %W activate current
        }
    }
	...
	rbc::SelectTab %W "current"
    }
    $widget bind all <Control-ButtonPress-1> { 
	rbc::ToggleTearoff %W %X %Y active
    }
    $widget configure -perforationcommand {
	rbc::ToggleTearoff %W $rbcTabnotebook(x) $rbcTabnotebook(y) select
    }
	...
	set rbcTabnotebook(x) %X
	set rbcTabnotebook(y) %Y
