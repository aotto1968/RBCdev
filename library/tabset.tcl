...
bind Tabset <ButtonPress-2> {
    set rbcTabset(cursor) [%W cget -cursor]
    set rbcTabset(activate) no
    %W configure -cursor hand1
    %W scan mark %x %y
}

bind Tabset <ButtonRelease-2> {
    %W configure -cursor $rbcTabset(cursor)
    set rbcTabset(activate) yes
    %W activate @%x,%y
}
...
#
# ----------------------------------------------------------------------
bind Tabset <KeyPress-Up> { rbc::SelectTab %W "up" }
bind Tabset <KeyPress-Down> { rbc::SelectTab %W "down" }
bind Tabset <KeyPress-Right> { rbc::SelectTab %W "right" }
bind Tabset <KeyPress-Left> { rbc::SelectTab %W "left" }
...
#
# ----------------------------------------------------------------------
proc rbc::Tearoff { widget x y index } {
...
#
# ----------------------------------------------------------------------
proc rbc::TabsetInit { widget } {
    $widget bind all <Enter> { 
	if { $rbcTabset(activate) } {
	    %W activate current
        }
    }
	...
	if { [%W cget -tearoff] } {
	    rbc::Tearoff %W %X %Y active
	}
    }
    $widget configure -perforationcommand {
	rbc::Tearoff %W $rbcTabset(x) $rbcTabset(y) select
    }
