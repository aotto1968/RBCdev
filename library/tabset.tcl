...
namespace eval ::rbc::Tabset {
  ...
  bind Tabset <ButtonPress-2> [namespace code {
  ...
  bind Tabset <ButtonRelease-2> [namespace code {
  ...
  #
  # ----------------------------------------------------------------------
  bind Tabset <KeyPress-Up>     [namespace code { SelectTab %W "up" }]
  bind Tabset <KeyPress-Down>   [namespace code { SelectTab %W "down" }]
  bind Tabset <KeyPress-Right>  [namespace code { SelectTab %W "right" }]
  bind Tabset <KeyPress-Left>   [namespace code { SelectTab %W "left" }]
  bind Tabset <KeyPress-space>  [namespace code { %W invoke focus }]
  bind Tabset <KeyPress-Return> [namespace code { %W invoke focus }]

  bind Tabset <KeyPress> [namespace code {
  ...
  #
  # ----------------------------------------------------------------------
  proc FindMatchingTab { widget key } {
  ...
  #
  # ----------------------------------------------------------------------
  proc DestroyTearoff { widget tab } {
  ...
  #
  # ----------------------------------------------------------------------
  proc CreateTearoff { widget tab rootX rootY } {
      ...
      #::rbc::winop changes $top

      # 
      # If the user tries to delete the toplevel, put the window back
      # into the tab folder.  
      #
      wm protocol $top WM_DELETE_WINDOW [namespace code [list DestroyTearoff $widget $tab]]
      # 
  ...
  proc Tearoff { widget x y index } {
      ...
      if { $container == "$widget" } {
          CreateTearoff $widget $tab $x $y
      } elseif { $container != "" } {
          DestroyTearoff $widget $tab
      }
  }
  ...
  # ----------------------------------------------------------------------
  proc ObjInit { widget } {
      $widget bind all <Enter> [namespace code { 
          if { $activate } {
              %W activate current
          }
      }]
      ...
      $widget bind all <ButtonPress-1> [namespace code { 
          SelectTab %W "current"
      }]
      $widget bind all <Control-ButtonPress-1> [namespace code { 
          if { [%W cget -tearoff] } {
              Tearoff %W %X %Y active
          }
      }]
      $widget configure -perforationcommand [namespace code {
          Tearoff %W $x $y select
      }]
      ...
      $widget bind Perforation <ButtonPress-1> [namespace code { 
          set x %X
          set y %Y
