...
bind Hierbox <ButtonPress-2> {
    set rbcHierbox(cursor) [%W cget -cursor]
...
bind Hierbox <ButtonRelease-2> {
    %W configure -cursor $rbcHierbox(cursor)
}

bind Hierbox <B1-Leave> {
    if { $rbcHierbox(scroll) } {
	rbc::Hierbox::AutoScroll %W 
    }
}

bind Hierbox <B1-Enter> {
    after cancel $rbcHierbox(afterId)
}
...
bind Hierbox <Shift-KeyPress-Up> {
    rbc::Hierbox::MoveFocus %W prevsibling
}

bind Hierbox <Shift-KeyPress-Down> {
    rbc::Hierbox::MoveFocus %W nextsibling
}

bind Hierbox <KeyPress-Prior> {
    rbc::Hierbox::MovePage %W top
}

bind Hierbox <KeyPress-Next> {
    rbc::Hierbox::MovePage %W bottom
}
...
bind Hierbox <KeyPress-space> {
    rbc::HierboxToggle %W focus
    set rbcHierbox(space) on
}

bind Hierbox <KeyRelease-space> { 
    set rbcHierbox(space) off
}
...
bind Hierbox <KeyPress> {
    rbc::Hierbox::NextMatchingEntry %W %A
}

bind Hierbox <KeyPress-Home> {
    rbc::Hierbox::MoveFocus %W root
}

bind Hierbox <KeyPress-End> {
    rbc::Hierbox::MoveFocus %W end
}
...
# ----------------------------------------------------------------------
proc rbc::HierboxToggle { widget index } {
...
proc rbc::Hierbox::MovePage { widget where } {
...
proc rbc::Hierbox::Init { widget } {
	...
	rbc::Hierbox::SetSelectionAnchor %W current
	set rbcHierbox(scroll) 1
    }
	...
	if { [%W cget -selectmode] == "multiple" } {
	    ...
	    rbc::Hierbox::SetSelectionAnchor %W $index
	}	    
    }
...
#
# ----------------------------------------------------------------------
proc rbc::Hierbox::AutoScroll { widget } {
    global rbcHierbox
    ...
    if { [$widget cget -selectmode] == "single" } {
	rbc::Hierbox::SetSelectionAnchor $widget $neighbor
    } else {
	$widget selection mark $index
    }
    set rbcHierbox(afterId) [after 10 rbc::Hierbox::AutoScroll $widget]
}

proc rbc::Hierbox::SetSelectionAnchor { widget index } {
...
proc rbc::Hierbox::NextMatchingEntry { widget key } {
    if {[string match {[ -~]} $key]} {
...
# "nextsibling", etc.
# ----------------------------------------------------------------------
proc rbc::Hierbox::MoveFocus { widget where } {
