...
image create photo rbc::tv::normalCloseFolder -format gif -data {
    R0lGODlhEAANAMIAAAAAAH9/f///////AL+/vwAA/wAAAAAAACH5BAEAAAUALAAAAAAQAA0A
    AAM8WBrM+rAEQWmIb5KxiWjNInCkV32AJHRlGQBgDA7vdN4vUa8tC78qlrCWmvRKsJTquHkp
    ZTKAsiCtWq0JADs=
}
image create photo rbc::tv::normalOpenFolder -format gif -data {
    R0lGODlhEAANAMIAAAAAAH9/f///////AL+/vwAA/wAAAAAAACH5BAEAAAUALAAAAAAQAA0A
    AAM1WBrM+rAEMigJ8c3Kb3OSII6kGABhp1JnaK1VGwjwKwtvHqNzzd263M3H4n2OH1QBwGw6
    nQkAOw==
}
image create photo rbc::tv::activeCloseFolder -format gif -data {
    R0lGODlhEAANAMIAAAAAAH9/f/////+/AL+/vwAA/wAAAAAAACH5BAEAAAUALAAAAAAQAA0A
    AAM8WBrM+rAEQWmIb5KxiWjNInCkV32AJHRlGQBgDA7vdN4vUa8tC78qlrCWmvRKsJTquHkp
    ZTKAsiCtWq0JADs=
}
image create photo rbc::tv::activeOpenFolder -format gif -data {
    R0lGODlhEAANAMIAAAAAAH9/f/////+/AL+/vwAA/wAAAAAAACH5BAEAAAUALAAAAAAQAA0A
    AAM1WBrM+rAEMigJ8c3Kb3OSII6kGABhp1JnaK1VGwjwKwtvHqNzzd263M3H4n2OH1QBwGw6
    nQkAOw==
}
...
proc rbc::tv::Initialize { w } {
	...
	rbc::tv::SetSelectionAnchor %W current
	set rbc::tv::scroll 1
    }
	...
	if { [%W cget -selectmode] == "multiple" } {
	    ...
	    rbc::tv::SetSelectionAnchor %W $index
	}
    }
	...
	set rbc::tv::column [%W column current]
	%W column configure $rbc::tv::column -titlerelief sunken
    }
	...
	%W column configure $rbc::tv::column -titlerelief raised
    }
...
proc rbc::tv::AutoScroll { w } {
	...
	rbc::tv::SetSelectionAnchor $w $neighbor
    } else {
	$w selection mark $index
    }
    set ::rbc::tv::afterId [after 50 rbc::tv::AutoScroll $w]
}

proc rbc::tv::SetSelectionAnchor { w tagOrId } {
...
#
# ----------------------------------------------------------------------
proc rbc::tv::MoveFocus { w tagOrId } {
...
bind ${className} <ButtonPress-2> {
    set rbc::tv::cursor [%W cget -cursor]
...
bind ${className} <ButtonRelease-2> {
    %W configure -cursor $rbc::tv::cursor
}
...
bind ${className} <Shift-KeyPress-Up> {
    rbc::tv::MoveFocus %W prevsibling
}

bind ${className} <Shift-KeyPress-Down> {
    rbc::tv::MoveFocus %W nextsibling
}

bind ${className} <KeyPress-Prior> {
    rbc::tv::MovePage %W top
}

bind ${className} <KeyPress-Next> {
    rbc::tv::MovePage %W bottom
}
...
bind ${className} <KeyRelease-space> { 
    set rbc::tv::space off
}

bind ${className} <KeyPress-Return> {
    rbc::tv::MoveFocus %W focus
    set rbc::tv::space on
}
...
bind ${className} <KeyPress> {
    rbc::tv::NextMatch %W %A
}

bind ${className} <KeyPress-Home> {
    rbc::tv::MoveFocus %W top
}

bind ${className} <KeyPress-End> {
    rbc::tv::MoveFocus %W bottom
}
...
if { [string compare $tcl_platform(platform) "windows"] != 0 } {
	...
	catch {rbc::tv::InsertText %W [selection get -displayof %W]}
    }
}
...
if 0 {
	...
	%W icursor [rbc::tv::PreviousWord %W insert]
	%W selection clear
    }
    bind ${className}Editor <Meta-d> {
	%W delete insert [rbc::tv::NextWord %W insert]
    }
    bind ${className}Editor <Meta-f> {
	%W icursor [rbc::tv::NextWord %W insert]
	%W selection clear
    }
    bind ${className}Editor <Meta-BackSpace> {
	%W delete [rbc::tv::PreviousWord %W insert] insert
    }
    ...
    proc rbc::tv::PreviousWord {w start} {
