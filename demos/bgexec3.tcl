...
bitmap define rbc.5 [bitmap data rbc.3]
bitmap define rbc.6 [bitmap data rbc.2]
bitmap define rbc.7 [bitmap data rbc.1]
...
proc Animate {} {
    ...
    if { $animate(index) >= 0 } {
	.logo configure -bitmap rbc.$animate(index) 
	incr animate(index)
	if { $animate(index) >= 7 } {
	    set animate(index) 0
	}
