...
rbc::bitmap compose text "Text String" -font {courier 12}
...
foreach angle { 0 90 180 270 360 45 -45 101 } {
	...
	set data [rbc::bitmap data $bitmap]
	rbc::bitmap define $bitmap$row$column $data -rotate $angle
...
foreach scale { 1.0 0.5 0.75 1.4 3.0 } {
    ...
    foreach bitmap { hobbes gort xbob text } {
	set data [rbc::bitmap data $bitmap]
	rbc::bitmap define $bitmap$row$column $data -scale $scale
	label .$bitmap$row$column -bitmap $bitmap$row$column -bg white
	rbc::table . \
	    $row,$column .$bitmap$row$column
	incr column
...
foreach scale { 2.0 0.8 1.2 } angle { 45 -45 101 } {
    ...
    foreach bitmap { hobbes gort xbob text } {
	set data [rbc::bitmap data $bitmap]
	rbc::bitmap define $bitmap$row $data -scale $scale -rotate $angle
